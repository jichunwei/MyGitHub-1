...
from win32event import CreateEvent, WaitForSingleObject, INFINITE
import win32con
import exceptions
...
class SerialPort:
    ...
    def __init__(self, dev, timeout=None, speed=None, mode='232', params=None):
        ...
        try:
            self.__handle=w32.CreateFile (dev,
    ...
    def __configure(self):
        ...
        w32.SetCommMask(self.__handle, w32.EV_RXCHAR)
        # Setup a 4k buffer
        w32.SetupComm(self.__handle, 4096, 4096)
        # Remove anything that was there
        w32.PurgeComm(self.__handle, w32.PURGE_TXABORT|w32.PURGE_RXABORT|w32.PURGE_TXCLEAR|
                  w32.PURGE_RXCLEAR)
        ...
        w32.SetCommTimeouts(self.__handle, timeouts)

        # Setup the connection info
        dcb=w32.GetCommState(self.__handle)
        dcb.BaudRate=SerialPort.BaudRatesDic[self.__speed]
        if not self.__params:
            dcb.ByteSize=8
            dcb.Parity=w32.NOPARITY
            dcb.StopBits=w32.ONESTOPBIT
        else:
            dcb.ByteSize, dcb.Parity, dcb.StopBits=self.__params
        w32.SetCommState(self.__handle, dcb)
    ...
    def read(self, num=1):
        ...
        (hr, buff) = w32.ReadFile(self.__handle, int(num))
    ...
    def write(self, s):
        ...
        w32.WriteFile(self.__handle, s, overlapped)
        # Wait for the write to complete
        WaitForSingleObject(overlapped.hEvent, INFINITE)

    def inWaiting(self):
        """Returns the number of bytes waiting to be read"""
        flags, comstat = w32.ClearCommError(self.__handle)
