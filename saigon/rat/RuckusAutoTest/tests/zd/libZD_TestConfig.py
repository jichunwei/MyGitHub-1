...
def get_testbed_active_ap(testbed, active_ap, announce_name = 'ActiveAP'):
    ap_macAddr = testbed.get_ap_mac_addr_by_sym_name(active_ap)
    ap = get_active_ap(ap_macAddr, testbed.components['AP'])
    if ap:
        logging.info('%s(%s) found in testbed with mac=%s' % (announce_name, active_ap, ap.get_base_mac()))
    return ap

#@auth:yuyanan 2014-7-22
def get_active_ap_mac(testbed, active_ap_tag, announce_name = 'ActiveAP'):
    ap_macAddr = testbed.get_ap_mac_addr_by_sym_name(active_ap_tag)
    return ap_macAddr
...
def get_wlan_profile(profile_name):
    ...
    for idx in range(0, 32):
        wlan = open_none.copy()
        wlan['ssid'] = '%s-%d-wlan%d' % (wlan['ssid'], randint, idx + 1)
...
def get_hotspot_auth_params(zd, username, password,
                            original_url = '', redirect_url = ''):
    '''
    A utility method to get the Hotspot Authentication parameters
    Params changing across ZD's builds are defined in ZDFeatureUpdate
    ...
    hotspot_auth_params = zd.info['hotspot_auth_params']
    user_login_auth = hotspot_auth_params.pop('user_login_auth')
    user_login_auth.update({
        'username': username,
        'password': password,
    })
    hotspot_auth_params.update({
        'user_login_auth': user_login_auth
    })
