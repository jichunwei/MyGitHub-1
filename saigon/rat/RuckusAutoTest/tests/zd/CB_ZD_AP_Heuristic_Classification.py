...
class CB_ZD_AP_Heuristic_Classification(Test):
    ...
    def test(self):
        try:
            logging.info('Verify Heuristic Classification')
            ...
            if self.wlan_name:
                ...
                traffic_result = self._send_downlink_zing_traffic(self.host, self.num_of_pkts, self.len_of_pkt, self.pkt_gap, self.run_time)
    ...
    def _init_test_params(self, conf):
        ...
        default_qos_conf = {'expect_queue': 'video',
                            'num_of_pkts': 1000,
                            'len_of_pkt': 10,
                            'pkt_gap': 10,
                            'run_time': 30,
        ...
        qos_conf = default_qos_conf
        qos_conf.update(self.conf['qos_conf'])
        
        self.expect_queue = qos_conf['expect_queue']
        self.num_of_pkts = qos_conf['num_of_pkts']
        self.len_of_pkt = qos_conf['len_of_pkt']
        self.pkt_gap = qos_conf['pkt_gap']
        self.run_time = qos_conf['run_time']        
        self.expect_result = qos_conf['expect_result']
    ...
    def _send_downlink_zing_traffic(self, host, num_of_pkts, len_of_pkt, pkt_gap, run_time):
        '''
        ...
        traffic_result = self.target_station.send_zing(host = host, num_of_pkts = num_of_pkts,
                                                      len_of_pkt = len_of_pkt, delay = pkt_gap,
                                                      sending_time = run_time)
