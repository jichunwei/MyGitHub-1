...
def test_show_system_parameters(zd_cli):
    ...
    # Correct the error message - Nov 2010
    if system_parameters['ip_addr'] == 'The information is not existed or NULL'\
    or system_parameters['ip_addr'][0] != zd_cli.ip_addr:
        errmsg = '"ip_addr" value is "%s" instead of "%s"' % (system_parameters['ip_addr'], zd_cli.ip_addr)
        return (passmsg, errmsg)
...
def test_command_show_ap(zd, zd_cli):
    ...
    if ('illegal option' or 'command not found') in all_ap_info['raw_info']:
        errmsg = all_ap_info['raw_info']
        return (passmsg, errmsg)
    
    # An Nguyen, an.nguyen@ruckuswireless.com
    # Add step to verify if the ap info exist or not - Nov 2010
    if all_ap_info['ap'] == 'The information is not existed or NULL' \
    or all_ap_info['total_ap'] == 'The information is not existed or NULL':
        errmsg = all_ap_info['raw_info']
        return (passmsg, errmsg)
...
def test_command_show_station(zd_cli, expected_sta_info):
    ...
    if ('illegal option' or 'command not found') in all_sta_info['raw_info']:
        errmsg = all_sta_info['raw_info']
        return (passmsg, errmsg)
    
    # An Nguyen, an.nguyen@ruckuswireless.com
    # Add step to verify if the station info exist or not - Nov 2010
    if all_sta_info['sta'] == 'The information is not existed or NULL' \
    or all_sta_info['total_sta'] == 'The information is not existed or NULL':
        errmsg = all_sta_info['raw_info']
        return (passmsg, errmsg)
