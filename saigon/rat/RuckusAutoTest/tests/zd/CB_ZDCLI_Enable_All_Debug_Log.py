...
class CB_ZDCLI_Enable_All_Debug_Log(Test):
    ...
    def _enable_all_debug_log(self):
        logging.info('Enable all debug log on ZD')
        try:
            lib.zdcli.shell.enable_all_debug_log(self.zdcli)
            self.passmsg = 'Enable all debug log on ZD successfully'
        except Exception, e:
            self.errmsg = 'Failed to enable all debug log on ZD: %s' % e.message
