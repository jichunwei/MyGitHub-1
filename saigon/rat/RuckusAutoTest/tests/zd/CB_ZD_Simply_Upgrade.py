...
from contrib.download import image_helper

class CB_ZD_Simply_Upgrade(Test):
...
#        logging.info('upgrade img is :%s' % self.conf['image_file_path'])
        ...
        self.conf['image_file_path'] = self._download_build(self.conf.get('build_stream'),
                                                            self.conf.get('build_number')
                                                            )
        logging.info('upgrade img is :%s' % self.conf['image_file_path'])
    ...
    def _download_build(self, build_stream, build_number):
        build_url = image_helper.get_build_url(build_stream, build_number)

        if not build_url:
            errmsg = 'There is not any URL for the build %s.%s'
            errmsg = errmsg % (build_stream.split('_')[1], build_number)
            raise Exception(errmsg)
        filename = build_url.split('/')[-1]
        if image_helper.download_build_v2(build_url, filename):
            return filename
        else:
