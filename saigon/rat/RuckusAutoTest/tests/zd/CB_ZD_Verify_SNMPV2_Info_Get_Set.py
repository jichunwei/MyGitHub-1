...
class CB_ZD_Verify_SNMPV2_Info_Get_Set(Test):
    '''
    '''
    ...
    def _verify_snmpv3_agent_get_set(self):
        try:
            gui_snmpv2_agent_info = self.gui_d
            set_snmpv2_agent_info = self.conf['snmp_agent_cfg']
            
            if not gui_snmpv2_agent_info.has_key('version'):
                if set_snmpv2_agent_info.has_key('version'):
                    set_snmpv2_agent_info.pop('version')
           
            res = self._verify_two_dicts(gui_snmpv2_agent_info, set_snmpv2_agent_info)
            
            if res:
                self.errmsg = res
            else:
                self.passmsg = 'SNMPV2 agent info is the same between GUI Get and Set.'
