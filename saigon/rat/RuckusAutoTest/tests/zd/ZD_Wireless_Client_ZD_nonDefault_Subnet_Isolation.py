...
"""
Description: ZD_Wireless_Client_ZD_nonDefault_Subnet_Isolation Test class verifies if a client can associate to a WLAN
             created on ZoneDirectorand can not ping to an IP which does not belong to the ZoneDirector's
             default subnet (192.168.0.0/24) when the feature 'Wireless Client Isolation' is enabled.
             The test is confirmed via a ping test and the addresses acquired on the client.
...
Test_ID/Customer_bug_ID:
     - ZD_CLIENT_ISOLATION_0003

Pre-requisite:
     - 1 client supports 11g
     - ZF2942 and/or ZF2925 and/or ZF7942
     - A sub interface must be configured on the server with the ip address supplied by the parameter 'ip'.
     - A non-default subnet must be configured on the ZoneDirector.

Configuration and Parameters:
     {'target_station': 'ip address of target station',
      'ip':'An ip that is not in the subnet of the ZoneDirector',
...
Test Procedure:
     - Config:
         ...
         + Set the non-default subnet value to the restricted subnet.
     - Test:
         ...
         + Ping from the target station to the address specified by the parameter 'ip', make sure that this ping fails.
     - Cleanup:
         + Remove all WLAN(s) on the ZoneDirector.
         + Set the restrited subnets to their original values.
...
class ZD_Wireless_Client_ZD_nonDefault_Subnet_Isolation(Test):
    ...
    def test(self):
        ...
        # Ping to the ip address specified by the parameter 'ip', make sure this ping fails.
        logging.info("Ping to the ip '%s' from the target station (timeout: %s)" % (self.ip_to_ping, self.ping_timeout))
        ping_result = self.target_station.ping(self.ip_to_ping, self.ping_timeout * 1000)
        if ping_result.find("Timeout") != -1:
            logging.info("Ping FAILED. Correct behavior")
        else:
            logging.info("Ping OK. Incorrect behavior")
            return ("FAIL", "The target station could send traffic while client isolation is enabled.")
    ...
    def cleanup(self):
        ...
        if self.restricted_subnet_list:
            logging.info("Set the restricted subnets to their original values '%s'." % \
