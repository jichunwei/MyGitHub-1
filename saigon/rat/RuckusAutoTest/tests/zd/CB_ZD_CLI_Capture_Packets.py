...
from RuckusAutoTest.components.lib.zdcli import debug_mode_functions as dgf
...
class CB_ZD_CLI_Capture_Packets(Test):
    required_components = ['ZoneDirectorCLI', 'AP']
    ...
    def capture_packets_local(self):
        apcap.remove_capture_files(self.active_ap)
        
        start_capture_cfg = {
           'ap_mac_list': [self.ap_mac],
           'wlan_name': self.wlan_name,
           'mode': 'local',
           'filter': self.conf['capture_filter'],
           }
        dgf.start_capture_in_aps(self.zdcli, **start_capture_cfg)
        ...
        dgf.stop_capture_in_aps(self.zdcli, **stop_capture_cfg)
