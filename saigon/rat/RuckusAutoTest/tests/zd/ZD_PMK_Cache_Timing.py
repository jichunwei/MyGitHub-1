...
"""
Description: This script is use to test PMK Timing with EAP-WPA2-TKIP or EAP-WPA2-AES
   ...
   2. Test:
       ...
       - Verify the PMK cache for target station should be deleted after 12 hours
   3. Cleanup:
...
class ZD_PMK_Cache_Timing(Test):
    ...
    def _initTestParamesters(self, conf):
        ...
        ssid = 'PMK Cache Timing - %s' % time.strftime("%H%M%S")
    ...
    def _testPMKTiming(self):
        ...
        pmk_count, pmk_info = self._verifyPMKCacheInfo()
        if not pmk_count or self.errmsg:
            return

        # Disconnect target station out of Zone Director
        tconfig.remove_all_wlan_from_station(self.target_station)

        allowed_different_time = time.time() - start_time
        # The PMK should be remove affer 12 hours by default
        start_count_time = time.time()
        passed_time = 0

        while True:
            passed_time = time.time() - start_count_time
            logging.info('Zone Director current time: %s' % self.zd.get_current_time())
            pmk_count, pmk_info = self._verifyPMKCacheInfo()
            if pmk_count == '0' or passed_time > self.default_timer:
                break
        ...
        time.sleep(30)
        logging.info('Zone Director current time: %s' % self.zd.get_current_time())
        pmk_count, pmk_info = self._verifyPMKCacheInfo(False)
        if not self.errmsg:
            msg = 'The PMK for station [%s] is deleted from Zone Director after %d seconds.'
            msg = msg % (self.target_station_wifi_macaddr, passed_time)
            logging.info(msg)
            different_time = self.default_timer - passed_time
            if different_time <= allowed_different_time:
                msg = 'Correct behavior: The different time is allowed [%s <= %s]'
                msg = msg % (different_time, allowed_different_time)
                logging.info(msg)
            else:
                self.errmsg = msg
                msg = 'Incorrect behavior: The different time is out of allowed range [%s > %s]'
                msg = msg % (different_time, allowed_different_time)
                logging.info(msg)
                return
