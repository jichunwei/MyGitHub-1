...
"""
   ...
   Test procedure:
    1. Config:
        -         
    2. Test:
        - Base on the definition eMesh 3 hops topology to configure the L3Switch
...
class CB_ZD_Setup_3Hops_EMesh_Topo(Test):
    ...
    def test(self):
        res, msg = tmethod.emesh.test_forming_emesh_3_hops_network(**self.mconf)
    ...
    def _define_mesh_tree_conf(self, conf):
        mconf = {'testbed': self.testbed,
                 ...
                 'mesh_2_hops_ap': self.testbed.get_ap_mac_addr_by_sym_name(conf['mesh_2_hops_ap']),
                 'emesh_3_hops_aps': [self.testbed.get_ap_mac_addr_by_sym_name(ap) 
                                      for ap in conf['emesh_3_hops_aps']],                
                }
