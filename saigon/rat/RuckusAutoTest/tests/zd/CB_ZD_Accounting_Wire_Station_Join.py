...
class CB_ZD_Accounting_Wire_Station_Join(Test):
    ...
    def _verify_join_leave_trap(self):
        if self.conf['acct_enable']:
            ...
            if len(self._find_account_packets('Value: Interim-Update')) != 0:
                   self.passmsg = "Radius Accounting Server works well"
                   return
               
            if len(self._find_account_packets('Value: Start')) != 0:
                self.passmsg = "ZD sent out trap accounting when client associate to wlan"
                return
            
            if len(self._find_account_packets('Value: Stop')) != 0:
                self.passmsg = "ZD sent out trap accounting when client associate to wlan"
                return
        else:
        ...
        self.errmsg = "ZD not send out trap packet to Accounting Server whenever client join/leave work properly"
