...
class CB_ZD_SNMP_Config_SNMP_Agent_Outside_Boundary(Test):
    '''
    '''
    ...
    def _verify_set_agent_outside_boundary(self):
        ...
        try:
            ...
            key_word = 'v2_ro_user'
            err_msg, pass_msg = self._verify_err_msg(snmp, key_word, invalid_v2_user_name_list, set_sys_snmp_info)
            if err_msg:
            ...
            key_word = 'v2_contact'        
            err_msg, pass_msg = self._verify_err_msg(snmp, key_word, invalid_v2_contact_list, set_sys_snmp_info)
            if err_msg:
                err_dict[key_word] = err_msg
            else:
                pass_dict[key_word] = pass_msg
            
            key_word = 'v2_location'        
            err_msg, pass_msg = self._verify_err_msg(snmp, key_word, invalid_v2_location_list, set_sys_snmp_info)
            if err_msg:
                err_dict[key_word] = err_msg
            else:
                pass_dict[key_word] = pass_msg            
            ...
            key_word = 'v3_ro_user'
            err_msg, pass_msg = self._verify_err_msg(snmp, key_word, invalid_v3_user_name_list, set_sys_snmp_info)
            if err_msg:
                err_dict[key_word] = err_msg
            else:
                pass_dict[key_word] = pass_msg
                
            key_word = 'v3_ro_auth_key'
    ...
    def _verify_err_msg(self, snmp, key_word, invalid_value_list, config_func):
        key_err_mapping = {'v2_ro_user': '(Value does not match DISPLAY-HINT :: {(1..32)})',
                           'v2_rw_user': '(Value does not match DISPLAY-HINT :: {(1..32)})',
                           'v2_contact': '(Value does not match DISPLAY-HINT :: {(1..64)})',
                           'v2_location': '(Value does not match DISPLAY-HINT :: {(1..64)})',
                           'v3_ro_user':  '(Value does not match DISPLAY-HINT :: {(1..31)})',
                           'v3_ro_auth_key': '(Value does not match DISPLAY-HINT :: {(8..32)})',
                           'v3_ro_priv_key': '(Value does not match DISPLAY-HINT :: {(8..32)})',
                           'v3_rw_user': '(Value does not match DISPLAY-HINT :: {(1..31)})',
                           'v3_rw_auth_key': '(Value does not match DISPLAY-HINT :: {(8..32)})',
                           'v3_rw_priv_key': '(Value does not match DISPLAY-HINT :: {(8..32)})',
                           }
        ...
        for invalid_value in invalid_value_list:
            ...
            if actual_err.upper() == expected_err_msg.upper():
