...
class CB_Station_CaptivePortal_Perform_HotspotAuth_With_Invalid_Account(Test):
    ...
    def test(self):
        '''
        '''
        #@author: Jane.Guo @since: 2013-08 add parameters to support start and stop browser
        ...
        if self.errmsg and self.negative_test:
            return self.returnResult("PASS", self.errmsg)
        
        else:
            return self.returnResult("FAIL", self.errmsg)
        
        if self.passmsg and self.negative_test:
    ...
    def _init_test_params(self, conf):
        ...
        self.negative_test = self.conf['negative_test']
    ...
    def _perform_hotspot_auth(self):
        ...
        try:
            ...
            self.passmsg += "Perform Hotspot Auth successfully on station [%s]" % self.sta_tag
