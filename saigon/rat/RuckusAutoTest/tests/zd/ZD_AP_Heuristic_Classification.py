...
'''
Description: Verify the traffic go to the right media queue with heuristic classify option
...
Prerequisite (Assumptions about the state of the testbed/DUT):
   ...
   'len_of_pkts' : lenght of the packets that will be sent
   'pkt_gap' : the packet gap of the traffic
   'num_of_pkts' : Number of packet will be sent out
   'expect_queue' : The approriate media queue with the Huerictis classify on AP
   ...
   Test procedure:
         ...
         2. Test procedure:
            - Send out traffic downlink with lenght and IMG base on the Heuristic classify on AP
...
class ZD_AP_Heuristic_Classification(Test):
    required_components = ['Station', 'RuckusAP', 'ZoneDirector']
    parameter_description = {'target_station': 'IP address of target station',
                           'active_ap': 'mac address (XX:XX:XX:XX:XX:XX) or sympolic name of target ap which client will associate to',
    ...
    def config(self, conf):
        ...
        self.len_of_pkt = conf['len_of_pkt']
        self.pkt_gap = conf['pkt_gap']
        ...
        logging.info('Create WLAN \'%s\' successfully' % self.test_wlan['ssid'])
    ...
    def test(self):
        ...
        logging.info('Verify information of the target station shown on the AP %s' % self.mac_addr)
        start_time = time.time()
        ...
        if not station_list_on_ap:
            return ('FAIL', 'AP %s didn\'t have any info about the stations' % self.mac_addr)

        found = False
        for sta_info in station_list_on_ap:
            if sta_info[0].upper() == sta_wifi_mac_addr.upper():
                if sta_info[1] == 0:
                    return ('FAIL', 'Target station\'s AID status is zero on the AP %s' % self.mac_addr)
        ...
        if not found:
            return ('FAIL', 'Not found station %s on the AP %s' % (sta_wifi_mac_addr, self.mac_addr))
        ...
        traffic_result = self.target_station.send_zing(host = self.host, num_of_pkts = self.num_of_pkts,
                                                      len_of_pkt = self.len_of_pkt, delay = self.pkt_gap,
                                                      sending_time = self.run_time)
        num_of_pkts_go_to_media_queue = self.ap.get_media_queue_stats(self.wlan_name)\
                                      ['%s_%s_%s' % (sta_wifi_mac_addr.lower(), self.expect_queue, self.expect_column)]
        ...
        msg = '%s packets with packet length [%s] and packet gap [%s] are send out'
        msg = msg % (total_pkts_send_out, self.len_of_pkt, self.pkt_gap)
        logging.info(msg)
    ...
    def _cfgActiveAP(self, conf):
        self.ap = tconfig.get_testbed_active_ap(self.testbed, conf['active_ap'])
