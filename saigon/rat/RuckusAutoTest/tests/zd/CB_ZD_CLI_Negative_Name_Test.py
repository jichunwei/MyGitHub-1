...
class CB_ZD_CLI_Negative_Name_Test(Test):
    def config(self, conf):       
        self._initTestParameters(conf)

    def test(self):
        {'all': self._test_negative_for_all,
         'wlan': self._test_negative_name_setting_for_wlan,
         'user': self._test_negative_name_setting_for_user,
         'aaa': self._test_negative_name_setting_for_aaa_server
    ...
    def _test_negative_name_setting_for_wlan(self):
        cmd_block = ""
        invalid_name_list = []
        ...
        self.passmsg = 'All invalid values %s could not set for WLAN name'
    ...
    def _test_negative_name_setting_for_aaa_server(self):
        cmd_block = ""
        invalid_name_list = []
        for invalid_name in invalid_name_list:
            res = zdcli.do_cfg(cmd_block % invalid_name)
            if '' in res['']:
                self.errmsg = '[INCORRECT]Invalid value "" till could set for AAA server name.' % invalid_name
        self.passmsg = 'All invalid values %s could not set for AAA server name'
