...
"""
   Description: The case is for verifying if the hostname of station obtained by ZD is correct.
   @author: Sean Chen
   ...
   Test procedure:
    ...
    2. Test:
        - Verify station hostname information on ZD is same as expect.  
    3. Cleanup:
        - N/A
   
   Result type: PASS/FAIL
   Results: PASS: If station hostname information is same as expect
...
class CB_ZD_Verify_Station_Host_Name_Info(Test):
    ...
    def test(self):
        ...
        if self.conf['expect_get_sta_hn']:
            if self.msg:
                return self.returnResult('FAIL', self.msg)
            else:
                self._update_carrier_bag()
                self.passmsg = 'Verify station hostname information [%s] on ZD successfully' % self.station_info_on_zd
            return self.returnResult('PASS', self.passmsg)
        else:
            if not self.msg:
                errmsg = 'ZD can display station hostname information even if Fingerprinting off'
    ...
    def _verify_station_os_type_info(self):
        logging.info("Verify target station hostname information shown on the Zone Director")
        exp_sta_hostname = self._get_sta_hostname()
        self.msg, self.station_info_on_zd, self.current_client_info = tmethod.verify_station_hostname_info(
                                                                      self.zd,
        ...
        logging.info('Target station hostname information on ZD is %s' % (self.station_info_on_zd))

    def _get_sta_hostname(self):
        ...
        return sta_platform_info[2]

    def _update_carrier_bag(self):
        self.carrierbag['station_os_hostname_on_zd'] = self.station_info_on_zd
