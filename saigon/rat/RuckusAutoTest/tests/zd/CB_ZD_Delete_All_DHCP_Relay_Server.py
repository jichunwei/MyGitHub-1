...
class CB_ZD_Delete_All_DHCP_Relay_Server(Test):
    ...
    def _delete_all_dhcp_relay_server(self):
        try:
            lib.zd.dhcpr.delete_all_dhcp_relay_svr(self.zd)
            self.passmsg = 'Delete all DHCP relay servers successfully'
        except Exception, e:
            self.errmsg = e.message
    
    def _verify_creating_dhcp_relay_server(self):
        logging.info('Verify to delete all DHCP relay servers')
        self._delete_all_dhcp_relay_server()
