"""
Description: This script is support to associate remote station to specify wlan
Author: Jason Lin
...
class CB_ZD_Associate_Station_1(Test):
    ...
    def test(self):
        ...
        if not self.passmsg:
            self.passmsg = 'Associate station [%s] successfully' % self.conf['sta_tag']
        ...
        if not self.errmsg:
            if self.is_negative:
                ...
                if self.conf['failed_event'] == 'max_client_exceed':
                    ...
                    self.errmsg, client_info_on_zd = tmethod.verify_zd_client_status(self.zd, self.refused_user_mac,
                                                                                    exp_client_info, check_status_timeout=30)
                    if self.errmsg:
                        self.errmsg = ''
    ...
    def _veirfy_assoc_failed_event_max_users_excceed(self):
        ...
        #@author: lipingping;@bug: ZF-8507;@since: 2014.6.13    
        if not self.errmsg:
            self.errmsg = ""
