...
class CB_Server_Del_STA_MAC_As_Radius_User(Test):
    ...
    def _del_user(self):
        ...
        if not user_name:
            self.errmsg = 'Cannot delete user, because no user can be parsed'
            return self.errmsg
        else:
            ...
            if not user_info in result:
                logging.info('user:%s,password:%s does not exist, do nothing'%(user_name,password))
                self.passmsg = 'The user %s does not exist in radius server.'%(user_name)
                return
            ...
            logging.info('result is:%s'%result)
            if not user_info in result:
                logging.info('user:%s,password:%s is deleted'%(user_name,password))
            else: 
            ...
            self.passmsg = "The specified radius user has been deleted."
            return
