...
class CB_ZD_CLI_Configure_Precedence_Policy(Test):
    ...
    def _verify_configurating_precedence_policy(self):
        logging.info('Verify to configure precedence policy')
        errmsg = ''
        passmsg = ''
        if not self.conf['prece_policy_list']:
            raise Exception('There is not any precedence policy to configure. Please check!')
        for pol in self.conf['prece_policy_list']:
            res, msg = lib.zdcli.dvccfg.configure_precedence_policy(self.zdcli, pol)
            logging.debug(msg)
            if res:
                passmsg += 'Passed to configure precedence policy %s. ' % pol['name']
            else:
                errmsg += 'Failed to configure precedence policy %s. ' % pol['name']
