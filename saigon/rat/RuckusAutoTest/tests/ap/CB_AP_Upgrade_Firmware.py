...
from RuckusAutoTest.components import create_ap_by_model
from RuckusAutoTest.components.lib.ap import upgrade_fw as up


class CB_AP_Upgrade_Firmware(Test):
    ...
    def _upgrade_firmware(self):
        logging.info('Starting upgrading AP/CPE firmware')
        ...
        try:
            ...
            if test_type.lower() == 'gui':
                ap_webui = create_ap_by_model(model, ap_ip_addr)
                if not ap_webui:
                    raise Exception("Can't create AP CLI for %s, %s" % (model,ap_ip_addr))
            ...
            if current_fw_version == fw_cfg['fw_version']:
                ...
                if upgrade_cfg['proto'].lower() in ['tftp','ftp','http']:
                    #For tftp, ftp, http, need to create contro file.
    ...
    def _upgrade_fw(self, ap_cli, ap_webui, upgrade_cfg, fw_cfg, timeout, is_first_check = False):
        ...
        if test_type.lower() == 'cli':
            res = up.upgrade_fw_via_cli(ap_cli, upgrade_cfg, timeout)
        else:
            #Change firstcheck via cli.
            if upgrade_cfg['auto']:
                ap_cli.change_fw_setting({'firstcheck': upgrade_cfg['firstcheck']})
            
            res = up.upgrade_fw_via_gui(ap_webui, upgrade_cfg)
    ...
    def _get_image_file(self, image_file_path):
        ...
        '''
        if '.bl7' in image_file_path.lower():
            fw_img_full_path = image_file_path
        else:
            img_ptn = '[0-9]+.*[_uImage,\.Bl7,\.bl7]'
