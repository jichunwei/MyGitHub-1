"""
Helper functions deals with ZoneDirector in general.
Including special commands to deal with login ZD using SSH.

You can include me directly with:

    from RuckusAutoTest.components.lib import node_zd as zdNode
...
def ssh_login_zd(hostname, username = 'admin', password = 'admin', timeout = 60):
    ...
    while time.time() - cut_time < timeout:
        ...
        if mid < 0:
            ...
            continue
        if mid == 0:
            print "[ssh_login_zd] username: %s" % username
            myzd.write(username + '\n')
            login_cnt += 1
            if login_cnt > 3:
                myzd.close()
            ...
            print "[ssh_login_zd] password: %s" % password
            myzd.write(password + '\n')
        elif mid > 1:
            print "[ssh_login_zd] login banner: %s" % resp
            return myzd 
        myzd.buffer = ''
        time.sleep(3)
    return False
...
def set_auth_info_local(zd, username = 'admin', password = 'admin', auth_serv = '', fallback_local = True):
    logging.info('[AuthInfoLocal] set ZoneDirector adminUser[%s %s] using local auth.' % (username, password))
    # zd.do_login()
...
def set_auth_info_external(zd, auth_server, username = 'admin', password = 'ruckus'):
    logging.info('[AuthInfoExternal] set ZoneDirector adminUser[%s %s] using External AuthServ[%s].' % (username, password, auth_server))
    # zd.do_login()
