...
"""
The SpectralinkGW class provides functions to configure Spectralink Gateway.
"""
...
class SpectralinkGW:
    ...
    def verify_component(self):
        logging.info("Sanity check: Verify Test engine can ping Spectralink Gateway[%s]" % (self.conf['ip_addr']))
        rmsg = RU.ping_win32( self.conf['ip_addr'], timeout_ms=self.conf['ping_timeout']*1000, echo_timeout=1000)
        if rmsg.find('Timeout') != -1:
            raise Exception("Spectralink Gateway is no response")
