...
Description: 
     This script is verify CPE manual upgrade via GUI.
"""
...
def _define_test_cfg_sub(tcfg, up_mtd, up_cfg, model, mf_ip_addr, up_flag):
    ...
    testcase_name = '[%s Manual %s %s]IP=%s-' % (up_mtd, action, model, mf_ip_addr)    
...
def define_test_parameters(tbcfg):
    ...
    manual_local_cfg = {}
    manual_local_cfg.update(default_upgrade_cfg)
    manual_local_cfg['proto'] = 'local'
    
    cfg = {'manual_tftp_cfg': manual_tftp_cfg,
           'manual_ftp_cfg': manual_ftp_cfg,
           'manual_http_cfg': manual_http_cfg,
           'manual_local_cfg': manual_local_cfg,
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'CPE GUI - Manual upgrade firmware %s to %s' % (baseline, target)
    ts = testsuite.get_testsuite(ts_name, 'Verify CPE manual upgrade via GUI', combotest=True)
