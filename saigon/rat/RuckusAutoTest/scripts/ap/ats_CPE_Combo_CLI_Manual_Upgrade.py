...
def _define_test_cfg_sub(tcfg, up_mtd, up_cfg, model, mf_ip_addr, up_flag):
    ...
    testcase_name = '[%s Manual %s %s]%s-' % (up_mtd, action, model, mf_ip_addr)
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'CPE CLI - Manual upgrade firmware %s to %s' % (baseline, target)
    ts = testsuite.get_testsuite(ts_name, 'Verify CPE manual upgrade via CLI', combotest=True)
