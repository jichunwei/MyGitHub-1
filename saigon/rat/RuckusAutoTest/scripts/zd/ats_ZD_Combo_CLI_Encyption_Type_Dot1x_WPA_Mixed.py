...
def define_wlan_cfg(tcfg):
    ...
    #_wlan_cfgs['dot1x-wpa-mixed-tkip'] = (dict(name = 'dot1x-wpa-mixed-tkip',
    #                       auth = "dot1x-eap", encryption = "wpa-mixed", algorithm = 'TKIP',
    #                       auth_server = auth_server))  
    
    _wlan_cfgs['dot1x-wpa-mixed-aes'] = (dict(name = 'dot1x-wpa-mixed-aes',
                           auth = "dot1x-eap", encryption = "wpa-mixed", algorithm = 'AES',
                           auth_server = auth_server))
    
    _wlan_cfgs['dot1x-wpa-mixed-auto'] = (dict(name = 'dot1x-wpa-mixed-auto',
...
def createTestSuite(**kwargs):
    ...
    if active_ap :
        ...
        if attrs["testsuite_name"]:
            ...
            #ts_name = "ZDCLI- Encryption Types-802.11x-WPA-Mixed - 11%s %s" % (target_sta_radio, ap_model)
            ts_name = "ZDCLI- Encryption Types-802.11x-WPA-Mixed - 11%s" % target_sta_radio
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(802.11x-WPA-Mixed) properly - 11%s radio" % target_sta_radio,
