'''
Set SNMP Trap settings via ZD CLI
    ...
    How to:
        1) Set snmp v3 trap setting with mid range values via CLI.
        2) Get snmp v3 trap setting via CLI.         
        4) Verify the values between CLI get and set.
        5) Get snmp v3 trap setting via GUI.
...
def define_test_cfg(tcfg):
    ...
    common_name = 'Enable SNMP Trap V3 with Default Config from CLI'
    ...
    test_name = 'CB_ZD_CLI_Config_SNMP_Trap_Outside_Boundary'
    common_name = '%sVerify Outside Boundary Values for SNMP Trap Settings' % (test_case_name,)
    param_cfg = {}
    test_cfgs.append((param_cfg, test_name, common_name, 1, False))
...
def create_test_suite(**kwargs):    
    ...
    if str(tb.tbtype) == "ZD_Stations_IPV6":
        ...
        ts_name = 'ZD CLI ZD %s AP %s - SNMP Trap Configuration - Length Checking' % (zd_ip_version, ap_ip_version)
    else:
        trap_server_ip = '192.168.0.10'
        ts_name = 'ZD CLI - SNMP Trap Configuration - Length Checking'
