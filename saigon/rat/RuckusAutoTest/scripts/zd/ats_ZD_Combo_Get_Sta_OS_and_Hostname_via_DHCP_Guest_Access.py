'''
Title: Get_Sta_OS_and_Hostname_via_DHCP_Guest_Access

Test purpose: Verify if ZD can get station OS type and hostname information correctly with guest-access wlan.
...
7) Create a wlan with guest access auth on ZD
...
14) Station perform guest access auth
...
#---TestCase-----------------------------------------------------------------------------------------
    ...
    ### new added tests script.
        ...
        common_name = '%sGenerate a guest pass for guest access auth' % (test_combo_case_name,)
        test_params = {'username': username, 
                       ...
                       'key': '', 
                       'duration': '300', 
                       'guest_fullname': 'guest-access', 
                       'type': 'single', 
                       'auth_ser': 'Local Database'}
        test_cfgs.append((test_params, test_name, common_name, 2, False))
    
        test_name = 'CB_Station_CaptivePortal_Perform_GuestAuth'
        common_name = '%sStation perform guest access auth' % (test_combo_case_name,)
        test_params = {'sta_tag': sta_tag,
    ...
    ### new added tests script.
        ...
        common_name = '%sRemove generated guest passes on ZD after case' % (test_combo_case_name,)
...
def create_test_suite(**kwargs):
    ...
    ssid = 'guest_access_wlan_%s' % (time.strftime("%H%M%S"))
    wlan_cfg = {'ssid': ssid, 
                'type': 'guest',
                'auth': 'open', 
                'encryption': 'none', 
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = 'Guest Access - Get Sta OS and Hostname via DHCP - Basic'

    ts = testsuite.get_testsuite(
        ts_name, 'Verify ZD obtaining station OS type and hostname via DHCP with Guest Access Auth WLAN.',
