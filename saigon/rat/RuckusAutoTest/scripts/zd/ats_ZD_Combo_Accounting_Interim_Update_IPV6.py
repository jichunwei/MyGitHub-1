"""
Verify interim-update package should not be sent out when wlan accounting sever is disabled. 

    Verify interim update packet is sent out as expected duration when ZD is dual stack and ipv6 only.
    ...
    How to:
        ...
        11) Verify interim update packets between zd and ipv6 server.                
...
def define_test_cfg(cfg):
    ...
    for wlan_cfg in wlans_cfg_list:
        test_cfgs.extend(_define_station_interim_update(cfg, wlan_cfg, sta_tag, browser_tag, ap_tag, server_tag, ip_version))
...
def _define_station_interim_update(cfg, wlan_cfg, sta_tag, browser_tag, ap_tag, server_tag, ip_version):
    test_cfgs = []
    
    acct_server_cfg = cfg['acct_server_cfg']
    acct_svr_port = acct_server_cfg['server_port']
    acct_svr_addr = acct_server_cfg['server_addr']
        
    test_case_name = '[%s-%s]' % ('Interim Update', _get_wlan_encrypt_type(wlan_cfg))
    ...
    test_cfgs.append(({'interim_update': wlan_cfg['interim_update'],
                       'acct_svr_addr': acct_svr_addr,
                       'acct_svr_port': acct_svr_port}, test_name, common_name, 2, False))
...
def _get_wlan_cfg(wlan_params):
    ...
    wlanCfg = dict(ssid=ssid, name="", auth="open", wpa_ver="", encryption="none", key_index="", key_string="",
                   username="", password="", inactivity_timeout='10')
...
def _define_wlan_cfg_list(auth_server_name, acct_server_name):
    ...
    #wlan_cfgs.append(_get_wlan_cfg(dict(auth="shared", encryption="WEP-64",
    ...
    for wlan_cfg in wlan_cfgs:
        if acct_server_name:
            ...
            if not acct_server_name.lower() == 'disabled':
                wlan_cfg['interim_update'] = interim_update
...
def create_test_suite(**kwargs):
    ...
    wlan_cfg_list = _define_wlan_cfg_list(auth_server_cfg['server_name'], acct_server_cfg['server_name'])
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "WLAN Accounting Interim Update - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify WLAN with acounting server interim update - 11%s" % (target_sta_radio), combotest = True)
