...
Verify Ip address column in packet capture onfiguration.
    ...
    How to:
        ...
        2)[system IPv6]set system IP mode is IPv6 only 
        3)[system IPv6]set all AP mode to ipv6/dualstack via randomly selected
        4)[system IPv6]check IP column title
        5)[system IPv6]check IP column value
...
def define_test_cfg(cfg):
    ...
    zd_ipv6_cfg = cfg['zd_ipv6_cfg']
    zd_restore_ip_cfg = cfg['restore_zd_ip_cfg']
    
    ap_restore_ip_cfg = cfg['restore_ap_ip_cfg']
    ap_ipv6_and_dualstack_cfg_list = cfg['ap_ipv6_and_dualstack_cfg_list']
    ...
    tc_common_name = tc_name_sub + const.IPV6
    test_name = 'CB_ZD_CLI_Set_Device_IP_Settings'
    common_name = '[%s]%s.%s set system IP mode is %s ' % (tc_common_name,round, step, const.IPV6)
    test_cfgs.append(({'ip_cfg': zd_ipv6_cfg}, test_name, common_name, 1, False))
    
    for mac_key,tag_value in ap_mac_tag_dict.items(): 
        #@attention: get random and not repeat ap_ip_mode_cfg   
        ap_list_len = len(ap_ipv6_and_dualstack_cfg_list)
        if ap_list_len > 0: 
            index = random.randint(0,len(ap_ipv6_and_dualstack_cfg_list)-1)
            if ap_ipv6_and_dualstack_cfg_list[index] in list_tmp and ap_list_len > 1:
                    del ap_ipv6_and_dualstack_cfg_list[index]
                    index = random.randint(0,len(ap_ipv6_and_dualstack_cfg_list)-1)
            list_tmp.append(ap_ipv6_and_dualstack_cfg_list[index])
            ap_ip_mode_cfg = ap_ipv6_and_dualstack_cfg_list[index]
        else:
    ...
    test_name = 'CB_ZD_CLI_Set_Device_IP_Settings'
    common_name = 'set system IP mode is dualstack'
    test_cfgs.append(({'ip_cfg': zd_restore_ip_cfg}, test_name, common_name, 0, True))
...
def define_test_parameters(tbcfg,ap_mac_addr):
    ...
    zd_ipv6_cfg = _define_zd_ip_cfg(const.IPV6,zd_ipv4_addr, netmask,ipv4_gateway,ipv4_pri_dns,zd_ipv6_addr, ipv6_gateway, ipv6_pri_dns,prefix_len)
    ...
    tcfg = {'restore_zd_ip_cfg': zd_dual_stack_cfg,
            'zd_ipv6_cfg': zd_ipv6_cfg,
            'restore_ap_ip_cfg': ap_dual_stack_dhcp_cfg,
            'ap_mac_addr': ap_mac_addr,
            'ap_ipv6_and_dualstack_cfg_list':[ap_dual_stack_dhcp_cfg,ap_ipv6_auto_cfg,ap_dual_stack_static_cfg],
            }
...
def create_test_suite(**kwargs):    
    ...
    ts_name = 'ZD-Verify IPv6 address column in packet capture configuration'
    ts = testsuite.get_testsuite(ts_name, 'Verify display ipv6 column in packet capture', combotest=True)
