...
#    walled_garden_entries = ["www.example.net", "172.21.0.252", "172.22.0.0/16", "172.23.0.252:8888", "172.23.0.252"]
    ...
    h_cfg = deepcopy(def_test_params['hotspot_cfg'])
    h_cfg.update({'logout_page': 'https://192.168.0.250/slogout.html'})
    test_cfgs.append((dict(hotspot_profiles_list = [h_cfg]),
                      'CB_ZD_Edit_Hotspot_Profiles',
                      'Update hotspot profile',
                      1,
    ...
    tcid = "[Wrong authentication using invalid username - local database]"  
    h_cfg = deepcopy(def_test_params['hotspot_cfg'])    
    test_cfgs.append((dict(hotspot_profiles_list = [h_cfg]),
                      'CB_ZD_Edit_Hotspot_Profiles',
                      'Update hotspot profile',
    ...
    test_cfgs.extend(station_a_steps(active_ap, 
                                     target_station, 
                                     target_station_radio, 
                                     open_wlan, 
                                     'wg_1'))            
    ...
    tcid = "[Wrong authentication using invalid password - local database]" 
    test_cfgs.append(({'sta_tag':'sta_1',
...
def create_test_suite(**kwargs):
    ...
    attrs = dict(interactive_mode = True,
                 station = (0,"g"),
                 targetap = False,
                 testsuite_name = "Reading WISPr Basic function - 11g",
                 )
    attrs.update(kwargs)
    ...
    if attrs["interactive_mode"]:
        ts_name = "Reading WISPr Basic function - 11%s" % (target_sta_radio)        
    else:
        ts_name = attrs["testsuite_name"]
    
    ts_name_list = [('Reading WISPr Basic', build_basic_ts),
                    ('Reading WISPr Session Timeout', build_session_timeout_ts),
                    ('Reading WISPr Idle Timeout', build_idle_timeout_ts),
                    ('Reading WISPr Attribute', build_adv_ts),
                    ]
