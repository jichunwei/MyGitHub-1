"""
Verify per client rate limit configuration integrated with WISPr wlan rate limiting option on root and mesh AP
    ...
    - Rate limit range is from 0.10mbps to 20.00mbps step is 0.25mbps except value 0.10mbps-0.25mpbs.
...
def _define_wlan_cfg(uplink_rate_limit, downlink_rate_limit, hs_name, do_tunnel=False, vlan_id=''):
...
def define_test_cfg(cfg):
    ...
    common_name = 'Create active AP 1'
    test_cfgs.append(({'active_ap':cfg['active_ap_list'][0],
                       'ap_tag': ap_tag1}, test_name, common_name, 0, False))
    
    test_name = 'CB_ZD_Config_AP_Radio'
    common_name = 'Config active AP 1 Radio %s - Enable WLAN Service' % (radio_mode)
    ...
    for rate_cfg in expected_list:
        ...
        if j==4:
            ...
            test_cfgs.append(({'active_ap':cfg['active_ap_list'][1],
                               'ap_tag': ap_tag2}, test_name, common_name, 1, False))
        ...
        # WLAN configuration
        wlan_cfg = _define_wlan_cfg(wlan_rate_option_up, wlan_rate_option_down, hs_cfg['name'], do_tunnel, vlan_id)
        ...
        edit_wlan_cfg = _define_wlan_cfg(wlan_cfg_up_edit, wlan_cfg_down_edit, hs_cfg['name'])
        ...
        if do_grace_period:
            ...
            new_hs_cfg['idle_timeout'] = grace_period

            test_name = 'CB_ZD_Edit_Hotspot_Profiles'
            common_name = '%sEdit hotspot profile for grace period' % (test_case_name)
            test_cfgs.append(({'hotspot_profiles_list':[new_hs_cfg]}, test_name, common_name, 2, False))

            test_name = 'CB_Station_CaptivePortal_Perform_HotspotAuth'
            common_name = '%sPerform Hotspot Auth after editing hs profile for client' % (test_case_name)
            test_cfgs.append(({'sta_tag':sta_tag, 
                               'browser_tag': browser_tag,
                               'username': radius_user_cfg[j-1], 
                               'password': radius_user_cfg[j-1]},  
            ...
            test_name = 'CB_ZD_Edit_Hotspot_Profiles'
            common_name = '%sEdit hotspot profile to orginal value' % (test_case_name)
...
def createTestSuite(**kwargs):
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = "PerClient Rate Limiting%s - WISPr%s" % (tmp_radius_auth_method, tmp_enable_tunnel)
