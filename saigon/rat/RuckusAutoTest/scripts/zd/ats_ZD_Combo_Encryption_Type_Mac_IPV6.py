...
def _define_expect_wlan_info_in_ap(tcfg, wlan_cfg):
    ...
    for radio in radio_mode_list:
        ...
        if radio in ['bg', 'ng']:
            ...
            if wlan_cfg.has_key('name') and wlan_cfg['name']:
                expect_wlan_info[wlan_name]['encryption_cfg'] = dict(ssid = wlan_cfg['ssid'], name = wlan_cfg['name'])
            else:
...
def _define_wlan_cfg_list(ras_cfg):
    ...
    wlan_cfgs.append(_get_wlan_cfg(dict(name = 'mac-none-%03d' % (random.randrange(1, 999)),
                                        auth = 'mac', encryption ='none', auth_svr = auth_server,
                                        username = username, password = password)))    
    wlan_cfgs.append(_get_wlan_cfg(dict(name = 'mac-wep-128-%03d' %  (random.randrange(1, 999)),
                                        auth = "mac", encryption = "WEP-128",
                                        key_index = random.randint(1,4) , key_string = utils.make_random_string(26, "hex"),
                                        auth_svr = auth_server, username = username, password = password)))    
    #wlan_cfgs.append(_get_wlan_cfg(dict(name = 'mac-wpa-aes-%03d' % (random.randrange(1, 999)),
    #                                    auth = "mac", wpa_ver = "WPA", encryption = 'AES',
    #                                    key_string = utils.make_random_string(random.randint(8, 63), "hex"),
    #                                    auth_svr = auth_server, username = username, password = password)))    
    #wlan_cfgs.append(_get_wlan_cfg(dict(name = 'mac-wpa2-tkip-%03d' % (random.randrange(1, 999)), 
    #                                    auth = "mac", wpa_ver = "WPA2", encryption = 'TKIP',
    #                                    key_string = utils.make_random_string(random.randint(8, 63), "hex"),
    #                                    auth_svr = auth_server, username = username, password = password)))    
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "ZD GUI - Encryption Types Mac - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)
