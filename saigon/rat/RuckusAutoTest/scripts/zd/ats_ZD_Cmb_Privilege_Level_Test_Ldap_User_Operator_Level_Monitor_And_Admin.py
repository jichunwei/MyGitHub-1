...
def define_test_configuration(tcfg):
    ...
    common_name = 'login zd web by LDAP operator user'
    ...
    common_name = '%soperator user ping ap should successfully'%test_case_name
    ...
    common_name = '%soperator user can not remove dpsk'%test_case_name
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Operator level LDAP User privilege level test-Monitor and Admin"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="Operator level LDAP User privilege level test-Monitor and Admin"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Operator level LDAP User privilege level test-Monitor and Admin", interactive_mode = attrs["interactive_mode"], combotest=True)
