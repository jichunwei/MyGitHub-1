...
ap_station_portbase_setting_map=[('%s',                               
                                            ...
                                            '[Port-based authenticator disable mac bypass, os_type=%s]'#tcid, ap model, os type
                                        #'[Port-based authenticator disable mac bypass on ap=%s, os_type=%s]'#tcid, ap model, os type
                                        ),
                                           
                                   #('24:c9:a1:13:47:d0', 'zf7363', 'sta_1', 'win7',
                                   # {'override_parent': True,
                                   #  'lan2': {
                                   #          'enabled': True,
                                   #          'type': 'trunk',              #[trunk, access, general]
                                   ...
                                   #          'dot1x_mac_bypass_enabled': True, #optional param.
                                   #       }},
                                   # '[Port-based authenticator enable mac bypass on ap=%s, os_type=%s]'#tcid, ap model, os type
                                   # )
                                   ] 
...
ap_station_macbase_setting_map=[('%s',
                                       ...
                                       '[Mac-based authenticator enable mac bypass, os_type=%s]'#tcid, ap model, os type  
                                       #'[Mac-based authenticator enable mac bypass on ap=%s, os_type=%s]'#tcid, ap model, os type                             
                                       ),
                                    ...
                                    #         'type': 'access',              #[trunk, access, general]
                                    #         'untagged_vlan': '1',         #[1-4094, none] (expected String type)
                                    #         'vlan_members': None,   #[1-4094] (expected String type)
                                    #         'dot1x': 'auth-mac', #[disabled, supp, auth-port, auth-mac]
                                    #         'dot1x_auth_svr': 'radius-svr', #Radius Server Name "radius-svr"
                                    #         'dot1x_acct_svr': 'radius-acct-svr', #Radius Accounting Server Name "radius-acct-svr"
                                    #         'dot1x_mac_bypass_enabled': False, #optional param.
                                    #      }},
                                    #'[Mac-based authenticator disable mac bypass on ap=%s, os_type=%s]'#tcid, ap model, os type
                                    #),                               
                               ] 
        ...
        for ap_mac_addr, port_setting, tcid in ap_station_portbase_setting_map: 
            ...
            for target_station, sta_tag , os_type in stas_list: 
                ...
                tcs.append(({'sta_tag':sta_tag,
                            ...
                            'lan_port': 'LAN2',
                            'status':'Authorized',
                            #Modified by Liang Aihua on 2014-9-5 to remove user
                            #'user':'ras.local.user'            
                            },
                           'CB_ZD_Verify_Wired_Client_Info',
                           '%sMonitor Page display correctly after auth' % test_case_name,
...
def create_test_suite(**kwargs):    
    ...
    ts_name_list = [('Udaipur 802.1x Ethernet Mac Based', build_mac_based_tcs),
                    ('Udaipur 802.1x Ethernet Port Based', build_port_based_tcs),                    
                    ]
