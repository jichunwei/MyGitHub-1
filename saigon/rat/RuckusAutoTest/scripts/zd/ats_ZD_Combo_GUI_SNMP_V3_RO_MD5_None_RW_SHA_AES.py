'''
Configure SNMP V3 agent via ZD GUI [RO Auth=MD5 Priv=None RW Auth=SHA Priv=AES]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=MD5 Priv=None RW Auth=SHA Priv=AES]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD GUI - SNMP V3 Validation - RO (MD5,None) RW (SHA,AES)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=MD5,priv=None) RW (auth=SHA,priv=AES)', combotest=True)
