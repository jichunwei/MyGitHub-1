'''
Configure SNMP V3 agent via ZD GUI [RO Auth=SHA Priv=AES RW Auth=MD5 Priv=None]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=SHA Priv=AES RW Auth=MD5 Priv=None]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD GUI - SNMP V3 Validation - RO (SHA,AES) RW (MD5,None)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=SHA,priv=AES) RW (auth=MD5,priv=None)', combotest=True)
