"""
Verify AP fail over to another ZD is active ZD is unreachable; stick to ZD if ZD is back.
    ...
    Test Data:
        Pri ZD1 IPV4 FQDN Sec ZD2 IPV6 FQDN
        Pri ZD1 IPV6 FQDN Sec ZD2 IPV4 FQDN
    ...
    How to:
        ...
        8) Prepare for test:
              ...
              Verify AP joins ZD1.
        9) Configure FQDN in ZD CLI, one of is zd1, another is zd2.
        10) Verify FQDN settings in AP CLI.
        11) Disable ZD1 switch port, zd1 is unreachable.
        12) Reboot AP.
        13) Verify AP join ZD2.
        14) Verify FQDN settings after AP join ZD2.
        15) Associate the station to wlan and verify ping traffic to server.
        16) Configure FQDN setting in ZD2.
        17) Verify FQDN setting in AP CLI.
        18) Verify AP joins ZD2.
        19) Associate the station to wlan and verify ping traffic to server.
        20) Clean up the environment:
              Enable ZD1 switch port.         
...
def define_test_cfg(cfg):
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        #Fail over to ZD2.
        test_case_name = 'Fail Over to ZD2 - %s' % zd_discovery_test_cfg['case_name']
        ...
        test_name = 'CB_ZD_CLI_Set_Primary_Secondary_ZD'
        common_name = '%s-Configure FQDN as Keep AP setting for ZD2' % test_case_name
        ...
        #Shutdown port on ZD1.
        test_name = 'CB_ZD_Disable_Enable_ZD_Switch_Port'
        common_name = '[%s]Disable switch port on ZD1' % test_case_name
        test_params = {'enable':False, 'zd_tag': zd1_gui_tag}
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        
        #Reboot AP.
        test_name = 'CB_ZD_Reboot_AP'
        common_name = '[%s]Reboot AP after disable switch port' % test_case_name
        test_cfgs.append(( {'ap_tag': ap_tag}, test_name, common_name, 2, False))
        
        test_name = 'CB_ZD_Verify_AP_Join'
        common_name = '[%s]Verify AP Join ZD2' % test_case_name
        ...
        test_name = 'CB_AP_CLI_Get_Verify_Primary_Secondary_ZD'
        common_name = '[%s]Verify FQDN in AP CLI after AP join ZD2' % test_case_name
        test_params = {'ap_tag': ap_tag,
                       'is_verify': True,
                       'primary_zd_ip': primary_zd_ip,
                       'secondary_zd_ip': secondary_zd_ip
        ...
        common_name = "[%s]Assign wlan group to radio '%s' of AP %s on ZD2" % (test_case_name, radio_mode, active_ap)
        test_params = {'active_ap': active_ap,
                       'wlan_group_name': zd2_wg_name, 
        ...
        #Stick to ZD2 when ZD1 is come back.
        test_case_name = 'Stick to ZD2 - %s' % zd_discovery_test_cfg['case_name']
        
        test_name = 'CB_ZD_CLI_Set_Primary_Secondary_ZD'
        common_name = '[%s]Configure FQDN in ZD2 via CLI' % test_case_name
        test_params = {'zd_tag': zd2_cli_tag}
        test_params.update(zd_discovery_cfg)
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        ...
        test_name = 'CB_ZD_Disable_Enable_ZD_Switch_Port'
        common_name = '[%s]Enable switch port on ZD1' % test_case_name
        test_cfgs.append(({'enable': True, 'zd_tag': zd1_gui_tag}, test_name, common_name, 2, False))
        
        #Reboot AP.
        test_name = 'CB_ZD_Reboot_AP'
        common_name = '[%s]Reboot AP after enable switch port' % test_case_name
        test_cfgs.append(( {'ap_tag': ap_tag}, test_name, common_name, 2, False))
        
        test_name = 'CB_ZD_Verify_AP_Join'
        common_name = '[%s]Verify AP Joins ZD2' % test_case_name
        test_cfgs.append(( {'is_need_approval': True,
                            'auto_approval': False,
                            'zd_tag': zd2_gui_tag,
                            'mac_addr_list': [active_ap_mac]}, test_name, common_name, 2, False))
    ...
    test_name = 'CB_ZD_Disable_Enable_ZD_Switch_Port'
    common_name = 'Enable switch port on ZD1 after test'
    test_cfgs.append(({'enable': True, 'zd_tag': zd1_gui_tag}, test_name, common_name, 0, False))
    
    test_name = 'CB_ZD_Reboot_AP'
    common_name = 'Reboot AP after enable switch port'
    test_cfgs.append(( {'ap_tag': ap_tag}, test_name, common_name, 0, False))
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Pri ZD1 IPV4 FQDN Sec ZD2 IPV6 FQDN',
                          'primary_zd_ip': zd1_ipv4_dn,
                          'secondary_zd_ip': zd2_ipv6_dn,})
    
    test_cfg_list.append({'case_name': 'Pri ZD1 IPV6 FQDN Sec ZD2 IPV4 FQDN',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - FQDN Failover and Stickiness with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - FQDN Failover and Stickiness 11%s" % (target_sta_radio)
    
        ts = testsuite.get_testsuite(ts_name, "Verify AP failover and stick to ZD", combotest = True)
