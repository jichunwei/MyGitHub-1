'''
Configure SNMP V3 agent via ZD GUI [RO Auth=MD5 Priv=DES RW Auth=SHA Priv=None]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=MD5 Priv=DES RW Auth=SHA Priv=None]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD GUI - SNMP V3 Validation - RO (MD5,DES) RW (SHA,None)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=MD5,priv=DES) RW (auth=SHA,priv=None)', combotest=True)
