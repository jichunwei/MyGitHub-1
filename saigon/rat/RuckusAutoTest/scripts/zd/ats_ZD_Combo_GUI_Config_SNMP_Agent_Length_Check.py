'''
Set SNMP Agent settings via GUI.
    ...
    How to:
        1) Set snmp v2 agent setting with mid range values via GUI.
        2) Get snmp v2 agent setting via GUI.         
        4) Verify the values between GUI get and set.
        5) Get snmp v2 agent setting via CLI.
        6) Verify the value between GUI get and CLI get.        
...
def define_test_cfg(tcfg):
    ...
    common_name = 'Enable SNMP Agent V3 via GUI'
    ...
    test_name = 'CB_ZD_Verify_SNMPV2_Info_Get_Set'
    common_name = '%sVerify SNMP Agent V2 Info between GUI Get and GUI Set' % (test_case_name,)
    param_cfg = {'snmp_agent_cfg':tcfg['snmp_agent_v2_cfg_mid_range']}
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    ...
    test_name = 'CB_ZD_SNMP_Get_Sys_SNMP_Info'
    common_name = '%sGet SNMP Agent V3 Info from SNMP' % (test_case_name,)
    param_cfg = {'snmp_cfg': tcfg['snmp_cfg'],
    ...
    test_name = 'CB_ZD_Config_SNMP_Agent_Outside_Boundary'
    common_name = '%sVerify outside boundary values for snmp agent and trap' % (test_case_name,)
    param_cfg = {}
    test_cfgs.append((param_cfg, test_name, common_name, 1, False))
...
def create_test_suite(**kwargs):    
    ...
    ts_name = 'ZD GUI - SNMP Agent Configuration - Length Checking'
