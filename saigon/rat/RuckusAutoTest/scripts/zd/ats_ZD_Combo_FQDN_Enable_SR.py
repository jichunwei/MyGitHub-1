...
Verify AP joins expected ZD when enable FQDN in AP CLI, enable mgmt vlan in ZD.
    ...
    Test Data:
        ...
        Primary Invalid Secondary ZD2 IPV6 FQDN
    ...
    How to:
        ...
        10) Set ZD1 as active ZD. 
        11) Configure FQDN in ZD, one is ZD2, another is invalid.
        12) Verify FQDN settings in AP CLI between set and get.
        13) Verify AP didn't join ZD2.
        14) Set ZD2 as active ZD.
        15) Verify AP join ZD2 successfully.
        16) Verify FQDN settings after AP join ZD2.
...
def define_test_cfg(cfg):
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        test_case_prefix = "AP only Join active ZD"
        ...
        common_name = "%s-Set ZD1 as active ZD" % test_case_name
        test_cfgs.append(({'zd': zd1_gui_tag},test_name,common_name,0,False))
        ...
        test_name = 'CB_ZD_Verify_AP_Join'
        common_name = "[%s]Verify AP don't Join Standby ZD - ZD2" % test_case_name
        test_cfgs.append(( {'auto_approval': False,
        ...
        test_name = 'CB_ZD_Verify_AP_Join'
        common_name = "[%s]Verify AP Join active ZD - ZD2" % test_case_name
        test_cfgs.append(( {'auto_approval': False,
                            'is_need_approval': True,
                            'zd_tag': active_zd,
                            'mac_addr_list': [active_ap_mac]}, test_name, common_name, 2, False))
        ...
        test_name = 'CB_AP_CLI_Get_Verify_Primary_Secondary_ZD'
        common_name = '[%s]Verify setting in AP CLI after AP join ZD2' % (test_case_name,)
        test_params = {'ap_tag': ap_tag,
                       'is_verify': True,
                       'primary_zd_ip': primary_zd_ip,
                       'secondary_zd_ip': secondary_zd_ip,
        ...
        common_name = "[%s]Assign wlan group to AP %s %s on ZD2" % (test_case_name, active_ap, radio_mode)
        ...
        common_name = '[%s]Associate the station to ZD2 wlan' % (test_case_name)
        ...
        common_name = '[%s]Verify ping target IPs for ZD2 wlan' % (test_case_name)
        params = {'sta_tag': sta_tag,
    ...
    common_name = 'Get active and standby ZD'
    test_cfgs.append(({'wait_time': 100}, test_name, common_name, 0, True))
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Primary Invalid Secondary ZD2 IPV6 FQDN',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - Enable FQDN with Smart Redundancy and AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Enable FQDN with Smart Redundancy 11%s" % (target_sta_radio)
