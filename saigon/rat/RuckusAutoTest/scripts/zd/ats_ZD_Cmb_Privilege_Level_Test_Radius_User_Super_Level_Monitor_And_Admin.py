...
super user can only monitor zd and see zd configuration, can change all the configuration
'''
...
def define_test_configuration(tcfg):
    ...
    common_name = '%slogin zd web by local super user' % test_case_name
    ...
    common_name = '%ssuper user ping ap should successfully'%test_case_name
    ...
    common_name = '%ssuper user can remove dpsk'%test_case_name
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="super level radius User privilege level test-Monitor and Admin"
    )
    attrs.update(kwargs)
    ...
    local_user_cfg =dict(  username = 'super_user',
                     password = 'super_user',
                     fullname = '',
    ...
    else: ts_name ="super level radius User privilege level test-Monitor and Admin"

    tcfg = dict(ping_ap_mac = active_ap_mac,
                ...
                ap_tag = active_ap,#@author: yuyanan @since:2014-8-29 optimize:ap tag replace ap mac
                )
    test_cfgs = define_test_configuration(tcfg)
    check_max_length(test_cfgs)
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "super level radius User privilege level test-Monitor and Admin", interactive_mode = attrs["interactive_mode"], combotest=True)
