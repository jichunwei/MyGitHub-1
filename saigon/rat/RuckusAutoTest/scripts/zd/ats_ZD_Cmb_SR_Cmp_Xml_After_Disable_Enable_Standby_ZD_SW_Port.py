...
1. zds on sync
2. disable switch port connected to standby zd
3. del/mod/add configuration on active ZD
4. enable switch port connected to standby zd
'''
...
#######################################################################
    ...
    test_name = 'CB_ZD_Disable_Given_Mac_Switch_Port'
    common_name = '[%s]Disable switch port connectet standby zd' % test_case_name
    test_cfgs.append(({'zd':'higher_mac_zd_mac','device':'zd'},test_name, common_name, 2, False))   
    ...
    test_name = 'CB_ZD_Enable_Sw_Port_Connect_To_Given_Device'
    common_name = '[%s]Enable sw port connected to standby zd' % test_case_name
    test_cfgs.append(({'device':'zd','number':1},test_name, common_name, 2, True))  
...
####################################################   
    ...
    test_name = 'CB_ZD_Enable_Sw_Port_Connect_To_Given_Device'
    common_name = '[%s]Enable sw port connected to standby zd' % test_case_name
    test_cfgs.append(({'device':'zd','number':1,'zdcli':'zdcli2'},test_name, common_name, 2, True)) 
...
#after the operation of edit configuration,cmp files
####################################################       
...
########################################################   
    ...
    test_name = 'CB_Scaling_Waiting'
    common_name = '[%s]:Waiting command prompt for %d seconds again' % (test_case_name,cmd_prompt_wait)
    test_cfgs.append(({'timeout':cmd_prompt_wait}, test_name, common_name, 2, False))
...
########################################################      
    ...
    test_name = 'CB_ZD_Enable_Sw_Port_Connect_To_Given_Device'
    common_name = 'Enable sw port connected to all aps' 
    test_cfgs.append(({'device':'ap'},test_name, common_name, 0, True)) 
    
    test_name = 'CB_ZD_Enable_Sw_Port_Connect_To_Given_Device'
    common_name = 'Enable sw port connected to all zds'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="compare after operation when disable-enable switch port connected to standby zd"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="compare after operation when disable-enable switch port connected to standby zd"
    test_cfgs = defineTestConfiguration(active_ap_list)
    ts = testsuite.get_testsuite(ts_name, "compare after operation when disable-enable switch port connected to standby zd", interactive_mode = attrs["interactive_mode"], combotest=True)
