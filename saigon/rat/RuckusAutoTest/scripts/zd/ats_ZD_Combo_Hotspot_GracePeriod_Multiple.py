...
def define_test_cfgs(tcfg):
    ...
    hotspot_cfg['name'] = 'gp-clients-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-clients-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    user_cfg = deepcopy(tcfg['local_user_cfg'])
    user_cfg_2 = deepcopy(tcfg['local_user_cfg_2'])
#    zj 2014-0217 fix ZF-7474 radius grace period configuration always open, and is :3
#    grace_period = 6
    grace_period = 3
    ...
    hotspot_cfg_1 = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg_1['name'] = 'gp-wlans-%s-1' % time.strftime("%H%M%S")
    wlan_cfg_1 = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg_1['ssid'] = 'wispr-wlans-%s-1' % time.strftime("%H%M%S")
    wlan_cfg_1['hotspot_profile'] = hotspot_cfg_1['name']
    grace_period_1 = 5
    
    hotspot_cfg_2 = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg_2['name'] = 'gp-wlans-%s-2' % time.strftime("%H%M%S")
    wlan_cfg_2 = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg_2['ssid'] = 'wispr-wlans-%s-2' % time.strftime("%H%M%S")
    wlan_cfg_2['hotspot_profile'] = hotspot_cfg_2['name']
    grace_period_2 = 6
    ...
    test_params = {'hotspot_cfg': hotspot_cfg_1,
                   'wlan_cfg': wlan_cfg_1,
                   'wg_cfg': tcfg['wg_cfg'],
    ...
    test_params = {'hotspot_cfg': hotspot_cfg_2,
                   'wlan_cfg': wlan_cfg_2,
                   'wg_cfg': tcfg['wg_cfg'],
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Hotspot - Grace Period - Multiple - %s - %s" 
