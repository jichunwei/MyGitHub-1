...
def define_test_cfgs(tcfg):
    ...
    tcid = '[Grace period can work when session is not timeout]'
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-sess-on-%s' % time.strftime("%H%M%S")
    hotspot_cfg['session_timeout'] = 30
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-sess-on-%s' % time.strftime("%H%M%S")
    ...
    tcid = '[Grace period not work after session is timeout]'
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-sess-out-%s' % time.strftime("%H%M%S")
    hotspot_cfg['session_timeout'] = 2
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-sess-out-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    grace_period = 2
    
    test_name = 'CB_ZD_Test_Grace_Period_Setting'
    common_name = "%sCreate a WLAN: %s" % (tcid, wlan_cfg['ssid'])
    ...
    common_name = "%sWait for client session timeout" % tcid
    test_params = {'sta_tag': sta_tag,
                   'ap_tag': ap_tag,
                   'username': user_cfg['username'],
                   'wlan_cfg': wlan_cfg,
                   'disconnect_method': 'wait_for_session_timeout',
                   'session_timeout': hotspot_cfg['session_timeout'],
                   'radio_mode': radio_mode}
    test_cfgs.append((test_params, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Test_Grace_Period'
    common_name = "%sTest client '%s' reconnect within grace period" % (tcid, sta_tag)
    test_params = {'sta_tag': sta_tag,
                   'ap_tag': ap_tag,
                   'grace_period': grace_period,
                   'reconnect_within_gp': True,
...
#    wlan_cfg['inactivity_timeout'] = 30
    ...
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-vlan-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-vlan-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    ...
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-tunnel-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-tunnel-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Hotspot - Grace Period - Combine - %s - %s" 
