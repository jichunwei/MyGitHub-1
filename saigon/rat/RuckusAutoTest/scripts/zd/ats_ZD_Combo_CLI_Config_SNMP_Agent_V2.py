'''
Configure SNMP V2 agent via ZD CLI

    SNMP V2 agent is enabled/disabled via ZD CLI successfully.
    1. The current agent information from CLI and GUI are same.
    2. Enable v2 agent, the information are same between get and set, CLI get and GUI get; verify snmpd is started, snmp command works well.
    2. Disable v2 agent, the information are same between get and set, CLI get and GUI get; verify snmpd is shutdown, snmp command can't work.
    expect result: All steps should result properly.
    
    How to:
        1) Get snmp v2 agent setting from CLI and GUI, verify they are same.
        2) Enable snmp v2 agent via ZD CLI.        
        4) Get snmp v2 agent information from GUI and CLI.
        ...
        8) Verify snmp v2 command works well.
        9) Disable snmp v2 agent via ZD CLI.        
        10) repeat do 4)-6) to disable agent.
        11) Verify snmpd process is shutdown [not in ps list].
        12) Verify snmp v2 command can't work.
...
def define_test_cfg(tcfg):
    ...
    common_name = '%sGet SNMP Agent V2 Info from CLI' % (test_case_name,)
    param_cfg = dict()
    ...
    test_case_name = '[Enable SNMP Agent V2]'
    
    test_name = 'CB_ZD_Set_SNMP_Agent_Info'
    common_name = '%sDisable SNMP Agent V2 from GUI' % (test_case_name,)
    ...
    test_name = 'CB_ZD_Get_SNMP_Agent_Info'
    common_name = '%sGet SNMP Agent V2 info from GUI' % (test_case_name,)
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    ...
    common_name = '%sVerify SNMP V2 Command Works Well' % (test_case_name,)
    param_cfg = {'snmp_agent_cfg':tcfg['enable_snmp_agent_v2_cfg'],
                 'snmp_cfg': tcfg['snmp_cfg']}
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    
    test_case_name = '[Disable SNMP Agent V2]'
    
    test_name = 'CB_ZD_Set_SNMP_Agent_Info'
    common_name = '%sEnable SNMP Agent V2 from GUI' % (test_case_name,)
    param_cfg =  {'snmp_agent_cfg': tcfg['enable_snmp_agent_v2_cfg']}
    test_cfgs.append((param_cfg, test_name, common_name, 1, False))
    
    test_name = 'CB_ZD_CLI_Set_SNMP_Agent'
    common_name = '%sDisable SNMP Agent V2 from CLI' % (test_case_name,)
    param_cfg = {'snmp_agent_cfg':tcfg['disable_snmp_agent_v2_cfg']}
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    ...
    test_name = 'CB_ZD_Get_SNMP_Agent_Info'
    common_name = '%sGet SNMPV2 Agent Info from GUI' % (test_case_name,)
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_CLI_Verify_SNMPV2_Info'
    common_name = '%sVerify SNMPV2 Agent Info between GUI Get and CLI Get' % (test_case_name,)
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    ...
    test_name = 'CB_ZD_SNMP_Verify_SNMP_Commands'
    common_name = '%sVerify SNMP V2 Command Can not Work' % (test_case_name,)
    param_cfg = {'snmp_agent_cfg':tcfg['disable_snmp_agent_v2_cfg'],
                 'snmp_cfg': tcfg['snmp_cfg']}
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
...
def create_test_suite(**kwargs):    
    ...
    ts_name = 'ZD CLI - SNMP Agent V2 Configuration'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP Agent V2 Configuration: CLI Set, GUI Get', combotest=True)
