...
#    
    ...
    common_name = 'create AD server via WebUI for operator user'
    ...
    common_name = 'login zd web by AD operator user'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Operator level AD User privilege level test-Configure"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="Operator level AD User privilege level test-Configure"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Operator level  User privilege level test-Configure", interactive_mode = attrs["interactive_mode"], combotest=True)
