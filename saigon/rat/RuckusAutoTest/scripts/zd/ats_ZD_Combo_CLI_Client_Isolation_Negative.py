...
Negative test
    ...
    AP reboot
    AP disconnect and rejoin
    AP set factory and rejoin
...
def _generate_test_cfg():
    ...
    test_cfg.append(({'tc_name' : '[AP set factory]',
                  ...
                  'AP2_radio' : '802.11g/n',
                  'ping_result' : False,
                  'action' : 'AP set factory',
                  }))

    test_cfg.append(({'tc_name' : '[AP reboot]',
                  'wlan1' : { 'wlan_cfg' : ci_wlan1_cfg,
                              'isolation_per_ap' : True,
                              'isolation_across_ap' : True,
                              'white_list' : whitelist_conf['name'],
    ...
    test_cfg.append(({'tc_name' : '[AP disconnect]',
                  'wlan1' : { 'wlan_cfg' : ci_wlan1_cfg,
                              'isolation_per_ap' : True,
                              'isolation_across_ap' : True,
                              'white_list' : whitelist_conf['name'],
...
def build_stcs_main(test_case):
    ...
    if test_case.get('action') == 'AP reboot':
        tcs.append(({'ap_tag': AP1,}, 
                     'CB_ZD_Reboot_AP', 
                      '%sAP reboot' % tc_name, 
                      2, 
                      False))
    elif test_case.get('action') == 'AP set factory':
        tcs.append(({'ap_tag': AP1,}, 
                     'CB_ZD_AP_Set_Factory_Default', 
                      '%sAP set factory' % tc_name, 
                      2, 
                      False))
    elif test_case.get('action') == 'AP disconnect':
        tcs.append(({'ap_tag': AP1,}, 
                     'CB_ZD_Disable_AP_Interface', 
                      '%sDisable AP interface' % tc_name, 
                      2, 
                      False))      

        tcs.append(({'ap_tag': AP1,}, 
                     'CB_ZD_Enable_AP_Interface', 
                      '%sEnable AP interface' % tc_name, 
                      2, 
                      False))
        ...
        tcs.append(({'wlan_cfg':wlan1_default_cfg,
                     ...
                     'ap_radio': AP1_radio},
                    'CB_AP_CLI_Get_BSSID',
                    '%sGet BSSID for AP 1 after rejion' % tc_name,
                    2,
                    False
                    ))
        ...
        tcs.append(({'sta_tag': 'sta_1', 
                     ...
                     'ap_tag': AP1}, 
                     'CB_Station_Associate_Wlan_And_Get_Wifi_Addr_BSSID', 
                      '%sAssociate the station 1 to wlan after rejion' % tc_name, 
                      2, 
                      False)) 
    ...
    tcs.append(({'src_sta_tag': 'sta_1',
                 ...
                 'clean_arp_before_ping': True},
                 'CB_ZD_Client_Ping_Another', 
                  '%sStation Ping Another Station is %s, after %s' % (tc_name,condition,test_case.get('action')), 
                  2,
                  False))
    ...
    tcs.append(({'sta_tag': 'sta_1',
                 'dest_ip': default_ip['server']}, 
                 'CB_Station_Ping_Dest_Is_Allowed', 
                  '%sStation Ping Linux Server is Allowed, after %s' % (tc_name,test_case.get('action')), 
                  2, 
                  False))
...
def create_test_suite(**kwargs):    
    attrs = dict(testsuite_name = "Client Isolation negative"
                 )
    attrs.update(kwargs)
