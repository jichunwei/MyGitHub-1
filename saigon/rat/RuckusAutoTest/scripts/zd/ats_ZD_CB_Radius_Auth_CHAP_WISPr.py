...
def build_tcs(target_station, target_station_radio, active_ap):
    ...
    for svr in svr_list:
        ...
        param_cfg = {'server_name': svr['server_name'], 
                 'user': auth_info['username'], 'password':auth_info['password'], 
                 'invalid_user': 'rad.cisco', 'invalid_password':'rad.cisco',
                 'radius_auth_method': svr['radius_auth_method'],}
        test_cfgs.append((param_cfg,
                      'CB_ZD_Verify_Radius_Server_Auth_Method',
                      'Test Radius Authentication Server',
                      0,
                      False,
                      ))
...
def clean_steps(LEVEL = 0):
    ...
    return [
            ...
            'Remove All WLAN Groups for cleanup ENV %s' % tmp,
            0, 
            clean_up),              
            ({}, 
            'CB_ZD_Remove_All_Wlans', 
            'Clean all WLANs for cleanup ENV %s' % tmp,
            0, 
            clean_up),            
            ({}, 
            'CB_ZD_Remove_All_Profiles', 
            'Remove all profiles %s' % tmp,
            0, 
            clean_up),            
            ({},
             'CB_ZD_Remove_All_Authentication_Server',
             'Remove all AAA servers %s' % tmp,
             0,
             clean_up)
            ]
...
def station_a_steps(active_ap, sta, radio, wlan, wg):
    ...
    test_cfgs.append(({'sta_tag': 'sta_1',
                       ...
                       'The station ping a target IP before hotspot auth', 
                       2, 
                       False))
...
#                     ))  
    ...
    test_cfgs.append(({'sta_tag': 'sta_1',
                     ...
                     'CB_ZD_Client_Ping_Dest', 
                     'The station ping a target IP after hotspot auth', 
                     2, 
                     False))
...
def _get_common_name(common_name):
    ...
    if b_index >=0 and e_index > b_index:        
        return '[%s]%s'% (common_name[b_index+1:e_index], common_name[e_index+1:])
    else:
        return common_name 
...
def create_test_suite(**kwargs):
    ...
    attrs = dict(interactive_mode = True,
                 station = (0,"g"),
                 targetap = False,
                 testsuite_name = "WISPr - 11ng - Radius Auth CHAP",
                 )
    attrs.update(kwargs)
    ...
    if attrs["interactive_mode"]:
        ts_name = "WISPr - 11%s - Radius Auth CHAP" % (target_sta_radio, )        
    else:
        ts_name = attrs["testsuite_name"]

    ts = testsuite.get_testsuite(ts_name, 
                                 "WISPr - 11%s - Radius Auth CHAP" % (target_sta_radio, ), 
                                 combotest=True)
