'''
Configure SNMP V3 agent via ZD CLI [RO RW Auth=MD5 Priv=AES]
    ...
    How to:
        1) Enable snmp v3 agent with the setting via ZD CLI.
        2) Get snmp v3 agent via ZD CLI.        
        4) Verify snmp v3 agent setting between CLI get and set.
...
Description: 
     This script is verify snmp authentication and privacy methods. 
"""
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO RW Auth=MD5 Priv=AES]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD CLI - SNMP V3 Validation - Both RO and RW (MD5,AES)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO and RW Authentication is MD5 and Privacy is AES', combotest=True)
