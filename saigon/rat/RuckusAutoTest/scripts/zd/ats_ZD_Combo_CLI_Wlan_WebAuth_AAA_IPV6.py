"""
Verify wlan with web authentication with local database works well for dual stack and ipv6.
...
def define_test_cfg(cfg):
    ...
    if cfg['server_type'] == AUTH_TYPE_LOCAL_DATABASE:
        ...
        test_params = {'server_cfg_list': [ras_cfg]}
        test_cfgs.append((test_params, test_name, common_name, 0, False))
...
def _define_station_test_cfg(cfg, test_case_name, wlan_cfg, sta_tag, browser_tag, ap_tag, ip_version):
    ...
    test_cfgs.append(({'sta_tag': sta_tag,
                       ...
                       'password': sta_wlan_cfg['password'],
...
def _get_wlan_cfg(wlan_params):
    ...
    wlanCfg = dict(ssid=ssid, name="", type='standard-usage', auth="open", 
                   ...
                   username="", password="",web_auth=True)
...
def _define_wlan_cfg_list(ras_cfg):
    ...
    #wlan_cfgs.append(_get_wlan_cfg(dict(auth = "open", encryption = "wpa", algorithm = 'TKIP',
    #                                    passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                                    username = username, password = password, auth_server = ras_name)))
    wlan_cfgs.append(_get_wlan_cfg(dict(auth = "open", encryption = "wpa2",algorithm = 'AES',
                                        passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                                        username = username, password = password, auth_server = ras_name)))
    '''
    wlan_cfgs.append(_get_wlan_cfg(dict(auth = "open", encryption = "wpa-mixed", algorithm = random.choice(['TKIP','AES']),
                                        passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "ZD CLI - Web Auth %s ZD %s AP %s 11%s" % (server_type, zd_ip_version, ap_ip_version, target_sta_radio)
