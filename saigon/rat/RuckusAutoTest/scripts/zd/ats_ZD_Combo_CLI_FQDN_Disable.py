...
def define_test_cfg(cfg):
    ...
    test_name = 'CB_ZD_CLI_Set_Primary_Secondary_ZD'    
    common_name = '[%s]Disable FQDN in ZD1 via CLI' % test_case_name
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - Disable FQDN with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Disable FQDN 11%s" % (target_sta_radio)
