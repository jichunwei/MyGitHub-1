...
def define_wlan_cfg(tcfg):
    ...
    #@author: liang aihua,@since: 2015-2-4,@change: remove these wlans for their security can't be set successfully using command "8021x eap-type EAP-SIM auth_server local"
    #_wlan_cfgs['dot1x-eap-sim-local'] = (dict(name = 'dot1x-eap-sim-local',
    #                       auth = 'dot1x-eap',
    #                       encryption = 'none',
    #                       eap_type = 'EAP-SIM',
    #                       auth_server = 'local'
    ...
    #_wlan_cfgs['dot1x-eap-peap-local'] = (dict(name = 'dot1x-eap-peap-local',
    #                       auth = 'dot1x-eap',
    #                       encryption = 'none',
    #                       eap_type = 'PEAP',
    #                       auth_server = 'local'
    ...
    _wlan_cfgs['dot1x-wep-64-local'] = (dict(name = 'dot1x-wep-64-local',
                           auth = "dot1x-eap", encryption = "wep-64",
                           auth_server = 'local'
                           ))

    _wlan_cfgs['dot1x-wep-128-local'] = (dict(name = 'dot1x-wep-128-local',
                           auth = "dot1x-eap", encryption = "wep-128",
                           auth_server = 'local'
                           ))
    
    #_wlan_cfgs['dot1x-wpa-tkip-local'] = (dict(name = 'dot1x-wpa-local',
    #                       auth = "dot1x-eap", encryption = "wpa", algorithm = 'TKIP',
    #                       auth_server = 'local'
    #                       ))
    
    #_wlan_cfgs['dot1x-wpa-aes-local'] = (dict(name = 'dot1x-wpa-local',
    #                       auth = "dot1x-eap", encryption = "wpa", algorithm = 'AES', 
    #                       auth_server = 'local'
    #                       ))
    
    _wlan_cfgs['dot1x-wpa-auto-local'] = (dict(name = 'dot1x-wpa-local',
                           auth = "dot1x-eap", encryption = "wpa", algorithm = 'auto' ,
                           auth_server = 'local'
                           ))

    #_wlan_cfgs['dot1x-wpa2-tkip-local'] = (dict(name = 'dot1x-wpa2-tkip-local',
    #                       auth = "dot1x-eap", encryption = "wpa2", algorithm =  'TKIP',
    #                       auth_server = 'local'
    #                       ))
    
    
    _wlan_cfgs['dot1x-wpa2-aes-local'] = (dict(name = 'dot1x-wpa2-aes-local',
                           auth = "dot1x-eap", encryption = "wpa2", algorithm =  'AES', 
                           auth_server = 'local'
                           ))
    
    _wlan_cfgs['dot1x-wpa2-auto-local'] = (dict(name = 'dot1x-wpa2-auto-local',
                           auth = "dot1x-eap", encryption = "wpa2", algorithm = 'auto',
                           auth_server = 'local'
                           ))
...
def define_test_cfg(cfg,wlan_cfg_dict):
    test_cfgs = []
    local_user = dict(username = cfg['username'],password = cfg['password'])
    ...
    test_cfgs.append(({'username':local_user['username'], 
                       'password':local_user['password']}, test_name, common_name, 0, False))
    ...
    for key, wlancfg in wlan_cfg_dict.items():
        if wlancfg.has_key('encryption') and wlancfg.get('encryption').lower().find('wpa') > -1:
            wpa_ver_list = [wlancfg.get('encryption').upper()]
...
def _define_data_plane_test_cfg(cfg, test_case_name, target_ip_addr, wlan_cfg, sta_tag, browser_tag, ap_tag, is_wpa_auto=False):
    ...
    sta_wlan_cfg = _convert_wlan_cfg(wlan_cfg, username, password)
    ...
    #@author: liang aihua,@since: 2015-2-4,@change: tb72 not ready for downloading files.
...
def createTestSuite(**kwargs):
    ...
    if active_ap :
        ...
        if attrs["testsuite_name"]:
            ...
            #@author: Liang Aihua,@since: 2014-12-26,@change: Remove ap model from ts name. 
            #ts_name = "ZDCLI- Encryption Types-802.11x-Local - 11%s %s" % (target_sta_radio, ap_model)
            ts_name = "ZDCLI- Encryption Types-802.11x-Local - 11%s" % target_sta_radio
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(802.11x-Local) properly - 11%s radio" % target_sta_radio,
