"""
Verify AP joins expected ZD when enable FQDN in AP CLI and set AP to factory default.
    ...
    Test Data:
        Primary ZD2 IPV4 FQDN Secondary Invalid
        Primary ZD2 IPV6 FQDN Secondary Invalid
    ...
    How to:
        ...
        5) Create station.
        6) Create active ap.
        7) Create wlan and wlan group in ZD1 and ZD2.
        8) Prepare for test:
              Disable FQDN in ZD1 and AP CLI 
              Remove all APs from ZD1 and ZD2
              Verify AP joins ZD1.
        9) Configure FQDN in AP CLI, primary is zd2, secondary is invalid domain name.
        10) Verify FQDN in AP CLI.
        11) Verify AP joins ZD2.
        12) Verify FQDN settings after AP joins ZD2.
        13) Associate station to ZD2 wlan and verify ping traffic to server.
        14) Set AP to factory default.
        15) Verify FQDN setting is disable [primary and secondary are ''(n/a)]
        16) Verify AP joins ZD1.
        17) Verify FQDN settings after AP joins ZD1.
        18) Associate station to ZD1 wlan and verify ping traffic to server.
        18) Clean up the environment: 
              Disable FQDN in ZD1, ZD2 and AP CLI
              Remove all APs in ZD1 and ZD2
              Make sure AP joins ZD1.
              Remove all wlan and wlan groups in ZD1 and ZD2.               
...
def define_test_cfg(cfg):
    ...
    test_cfgs.append(({'cfg_type': "init", 'zd_tag':zd1_gui_tag}, test_name, common_name, 0, False))
    ...
    common_name = 'Create a wlan and remove from default wlan group on ZD1'
    ...
    test_name = 'CB_ZD_Create_Wlan'
    common_name = 'Create a wlan and remove from default wlan group on ZD2'
    test_cfgs.append(({'zd_tag': zd2_gui_tag,
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        #@author: Anzuo, @bug: ZF-5551, remove AP from ZD before set AP factory , otherwise AP will join last join ZD again
        test_name = 'CB_ZD_SR_Delete_All_AP'
        common_name = '[%s]Remove all APs from ZD1 ZD2 during test' % test_case_name
        test_params = {}
        test_cfgs.append((test_params, test_name, common_name, 2, False))

        #AP set factory default.
        test_name = 'CB_ZD_AP_Set_Factory_Default'
        common_name = '[%s]Set AP to factory default' % test_case_name
        test_params = {'ap_tag': ap_tag,
                       }
        test_cfgs.append((test_params, test_name, common_name, 2, False))

        test_name = 'CB_AP_CLI_Get_Verify_Primary_Secondary_ZD'
        common_name = '[%s]Verify FQDN in AP CLI after set factory' % test_case_name
        test_params = {'ap_tag': ap_tag,
                       'is_verify': True,
                       'primary_zd_ip': '',
                       'secondary_zd_ip': '',
        ...
        test_name = 'CB_Station_Associate_Get_IP_Verify_Subnet'
        common_name = '[%s]Associate the station to ZD1 wlan after set factory default' % (test_case_name,)
        params = {'sta_tag': sta_tag,
        ...
        test_name = 'CB_Station_Ping_Targets_Download_File'
        common_name = '[%s]Verify ping target IPs for ZD1 wlan after set factory default' % (test_case_name,)
        params = {'sta_tag': sta_tag,
                  'target_ip_list': target_ip_list,
                  'download_file': False,
                  'close_browser': False,
    ...
    test_cfgs.append(({'cfg_type': "teardown", 'zd_tag':zd1_gui_tag}, test_name, common_name, 0, False))
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Primary ZD2 IPV4 FQDN Secondary Invalid',
                          'primary_zd_ip': zd2_ipv4_dn,
                          'secondary_zd_ip': zd_ipv4_dn_unreachable,
                          'expected_zd_tag': zd2_gui_tag})
    
    test_cfg_list.append({'case_name': 'Primary ZD2 IPV6 FQDN Secondary Invalid',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "AP CLI - Enable FQDN Set Factory Default with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "AP CLI - Enable FQDN Set Factory Default 11%s" % (target_sta_radio)
