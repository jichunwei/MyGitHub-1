'''
Title: Get_Sta_OS_and_Hostname_via_DHCP_Hotspot

Test purpose: Verify if ZD can get station OS type and hostname information correctly with hotspot wlan.
...
7) Create a wlan with hotspot service on ZD
...
14) Station perform hotspot auth
...
def define_test_cfg(cfg):
    ...
    common_name = 'Create hotspot file for hotspot auth by CLI'
    hotspot_cfg = {}
    hotspot_cfg.update(cfg['hotspot_cfg'])
    test_params = {'hotspot_conf': hotspot_cfg}
    test_cfgs.append((test_params, test_name, common_name, 0, False))
...
#---TestCase-----------------------------------------------------------------------------------------
    ...
    for test_case_name in test_case_name_list:
        ...
        common_name = '%sConfigure a wlan on ZD by CLI' % (test_combo_case_name,)
        wlan_cfg = {}
        ...
        if test_combo_case_name == '[Client Fingerprinting on, option82 off]':
            ...
            wlan_cfg.update({'tunnel_mode': True, 'vlan_id': 2, 'fingerprinting': True, 'option82': False})
            
        elif test_combo_case_name == '[Tunnel on, VLAN on, Client Fingerprinting on, option82 on]':
            wlan_cfg.update({'tunnel_mode': True, 'vlan_id': 2, 'fingerprinting': True, 'option82': True})
            
        test_cfgs.append(({'wlan_cfg': wlan_cfg}, test_name, common_name, 1, False))
        ...
        if test_combo_case_name in ['[Tunnel on, VLAN on, Client Fingerprinting on, option82 off]', 
                              '[Tunnel on, VLAN on, Client Fingerprinting on, option82 on]']:
    ...
    ### new added tests script.
        ...
        common_name = '%sStation perform hotspot auth' % (test_combo_case_name,)
        test_cfgs.append(({'sta_tag': sta_tag,
                           'username': username,
                           'password': password,
...
#---TestCase End-----------------------------------------------------------------------------------
    ...
    test_name = 'CB_ZD_CLI_Configure_Hotspot'
    common_name = 'Remove generated hotspot files on ZD by CLI'
    test_params = {'cleanup': True, 'hotspot_conf': {'name': hotspot_cfg['name']}}
    test_cfgs.append((test_params, test_name, common_name, 0, True))
...
def create_test_suite(**kwargs):
    ...
    ssid = 'hotspot_%s' % (time.strftime("%H%M%S"))
    hs_name = 'get-sta-os-hn-%s' % (time.strftime("%H%M%S"))
    hotspot_cfg = {'authentication_server': 'Local Database', 
                   'name': hs_name, 
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = 'Hotspot Auth - Get Sta OS and Hostname via DHCP - Basic'

    ts = testsuite.get_testsuite(
        ts_name, 'Verify ZD obtaining station OS type and hostname via DHCP with Hotspot Auth WLAN.',
