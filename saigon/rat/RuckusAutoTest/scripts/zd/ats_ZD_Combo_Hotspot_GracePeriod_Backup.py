...
def grace_period_test(cfg):
    ...
    test_params = {'hotspot_name': conf['hotspot_cfg']['name'],
                   'grace_period': grace_period}
    test_cfgs.append((test_params, test_name, common_name, 2, False))
...
def define_test_cfgs(tcfg):
    ...
    hotspot_cfg['name'] = 'gp-full-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-full-%s' % time.strftime("%H%M%S")
    ...
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-failover-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-failover-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    test_params = {'tcid': tcid,
                   'sta_tag': sta_tag,
                   'ap_tag': ap_tag,
    ...
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-policy-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-policy-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    test_params = {'tcid': tcid,
                   'sta_tag': sta_tag,
                   'ap_tag': ap_tag,
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Hotspot - Grace Period - Backup Restore - %s - %s" 
