...
def define_test_configuration(tcfg):
    ...
    common_name = '%screate one monitor role'%test_case_name
    test_cfgs.append(({'role_cfg':tcfg['role']}, test_name, common_name, 0, False))
    
    test_name = 'CB_ZD_Create_Local_User' 
    common_name = '%screate one monitor user'%test_case_name
    ...
    common_name = '%slogin zd web by tacacs plus monitor user' % test_case_name
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Monitor level Tacacs Plus User privilege level test-Monitor and Admin"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="Monitor level Tacacs Plus User privilege level test-Monitor and Admin"
    ...
    tcfg = dict(ping_ap_mac = active_ap_mac,
                ...
                server_cfg = monitor_cfg,
                server_admin = monitor_admin,
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Monitor level Tacacs Plus User privilege level test-Monitor and Admin", interactive_mode = attrs["interactive_mode"], combotest=True)
