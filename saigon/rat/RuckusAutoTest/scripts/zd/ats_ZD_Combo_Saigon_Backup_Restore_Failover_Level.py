...
def defineInputConfig(tbcfg):
    test_conf = {'zd1_ip_addr':'192.168.0.2',
                 'zd2_ip_addr':'192.168.0.3',
                 }
    test_conf['share_secret'] = _generate_secret_key(random.randint(5,15))
#    test_conf['zd1_ip_addr'] = tbcfg['ZD']['ip_addr']
#    zd2_ip_addr = raw_input("Enter the Standby ZD IP address: ")
#    if not zd2_ip_addr:
#        test_conf['zd2_ip_addr'] =zd2_ip_addr
...
def defineTestConfiguration(tbcfg):
    test_cfgs = []
    input_cfg = defineInputConfig(tbcfg)
    ...
    common_name = '[Backup in SM-Failover]Remove all the Configurations'
    test_cfgs.append(({},test_name, common_name,1,False))
    
    test_name = 'CB_ZD_Create_Wlan'
    common_name = '[Backup in SM-Failover]Create "OPEN-NONE" WLANs'
    ...
    common_name = '[Backup in SM-Failover]Create a Single Role on ZD named rat-role'
    test_cfgs.append(({'role_cfg':role_cfg},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Create_Local_User'
    common_name = '[Backup in SM-Failover]:Create a Local user rat.local.user'
    ...
    common_name = '[Backup in SM-Failover]Create a Wlan Group'
    test_cfgs.append(({'wgs_cfg':wgs_cfg},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Create_L2_ACLs'
    common_name = '[Backup in SM-Failover]Create a L2 ACL'
    test_cfgs.append(({'num_of_mac':1},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Create_L3_ACLs'
    common_name = '[Backup in SM-Failover]Create a L3 ACL'
    test_cfgs.append(({'num_of_acls':1,
                       'num_of_rules':1,},
                       test_name, common_name,2,False)) 

    test_name = 'CB_ZD_Change_System_Name'
    common_name = '[Backup in SM-Failover]Change System Name to ruckus-before-backup'
    test_cfgs.append(({'system_name':'ruckus-before-backup'},test_name,common_name,2,False))
    
    test_name = 'CB_ZD_Create_Hotspot_Profiles'
    common_name = '[Backup in SM-Failover]Create a hotspot profile with grace period'
    test_cfgs.append(({'hotspot_profiles_list': gp_profile_list}, 
                      test_name, common_name, 0, False))
    
    test_name = 'CB_ZD_Create_Wlan'
    common_name = '[Backup in SM-Failover]Create WLANs with grace period'
    test_cfgs.append(({'wlan_cfg_list': gp_wlan_cfg_list}, 
                      test_name, common_name, 0, False))
        
    save_to = constant.save_to
    test_name = 'CB_ZD_Backup'
    common_name = '[Backup in SM-Failover]Backup System'
    test_cfgs.append(({'save_to':save_to},test_name, common_name,2,False)) 
    
    test_name = 'CB_Scaling_RemoveZDAllConfig'
    common_name = '[Restore in SM-Failover]Remove all the Configurations'
    test_cfgs.append(({},test_name, common_name,1,False)) 
    
#    test_name = 'CB_ZD_Change_System_Name'
#    common_name = 'TCID: 45.03.02.06.15 Change System Name to ruckus-after-backup'
#    test_cfgs.append(({'system_name':'ruckus-after-backup'},test_name,common_name,0,False))
#    
    test_name = 'CB_ZD_Restore'
    common_name = '[Restore in SM-Failover]:Restore the backup file'
    ...
    common_name = '[Restore in SM-Failover]Verify L3 ACL has restored correctly'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_Local_User'
    common_name = '[Restore in SM-Failover]Verify users which are restored to original configuration'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_Wlan_Groups_Info'
    common_name = '[Restore in SM-Failover]Verify WLAN Group rat-restore-wlangroup exist'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_Wlans_Info'
    common_name = '[Restore in SM-Failover]Verify WLANs which are restored to original configuration'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_System_Name'
    common_name = '[Restore in SM-Failover]Verify System Name can not restore'
    test_cfgs.append(({'before_system_name':'ruckus-before-backup',
                       'system_name_change':False},
                       test_name, common_name,2,False))
    
    for profile_cfg in gp_profile_list:
        test_name = 'CB_ZD_Verify_Grace_Period_In_GUI'
        common_name = "[Restore in SM-Failover]Verify grace period in profile: %s" % profile_cfg['name']
    ...
    for wlan_cfg in gp_wlan_cfg_list:
        test_name = 'CB_ZD_Verify_Grace_Period_In_GUI'
        common_name = "[Restore in SM-Failover]Verify grace period in WLAN: %s" % wlan_cfg['ssid']
    ...
    common_name = "[Restore in SM-Failover]Clear up the Smart Redundancy test environment"
    test_cfgs.append(({},test_name, common_name,0,False)) 
...
def createTestSuite(**kwargs):
    ...
    ts_name = 'Backup and Restore by Failover mode under ZD smart redundancy'
    ts = testsuite.get_testsuite(ts_name, 'Verify ZD can backup restore by Failover mode under ZD smart redundancy ENV', combotest=True)
    test_cfgs = defineTestConfiguration(tbcfg)
