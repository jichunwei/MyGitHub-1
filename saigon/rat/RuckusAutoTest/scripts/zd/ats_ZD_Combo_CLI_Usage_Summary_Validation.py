...
def gen_id(id):
    ex_id = "[Usage Summary configure from GUI, check from CLI]"
    return '%s(%d) ' % (ex_id, id) 

def define_test_configuration(cfg):
    test_cfgs = []
    i = 1     
    test_name = 'CB_ZD_Get_Usage_Summary'
    ...
    i += 1    
    test_name = 'CB_ZD_CLI_Verify_Usage_Summary'
    common_name = '%sVerify usage summary between GUI and CLI' % gen_id(i)
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))  
...
def create_test_suite(**kwargs):
    ts_name = 'TCID: x-Show system usage summary information in CLI'
    ts = testsuite.get_testsuite(ts_name, 'Show system usage summary information in CLI', combotest=True)
