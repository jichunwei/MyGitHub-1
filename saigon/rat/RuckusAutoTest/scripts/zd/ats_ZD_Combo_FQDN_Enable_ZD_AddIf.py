...
Verify AP joins expected ZD when enable FQDN in ZD CLI.
    ...
    Test Data:
        ...
        Primary IPV4 FQDN Secondary IPV6 FQDN Join Secondary
...
def define_test_cfg(cfg):
    ...
    #Enable ZD additional management vlan
    test_name = 'CB_ZD_Enable_Mgmt_Interface'
    common_name = 'Enable Management Interface from GUI on ZD1'
    test_cfgs.append(({'ip_addr':'192.168.10.5',
                       'vlan': 10,
                       'zd_tag': zd1_gui_tag},test_name, common_name, 0, False))    
    
    test_name = 'CB_ZD_Enable_Mgmt_Interface'
    common_name = 'Enable Management Interface from GUI on ZD2'
    test_cfgs.append(({'ip_addr':'192.168.10.6',
                       'vlan': 10,
                       'zd_tag': zd2_gui_tag},test_name, common_name, 0, False))
    ...
    common_name = 'Disable Additional MGMT IF via GUI on ZD1'
    param_cfg = dict(zd_tag=zd1_gui_tag)
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))
    
    test_name = 'CB_ZD_Disable_Mgmt_Interface'
    common_name = 'Disable Additional MGMT IF via GUI on ZD2'
    param_cfg = dict(zd_tag=zd2_gui_tag)
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Primary IPV4 FQDN Secondary IPV4 FQDN Join Secondary',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - Enable FQDN with ZD Addition Interface 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Enable FQDN with ZD Addition Interface 11%s" % (target_sta_radio)
    
        ts = testsuite.get_testsuite(ts_name, "Verify AP join specified ZD when enable FQDN and ZD additional mgmt vlan", combotest = True)
