...
def define_test_configuration(tcfg):
    ...
    common_name = 'login zd web by AD monitor user'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Monitor level AD User privilege level test-Monitor and Admin"
    )
    attrs.update(kwargs)
...
#    
    ...
    else: ts_name ="Monitor level AD User privilege level test-Monitor and Admin"

    tcfg = dict(target_sta = target_sta,
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Monitor level AD User privilege level test-Monitor and Admin", interactive_mode = attrs["interactive_mode"], combotest=True)
