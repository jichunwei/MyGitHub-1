...
def define_test_cfgs(tcfg):
    ...
    tcid = '[IPASS - Web Authentication combination] ' 
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'webauth-inactive-%s' % time.strftime("%H%M%S")
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Radius Class Attribute - WebAuth Accounting" 
        
    ts = testsuite.get_testsuite(ts_name,
                                 "Verify the Radius Class Attribute (IPass) on WebAuth Accounting WLANs",
