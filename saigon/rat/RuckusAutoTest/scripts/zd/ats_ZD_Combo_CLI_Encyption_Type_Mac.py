...
def define_wlan_cfg(tcfg):
    ...
    _wlan_cfgs['mac-none'] = (dict(name = 'mac-none',
                           auth = 'mac',
                           encryption ='none',
                           auth_server = auth_server,
                           ))
    
    _wlan_cfgs['mac-wep-64'] = (dict(name = 'mac-wep-64',
                           auth = "mac", encryption = "wep-64",
                           key_index = random.randint(1,4) , key_string = utils.make_random_string(10, "hex"),
                           auth_server = auth_server
                           ))
    
    _wlan_cfgs['mac-wep-128'] = (dict(name = 'mac-wep-128',
                           auth = "mac", encryption = "wep-128",
                           key_index = random.randint(1,4) , key_string = utils.make_random_string(26, "hex"),
                           auth_server = auth_server
                           ))
    
    #_wlan_cfgs['mac-wpa-tkip'] = (dict(name = 'mac-wpa-tkip',
    #                       auth = "mac", encryption = "wpa", algorithm = 'TKIP',
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       auth_server = auth_server
    #                       ))
    
    #_wlan_cfgs['mac-wpa-aes'] = (dict(name = 'mac-wpa-aes',
    #                       auth = "mac", encryption = "wpa", algorithm = 'AES', 
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       auth_server = auth_server
    #                       ))
    
    #_wlan_cfgs['mac-wpa-auto'] = (dict(name = 'mac-wpa-auto',
    #                       auth = "mac", encryption = "wpa", algorithm = 'auto',
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       auth_server = auth_server
    #                       ))
    

    #_wlan_cfgs['mac-wpa2-tkip'] = (dict(name = 'mac-wpa2-tkip',
    #                       auth = "mac", encryption = "wpa2", algorithm = 'TKIP', 
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       auth_server = auth_server
    #                       ))
    
    _wlan_cfgs['mac-wpa2-aes'] = (dict(name = 'mac-wpa2-aes',
                           auth = "mac", encryption = "wpa2", algorithm = 'AES', 
                           passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                           auth_server = auth_server
                           ))
    _wlan_cfgs['mac-wpa2-auto'] = (dict(name = 'mac-wpa2-auto',
                           auth = "mac", encryption = "wpa2", algorithm = 'auto',
                           passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                           auth_server = auth_server
                           ))
...
def _define_data_plane_test_cfg(cfg, test_case_name, target_ip_addr, wlan_cfg, sta_tag, browser_tag, ap_tag, is_wpa_auto=False):
    ...
    test_cfgs.append(({'sta_tag': sta_tag,
                       ...
                       #'username': username #Remove it for extra username is not need in mac auth
                       },
                       test_name, common_name, 2, False))
    ...
    #Romove this step for tb72 bad test environment
...
def createTestSuite(**kwargs):
    ...
    if active_ap :
        ...
        if attrs["testsuite_name"]:
            ...
            ts_name = "ZDCLI- Encryption Types-Mac - 11%s" %target_sta_radio
            #ts_name = "ZDCLI- Encryption Types-Mac - 11%s %s" % (target_sta_radio, ap_model)
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(Mac) properly - 11%s radio" % target_sta_radio,
