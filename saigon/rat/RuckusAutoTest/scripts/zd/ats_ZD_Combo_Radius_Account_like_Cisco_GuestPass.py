"""
Verify Radius accounting procedure like Cisco with WLAN type Guest Pass
    ...
    How to:
        ...
        5) Create a Guest-Access wlan with radius authentication and radius accounting server(interim update 1 min)
        6) Generate a guest pass automatically'
        7) Station associate the wlan
        8) Get station wifi address
        9) Verify station information in ZD, status is unauthorized
        10) Perform web authentication by Guest Key, and verify station information in ZD, status is authorized
...
def _define_wlan_cfg_guestpass(acct_svr = '', wlan_interim_update='', do_tunnel=False, vlan_id='',do_grace_period=False, grace_period=''):
    wlan_cfg = dict(ssid='ras-acct-cisco_guest', auth="open", encryption="none")
...
def define_test_cfg(cfg):
    ...
    acct_authentic = 'Local'
    wlan_cfg = _define_wlan_cfg_guestpass(ras_acct_cfg['server_name'], wlan_interim_update, do_tunnel, vlan_id, do_grace_period)

    nas_id_type='wlan-bssid'
    test_case_name = '[Radius Acct GuestPass WLAN, NAS-ID type %s]' % nas_id_type
...
def createTestSuite(**kwargs):
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = "Radius Accounting like Cisco - GuestPass"

    ts = testsuite.get_testsuite(ts_name, "Radius Accounting like Cisco - GuestPass" , combotest=True)
