...
def define_wlan_cfg(tcfg):
    auth_server = tcfg['ras_name']
    
    _wlan_cfgs = dict()
    
    #@author: liang aihua,@change: remove wlans not configured under zdcli,@since: 2015-2-4    
    #_wlan_cfgs['dot1x-eap-sim'] = (dict(name = 'dot1x-eap-sim',
    #                       auth = 'dot1x-eap',
    #                       encryption = 'none',
    #                       eap_type = 'EAP-SIM',
    ...
    #_wlan_cfgs['dot1x-eap-peap'] = (dict(name = 'dot1x-eap-peap',
    #                       auth = 'dot1x-eap',
    #                       encryption = 'none',
    #                       eap_type = 'PEAP',
    #                       auth_server = auth_server
    ...
    _wlan_cfgs['dot1x-wep-64'] = (dict(name = 'dot1x-wep-64',
                           auth = "dot1x-eap", encryption = "wep-64",key_index=1,
                           auth_server = auth_server
                           ))
    
    _wlan_cfgs['dot1x-wep-128'] = (dict(name = 'dot1x-wep-128',
                           auth = "dot1x-eap", encryption = "wep-128",key_index=2,
                           auth_server = auth_server
                           ))
    
    #_wlan_cfgs['dot1x-wpa-tkip'] = (dict(name = 'dot1x-wpa-tkip',
    #                       auth = "dot1x-eap", encryption = "wpa", algorithm = 'TKIP', 
    #                       auth_server = auth_server
    #                       ))
    
    #_wlan_cfgs['dot1x-wpa-aes'] = (dict(name = 'dot1x-wpa-aes',
    #                       auth = "dot1x-eap", encryption = "wpa", algorithm = 'AES', 
    #                       auth_server = auth_server
    #                       ))
    
    #_wlan_cfgs['dot1x-wpa-auto'] = (dict(name = 'dot1x-wpa-auto',
    #                       auth = "dot1x-eap", encryption = "wpa", algorithm = 'auto' ,
    #                       auth_server = auth_server
    #                       ))

    #_wlan_cfgs['dot1x-wpa2-tkip'] = (dict(name = 'dot1x-wpa2-tkip',
    #                       auth = "dot1x-eap", encryption = "wpa2", algorithm = 'TKIP', 
    #                       auth_server = auth_server
    #                       ))
    
    _wlan_cfgs['dot1x-wpa2-aes'] = (dict(name = 'dot1x-wpa2-aes',
                           auth = "dot1x-eap", encryption = "wpa2", algorithm = 'AES', 
                           auth_server = auth_server
                           ))
    
    _wlan_cfgs['dot1x-wpa2-auto'] = (dict(name = 'dot1x-wpa2-auto',
        ...
        if wlancfg.has_key('encryption') and wlancfg.get('encryption').lower().find('wpa') > -1:
            ...
            for wpa_ver in wpa_ver_list:
                for encypt in encypt_list:
                    ...
                    common_name = '%sGet ZD Wlan Info via CLI' % (test_case_name,)
                    test_cfgs.append(({}, test_name, common_name, 2, False))
                    
                    test_name = 'CB_ZD_CLI_Verify_Wlan_Info_Between_Set_Get'
                    common_name = '%sVerify Wlan Info Between CLI Set and CLI Get' % (test_case_name,)
                    test_cfgs.append(( {}, test_name, common_name, 2, False))
                    
                    test_name = 'CB_ZD_Get_Wlans_Info'
                    common_name = '%sGet Wlan Info via GUI' % (test_case_name)
                    test_cfgs.append(( {}, test_name, common_name, 2, False))
                    
                    test_name = 'CB_ZD_CLI_Verify_Wlan_Info_Between_CLISet_GUIGet'
                    common_name = '%sVerify Wlan Info Between CLI Get and GUI Get' % (test_case_name)
                    test_cfgs.append(( {}, test_name, common_name, 2, False))
...
def _define_data_plane_test_cfg(cfg, test_case_name, target_ip_addr, wlan_cfg, sta_tag, browser_tag, ap_tag, is_wpa_auto=False):
    ...
    #@author: liang aihua,@change: tb72 test environment problem for download,@since: 2015-2-4
...
def createTestSuite(**kwargs):
    ...
    if active_ap:
        tcfg = {'ras_ip_addr':server_ip_addr,
                'ras_port' : '1812',
                'ras_name' : 'rat-radius',
        ...
        if attrs["testsuite_name"]:
            ...
            ts_name = "ZDCLI- Encryption Types-802.11x - 11%s" %target_sta_radio
            #ts_name = "ZDCLI- Encryption Types-802.11x - 11%s %s" % (target_sta_radio, ap_model)
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(802.11x) properly - 11%s radio" % target_sta_radio,
