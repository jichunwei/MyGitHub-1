'''
Configure SNMP V3 agent via ZD CLI [RO Auth=MD5 Priv=None RW Auth=SHA Priv=DES]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=MD5 Priv=None RW Auth=SHA Priv=DES]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD CLI - SNMP V3 Validation - RO (MD5,None) RW (SHA,DES)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=MD5,priv=None) RW (auth=SHA,priv=DES)', combotest=True)
