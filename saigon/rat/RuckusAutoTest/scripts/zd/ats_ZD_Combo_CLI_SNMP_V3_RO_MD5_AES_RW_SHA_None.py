'''
Configure SNMP V3 agent via ZD CLI [RO Auth=MD5 Priv=AES RW Auth=SHA Priv=None]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=MD5 Priv=AES RW Auth=SHA Priv=None]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD CLI - SNMP V3 Validation - RO (MD5,AES) RW (SHA,None)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=MD5,priv=AES) RW (auth=SHA,priv=None)', combotest=True)
