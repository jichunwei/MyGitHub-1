"""
Verify wlan with shared and eap encryption types and zero-it works well for dualstack and ipv6

    Verify wlan with shared and eap works well when ZD is dual stack and ipv6 only.
...
def define_test_cfg(cfg):
    ...
    open_wlan_cfg = cfg['open_wlan_cfg']
    ...
    common_name = 'Create a open none wlan on ZD for download zero it'
    test_cfgs.append(({'wlan_cfg_list':[open_wlan_cfg],
    ...
    for wlan_cfg in wlans_cfg_list:
        ...
        if wlan_cfg.has_key('auth_svr'):
            auth_svr = wlan_cfg['auth_svr']
        else:
            auth_svr = "Local Database"
                
        test_case_name = '[%s %s]' % (wlan_encrypt, auth_svr)
        
        #Wlan and zero it authentication servers must be same.
        #Now our RatToolAgent only support Local Database.
        test_name = 'CB_ZD_Create_Wlan'
        ...
        expect_ap_wlan_cfg = _define_expect_wlan_info_in_ap(cfg, wlan_cfg, index = 1)
        ...
        test_cfgs.extend(_define_station_test_cfg(cfg, test_case_name, open_wlan_cfg,wlan_cfg, sta_tag, browser_tag, ap_tag, ip_version))
...
def _define_station_test_cfg(cfg, test_case_name, open_wlan_cfg, wlan_cfg, sta_tag, browser_tag, ap_tag, ip_version):
    test_cfgs = []
    ...
    common_name = '%sAssociate the station with open none wlan' % (test_case_name,)
    params = {'sta_tag': sta_tag,
              'wlan_cfg': open_wlan_cfg,
              'start_browser': False,
    ...
    test_name = 'CB_ZD_Station_Download_ZeroIT'
    common_name = '%sDownload zero it in station' % (test_case_name,)
    test_cfgs.append(({'sta_tag': sta_tag,
                       'wlan_cfg': wlan_cfg}, test_name, common_name, 2, False))
    
    test_name = 'CB_Station_Associate_Get_IP_Verify_Subnet'
    common_name = '%sAssociate the station via zero-it and verify wifi ip in subnet' % (test_case_name,)
    params = {'sta_tag': sta_tag,
              'wlan_cfg': wlan_cfg,
...
def _define_expect_wlan_info_in_ap(tcfg, wlan_cfg, index):
    ...
    for radio in radio_mode_list:
        ...
        if radio in ['bg', 'ng']:            
            ...
            MAXIMUM_WLAN = 8
            wlan_name = "wlan%d" % (MAXIMUM_WLAN+index)
            expect_wlan_info[wlan_name] = {}
            expect_wlan_info[wlan_name]['status'] = status
            expect_wlan_info[wlan_name]['encryption_cfg'] = dict(ssid=wlan_cfg['ssid'])
...
def _define_open_wlan_cfg():
    return _get_wlan_cfg({})
...
def _define_wlan_cfg_list(username, password):
    ...
    wlan_cfgs.append(_get_wlan_cfg(dict(auth="EAP", wpa_ver="WPA2", encryption="AES", 
                                        username = username, password = password)))    
    #wlan_cfgs.append(_get_wlan_cfg(dict(auth="EAP", wpa_ver="WPA", encryption="TKIP",
    #                                    username = username, password = password)))
...
def create_test_suite(**kwargs):
    ...
    open_wlan_cfg = _define_open_wlan_cfg()
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "WLAN Shared Dot1x Zero IT - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify shared and dot1x wlan with Zero-IT - 11%s" % (target_sta_radio), combotest = True)
