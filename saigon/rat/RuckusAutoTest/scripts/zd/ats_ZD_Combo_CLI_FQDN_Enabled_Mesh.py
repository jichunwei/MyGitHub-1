...
Verify AP joins expected ZD when enable FQDN in ZD CLI.
    ...
    Test Data:
        ...
        Primary IPV4 FQDN Secondary IPV6 ADDR Join Secondary
    ...
    How to:
        ...
        5) Create station.
        6) Create active ap for root and mesh ap.
        7) Create a wlan group and wlan in ZD2.
        8) Prepare for test:
              Disable FQDN in ZD1 and AP CLI 
              Remove all APs from ZD1 and ZD2
              Verify root and mesh AP joins ZD1.
        9) Configure FQDN in ZD CLI.
...
def define_test_cfg(cfg):
    ...
    if cfg.has_key('emesh_ap'):
        emesh_ap = cfg['emesh_ap']
    else:
        emesh_ap = ''
    ...
    #@author: yuyanan @since: 2014-10-20 @bug: zf-10361
    test_name = 'CB_ZD_Enable_Mesh'
    common_name = 'Enable mesh in ZD2'
    test_cfgs.append(({'zd_tag': zd2_gui_tag,
                       'mesh_ap_list':[],'ap_should_connect':False},test_name, common_name, 2, False))
    ...
    if emesh_ap:
        test_name = 'CB_ZD_Create_Active_AP'
        common_name = 'Create active AP for eMesh AP'
        test_cfgs.append(({'active_ap': emesh_ap,
                           'ap_tag': emesh_ap_tag}, test_name, common_name, 0, False))
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        common_name = '[%s]Disable all AP FQDN in AP CLI before test' % test_case_name
        ...
        if emesh_ap:
            ...
            common_name = '[%s]Verify FQDN in eMesh AP CLI' % test_case_name
            test_params = {'ap_tag': emesh_ap_tag,
                           'is_verify': True,
        ...
        test_cfgs.append(({'zd_tag': zd1_gui_tag,'wait_to_connect':False}, test_name, common_name, 2, False))
        ...
        if emesh_ap:
            test_name = 'CB_AP_CLI_Get_Verify_Primary_Secondary_ZD'
            common_name = '[%s]Verify setting in eMesh AP CLI after AP join %s' % (test_case_name, exp_zd_name)
            test_params = {'ap_tag': emesh_ap_tag,
                           'is_verify': True,
                           'primary_zd_ip': primary_zd_ip,
                           'secondary_zd_ip': secondary_zd_ip
        ...
        common_name = "[%s]Assign wlan group to radio '%s' of Mesh AP %s on %s" % (test_case_name, radio_mode, ap_for_wg, exp_zd_name)
        test_params = {'active_ap': ap_for_wg,
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag, tbcfg):
    ...
    test_cfg_list.append({'case_name': 'Primary IPV4 FQDN Secondary IPV6 ADDR Join Secondary',
...
def _select_ap(ap_sym_dict, ap_type, is_required = True):
...
def create_test_suite(**kwargs):
    ...
    if ts_cfg["interactive_mode"]:
        ...
        emesh_ap = _select_ap(ap_sym_dict, 'eMesh AP', False)
    
    if root_ap and mesh_ap:       
        if emesh_ap:
            emesh_ap_cfg = ap_sym_dict[emesh_ap]
            emesh_ap_model = emesh_ap_cfg['model']
        ...
        if emesh_ap:
            tcfg.update({'emesh_ap': '%s' % emesh_ap,})
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                vlan = ' with AP Mgmt Vlan'
            else:
                vlan = ''
                
            if emesh_ap:
                meshap_desc = "eMesh AP"
            else:
                meshap_desc = "Mesh AP"
                
            ts_name = "ZD CLI - Enable FQDN for %s%s 11%s" % (meshap_desc, vlan, target_sta_radio)
            
        ts = testsuite.get_testsuite(ts_name, "Verify AP join specified ZD when enable FQDN for Mesh AP", combotest = True)
