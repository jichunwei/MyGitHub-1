...
def define_test_cfgs(tcfg):
    ...
    wlan_cfg['ssid'] = 'gp-range-%s' % time.strftime("%H%M%S")
    
    grace_period = 1
    test_name = 'CB_ZD_Test_Grace_Period_Setting'
    common_name = "%sMinimum grace period '%s' can be set" % (tcid, grace_period)
    test_params = {'wlan_cfg': wlan_cfg, 
    ...
    grace_period = 144000
    test_name = 'CB_ZD_Test_Grace_Period_Setting'
    common_name = "%sMaximum grace period '%s' can be set" % (tcid, grace_period)
    test_params = {'wlan_cfg': wlan_cfg, 
                   'grace_period': grace_period}
    test_cfgs.append((test_params, test_name, common_name, 1, False))
    
    test_name = 'CB_ZD_Test_Grace_Period_Setting'
    common_name = "%sRandom grace period from '%s' to '%s' can be set"
    ...
    grace_period = ''
    test_name = 'CB_ZD_Test_Grace_Period_Setting'
    common_name = "%sEmpty grace period cannot be set" % tcid
    test_params = {'wlan_cfg': wlan_cfg, 
                   'grace_period': grace_period}
    test_cfgs.append((test_params, test_name, common_name, 1, False))
    ...
    grace_period = 0
    test_name = 'CB_ZD_Test_Grace_Period_Setting'
    common_name = "%sGrace period '%s' cannot be set" % (tcid, grace_period)
    test_params = {'wlan_cfg': wlan_cfg, 
                   'grace_period': grace_period}
    test_cfgs.append((test_params, test_name, common_name, 1, False))
    ...
    tcid = '[Minimum grace period can work]'
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-mini-%s' % time.strftime("%H%M%S")
    guest_pass_cfg = deepcopy(tcfg['guest_pass_cfg'])
    guest_pass_cfg['wlan'] = wlan_cfg['ssid']
    guest_pass_cfg['guest_fullname'] = 'guest-mini'
    test_params = {
        'tcid': tcid,
        'sta_tag': sta_tag,
        'ap_tag': ap_tag,
    ...
    tcid = '[Maximum grace period can work]'
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-max-%s' % time.strftime("%H%M%S")
    guest_pass_cfg = deepcopy(tcfg['guest_pass_cfg'])
    guest_pass_cfg['wlan'] = wlan_cfg['ssid']
    guest_pass_cfg['guest_fullname'] = 'guest-max'
    test_params = {
        'tcid': tcid,
        'sta_tag': sta_tag,
        'ap_tag': ap_tag,
    ...
    tcid = '[Random active grace period can work]'
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-random-%s' % time.strftime("%H%M%S")
    guest_pass_cfg = deepcopy(tcfg['guest_pass_cfg'])
    guest_pass_cfg['wlan'] = wlan_cfg['ssid']
    guest_pass_cfg['guest_fullname'] = 'guest-random'
    test_params = {
        'tcid': tcid,
        'sta_tag': sta_tag,
        'ap_tag': ap_tag,
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Guest Access - Grace Period - Basic - %s - %s" \
