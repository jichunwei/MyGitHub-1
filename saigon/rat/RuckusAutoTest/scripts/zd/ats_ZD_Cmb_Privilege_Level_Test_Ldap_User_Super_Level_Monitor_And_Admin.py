...
def define_test_configuration(tcfg):
    ...
    common_name = 'create monitor user LDAP server via WebUI'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="super level Ldap User privilege level test-Monitor and Admin"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="super level Ldap User privilege level test-Monitor and Admin"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "super level Ldap User privilege level test-Monitor and Admin", interactive_mode = attrs["interactive_mode"], combotest=True)
