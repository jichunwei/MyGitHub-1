...
def define_test_configuration(tcfg):
    ...
    common_name = '%slogin zd web by Radius monitor user' % test_case_name
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Monitor level Radius User privilege level test-Download File"
    )
    attrs.update(kwargs)
...
#    
    ...
    else: ts_name ="Monitor level Radius User privilege level test-Download File"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Monitor level Radius User privilege level test-Download File", interactive_mode = attrs["interactive_mode"], combotest=True)
