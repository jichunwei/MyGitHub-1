...
def define_wlan_cfg(tcfg):
    ...
    #_wlan_cfgs['mac-wpa-mixed-tkip'] = (dict(name = 'mac-wpa-mixed-tkip',
    #                       auth = "mac", encryption = "wpa-mixed", algorithm = 'TKIP',
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       auth_server = auth_server))
    
    _wlan_cfgs['mac-wpa-mixed-aes'] = (dict(name = 'mac-wpa-mixed-aes',
                           auth = "mac", encryption = "wpa-mixed", algorithm = 'AES',
                           passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                           auth_server = auth_server))
    
    _wlan_cfgs['mac-wpa-mixed-auto'] = (dict(name = 'mac-wpa-mixed-auto',
                           auth = "mac", encryption = "wpa-mixed", algorithm = 'auto',
                           passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                           auth_server = auth_server))
...
def _define_data_plane_test_cfg(cfg, test_case_name, target_ip_addr, wlan_cfg, sta_tag, browser_tag, ap_tag, is_wpa_auto=False):
    ...
    test_cfgs.append(({'sta_tag': sta_tag,
                       ...
                       #'username': username},#Remove it for username is not needed
                       },
                       test_name, common_name, 2, False))
    ...
    #@author: Liang Aihua,@since: 2015-3-12,@change: hide this step for tb72 bad environment.
...
def createTestSuite(**kwargs):
    ...
    if active_ap :
        ...
        if attrs["testsuite_name"]:
            ...
            ts_name = "ZDCLI- Encryption Types-Mac-WPA-Mixed - 11%s" % target_sta_radio
            #ts_name = "ZDCLI- Encryption Types-Mac-WPA-Mixed - 11%s %s" % (target_sta_radio, ap_model)
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(mac wpa mixed) properly - 11%s radio" % target_sta_radio,
