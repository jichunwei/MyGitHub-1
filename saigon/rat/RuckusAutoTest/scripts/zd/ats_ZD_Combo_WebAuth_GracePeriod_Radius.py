...
def grace_period_test(cfg):
    ...
    #@author: Jane.Guo @since: 2013-11 ZF-5935
    #To verify ZD doesn't use radius grace-period attribute for web auth/guest wlan grace period, ZD only can use the grace period configured on ZD side
...
def define_test_cfgs(tcfg):
    ...
    wlan_cfg['ssid'] = 'gp-en-svr-%s' % time.strftime("%H%M%S")
    ...
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-dis-svr-%s' % time.strftime("%H%M%S")
    test_params = {'tcid': tcid,
                   'sta_tag': sta_tag,
                   'ap_tag': ap_tag,
                   'wlan_cfg': wlan_cfg,
    ...
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-svr-del-%s' % time.strftime("%H%M%S")
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Web Auth - Grace Period - Radius - %s" 
