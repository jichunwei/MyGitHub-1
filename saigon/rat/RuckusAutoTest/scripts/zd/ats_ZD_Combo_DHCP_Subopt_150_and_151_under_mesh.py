...
Verify AP can insert DHCP option82 subopt into DHCP packages.
    ...
    How to:
        ...
        2) Enable Mesh ENV
        3) Disable all APs' WLAN service
        4) Disable SW DHCP relay
        5) Enable ZD DHCP relay
        6) Create AP instance and enable its WLAN service
        7) Create station.
        8) Create WLAN and enable subopt randomly
        9) Start Tshark on DHCP server and station
        10) Associate station to WLAN, and authenticate if need
        11) Verify station connection
        12) Stop Tshark on DHCP server and station
        13) Verify DHCP package on DHCP server and station
        14) Edit WLAN to disable subopt
        15) jump to step 8 to 13
        16) Enable DHCP relay on SW
        17) Disable DHCP relay on ZD
        18) Enable all APs' WLAN service
        19) ZD set factory
...
def define_test_cfg(cfg):
    test_cfgs = []
     #@author:Tanshixiong #@bug:ZF-12436 #@since: 2015-04-08
    ...
    common_name = 'Remove all configurations of ZD'
    test_cfgs.append(({}, test_name, common_name, 0, False))
    
    #@auth:yuyanan #@bug:10060 #@since:2014-11-3 
    test_name = 'CB_ZD_Set_Country_Code'
    common_name = 'Set country code to US'
    test_cfgs.append(({'country_code':'United States', 'unfix_ap': False, 'allow_indoor_channel':True}, test_name, common_name, 0, False))
    
    test_name = 'CB_ZD_Enable_Mesh'

    #@author:Tanshixiong #@bug:ZF-12436 #@since: 2015-04-08
    common_name = 'Enable mesh in ZD and disable switch port connectet to ap %s,let it become mesh ap'% mesh_ap_tag
    test_cfgs.append(({'mesh_ip_list':[mesh_ap_tag], 'for_upgrade_test':False}, test_name, common_name, 0, False))
    ...
    #@author:Tanshixiong #@bug:ZF-12436 #@since: 2015-04-08 
    for active_ap in cfg.get('active_ap_list'):
        ap_sym_name = active_ap.keys()[0]
        
        test_name = 'CB_ZD_Create_Active_AP'

        common_name = 'Create active AP %s' % (ap_sym_name)
        test_cfgs.append(({'active_ap': ap_sym_name, 'ap_tag': ap_sym_name}, test_name, common_name, 0, False))
    ...
    tc_common_name = 'Verify DHCP Option82 subopt under mesh combine with'
    
    enable_wlan_service_ap_tag = cfg['mesh_ap_tag']
    disable_wlan_service_ap_tag = cfg['root_ap_tag']
    round = 0
    for tc_name in wlan_cfgs.keys():
        ...
        tmp = enable_wlan_service_ap_tag
        enable_wlan_service_ap_tag = disable_wlan_service_ap_tag
        disable_wlan_service_ap_tag = tmp
        test_name = 'CB_ZD_CLI_Configure_AP'
        step += 1
        common_name = '[%s %s]%s.%s Enable AP WLAN service in ZD CLI' % (tc_common_name, tc_name, round, step)
        test_cfgs.append(({'ap_cfg': {'radio_a':{'wlan_service':'No'},  'radio_ng':{'wlan_service':'Yes'}, 'mac_addr':''},
                           'ap_tag': enable_wlan_service_ap_tag}, test_name, common_name, 1, False))
        
        test_name = 'CB_ZD_CLI_Configure_AP'
        step += 1
        common_name = '[%s %s]%s.%s Disable AP WLAN service in ZD CLI' % (tc_common_name, tc_name, round, step)
        test_cfgs.append(({'ap_cfg': {'radio_a':{'wlan_service':'No'},  'radio_ng':{'wlan_service':'No'}, 'mac_addr':''},
                           'ap_tag': disable_wlan_service_ap_tag}, test_name, common_name, 1, False))
        ...
        #@author: yuyanan @since: 2015-2-3 @bug: zf-11882 @change:autonomous wlan can not enable tunnel
        ...
        test_cfgs.append(({'ssid':wlan_cfg.get('ssid'),'ap_tag':enable_wlan_service_ap_tag}, test_name, common_name, 2, False))
        ...
        test_cfgs.append(({'sta_tag': 'sta1',
                           'ap_tag': enable_wlan_service_ap_tag,
                           'wlan_cfg': wlan_cfg,
                           'status': 'Authorized',
                           }, 
                          test_name, common_name, 2, False))
...
def create_test_suite(**kwargs):
    ...
    access_vlan = raw_input("Please input access vlan for wlan[default is None]")
    if not access_vlan:
        access_vlan = None 
    ...
    if root_ap and mesh_ap:
        mesh_ap_cfg = ap_sym_dict[mesh_ap]        
        mesh_ap_mac = mesh_ap_cfg['mac']
#        mesh_ap_model = mesh_ap_cfg['model']        
        
        root_ap_cfg = ap_sym_dict[root_ap]        
        root_ap_mac = root_ap_cfg['mac']
#        root_ap_model = root_ap_cfg['model']
    ...
    if active_ap_list != []:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if access_vlan:
                ts_name = "ZD CLI - Verify DHCP Option82 subopt under mesh with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Verify DHCP Option82 subopt under mesh 11%s" % (target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify DHCP Option82 subopt combine with different wlan type under mesh", combotest = True)
