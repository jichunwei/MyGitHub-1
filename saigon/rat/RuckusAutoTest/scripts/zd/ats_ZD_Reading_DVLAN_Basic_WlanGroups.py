...
def tcid(id, radio_id, description, ap_model, ap_type_role, ap_tag):
    ...
    return "TCID:34.01.%s.%02d.%02d.%02d - %s" % (id, radio_id , aptcid, aprtid, xdescription)
...
def getTestCfg(ssid, wlan_cfg, test_case):
    TestCfg={}
    TestCfg['wlan_cfg'] = {'auth': 'EAP',
                           ...
                           'sta_encryption': '',
                           'sta_wpa_ver': '',
                           'wpa_ver': 'WPA2'}#Chico, 2015-5-21, optimize to remove WPA
    ...
    TestCfg['test_case'] = test_case
    TestCfg['wlan_cfg'].update(wlan_cfg)
    return TestCfg 
...
def defineTestCfgByAPModel(ssid, radio_mode, ap_model, ap_type_role, ap_tag):
    ...
    test_cfg_list = \
    [  (  getTestCfg(  ssid, dict(
                       wpa_ver="WPA2",
                       encryption="Auto",#Chico, 2015-5-21, optimize to remove WPA
                       sta_wpa_ver="WPA",
                       sta_encryption="TKIP"), "same wlan"),
          tcid("02.01", radio, "Clients have different Dynamic VLAN ID - Same WLAN with WPA2 Auto", ap_model, ap_type_role, ap_tag)),
       (  getTestCfg(  ssid, dict(
                       wpa_ver="WPA2",
                       encryption="AES",
                       sta_wpa_ver="WPA2",
                       sta_encryption="AES"), "same wlan"),
          tcid("02.02", radio, "Clients have different Dynamic VLAN ID - Same WLAN with WPA2 AES", ap_model, ap_type_role, ap_tag)),
       (  getTestCfg(  ssid, dict(
                       wpa_ver="WPA2",
                       encryption="Auto",#Chico, 2015-5-21, optimize to remove WPA
                       sta_wpa_ver="WPA",
                       sta_encryption="TKIP"), "diff wlan"),
          tcid("03.01", radio, "Clients have different Dynamic VLAN ID - different WLAN with WPA2 Auto", ap_model, ap_type_role, ap_tag)),
       (  getTestCfg(  ssid, dict(
                       wpa_ver="WPA2",
                       encryption="AES",
                       sta_wpa_ver="WPA2",
                       sta_encryption="AES"), "diff wlan"),
          tcid("03.02", radio, "Clients have different Dynamic VLAN ID - different WLAN with WPA2 AES", ap_model, ap_type_role, ap_tag))
    ]
...
def createTestSuite(**kwargs):
    ...
    ts = testsuite.get_testsuite( "Mesh - Dynamic VLAN - Basic - WlanGroups"
       , "Verify dynamic function in mesh environment.")
