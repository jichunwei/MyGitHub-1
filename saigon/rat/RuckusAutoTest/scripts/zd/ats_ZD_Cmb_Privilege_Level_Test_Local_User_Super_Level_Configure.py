...
def define_test_configuration(tcfg):
    ...
    common_name = 'create one super user'
    ...
    common_name = 'Select authentication server to Local Database'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Super level Local User privilege level test-Configure"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="Super level Local User privilege level test-Configure"
...
#                guest_gen_para=guest_gen_para,
                ...
                ap_tag = active_ap,#@author: yuyanan @since: 2014-8-28 optimize:ap tag replace ap mac
                )
    
    test_cfgs = define_test_configuration(tcfg)
    check_max_length(test_cfgs)
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Super level Local User privilege level test-Configure", interactive_mode = attrs["interactive_mode"], combotest=True)
