"""
Verify AP joins should not go back to ordinary discover when enable FQDN and both primary and secondary is unreachable.

    Verify AP didn't join any ZD when primary and seondary are gone
    Pre-condition:
       AP joins ZD1
    ...
    Test Data:
        Pri ZD2 IPV4 FQDN Sec Invalid Domain Name
        Pri ZD2 IPV6 FQDN Sec Invalid Domain Name
    ...
    How to:
        ...
        8) Prepare for test:
              ...
              Verify AP joins ZD1.
        9) Configure FQDN in ZD CLI, one is zd2 and another is invalid.
        ...
        14) Shutdown ZD2 switch port.
        15) Verify AP didn't join ZD1 and ZD2.
...
def define_test_cfg(cfg):
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        #Shutdown port on ZD2.
        test_name = 'CB_ZD_Disable_Enable_ZD_Switch_Port'
        common_name = '[%s]Disable switch port on ZD2' % test_case_name
        test_params = {'enable': False, 'zd_tag': zd2_gui_tag}
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        ...
        test_name = 'CB_ZD_Disable_Enable_ZD_Switch_Port'
        common_name = '[%s]Enable switch port on ZD2' % test_case_name
        test_params = {'enable': True, 'zd_tag': zd2_gui_tag}
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        
    test_name = 'CB_ZD_Disable_Enable_ZD_Switch_Port'
    common_name = 'Enable switch port on ZD2 after test'
    test_params = {'enable': True, 'zd_tag': zd2_gui_tag}
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Pri ZD2 IPV4 FQDN Sec Invalid Domain Name',
                          'primary_zd_ip': zd2_ipv4_dn,
                          'secondary_zd_ip': zd_ipv6_dn_unreachable,})
    
    test_cfg_list.append({'case_name': 'Pri ZD2 IPV6 FQDN Sec Invalid Domain Name',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - FQDN Stick Primary Secondary ZD Only with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - FQDN Stick Primary Secondary ZD Only 11%s" % (target_sta_radio)
    
        ts = testsuite.get_testsuite(ts_name, "Verify AP only try to connect to primary secondary ZD when enable FQDN", combotest = True)
