"""
Verify AP joins expected ZD when enable FQDN in ZD GUI.
    ...
    Test Data:
        Primary IPV4 FQDN Secondary IPV4 FQDN AP Join Primary
        Primary IPV4 FQDN Max Length Secondary Empty Join Primary
        Primary IPV6 FQDN Secondary IPV6 FQDN AP Join Primary
        Primary IPV6 ADDR Secondary IPV4 Join Primary
...
def define_test_cfg(cfg):
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        common_name = '[%s]Configure FQDN in ZD1 via GUI' % test_case_name
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag, tbcfg):
    ...
    #    1    FQDN IPV4    FQDN IPV4 Unreachable    Join Primary
    test_cfg_list.append({'case_name': 'Primary IPV4 FQDN Secondary IPV4 FQDN AP Join Primary',
    ...
    #    3    FQDN IPV6    FQDN IPV6 Unreachable    Join Primary
    test_cfg_list.append({'case_name': 'Primary IPV6 FQDN Secondary IPV6 FQDN AP Join Primary',
                          'primary_zd_ip': zd2_ipv6_dn,
                          'secondary_zd_ip': zd_ipv6_dn_unreachable,
                          'expected_zd_tag': zd2_gui_ipv6_tag })

    #    7    IPV6    IPV4 Unreachable    Join Primary
    test_cfg_list.append({'case_name': 'Primary IPV6 ADDR Secondary IPV4 Join Primary',
    ...
    #9    FQDN IPV4    IPV4    Join Secondary
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD GUI - Enable FQDN with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD GUI - Enable FQDN 11%s" % (target_sta_radio)
