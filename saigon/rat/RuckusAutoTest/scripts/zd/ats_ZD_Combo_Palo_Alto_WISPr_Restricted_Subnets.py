...
class TestStepBuilder:
    ...
    @classmethod
    def build_clean_wlan_wlangroup_steps(cls, EXEC_LEVEL = 0, Clean_up = False):
    ...
    @classmethod
    def build_clean_hotspot_step(cls, EXEC_LEVEL = 0, Clean_up = False):
        return [({}, 
...
def define_wlans():
    wlan_cfg_list = []
    # Open-None
    wlan_cfg_list.append(dict(ssid = "RAT-Open-None", auth = "open", encryption = "none", web_auth = None, type='hotspot' ))
...
def define_test_params(target_station):
    ...
    cfg['restricted_subnet_list'] = ["172.21.0.252/32", "172.22.0.252/32", 
...
def build_test_cases(target_station, target_station_radio, active_ap):
    ...
    #common_name = 'Create Hotspot service'
    hotspot_service = copy.deepcopy(test_params['hotspot_cfg'])
    hotspot_service['restricted_subnet_list'] = test_params['restricted_subnet_list'][:5]
    ...
    #How to transfor test_name in following functions? Liang Aihua
    test_cfgs.extend(TestStepBuilder._encode_tcid(tcid, TestStepBuilder.build_station_pre_steps(**param_dict)))
    test_cfgs.extend(TestStepBuilder._encode_tcid(tcid, TestStepBuilder.build_station_check_steps(TestStepBuilder.HOTSPOT, **param_dict)))
    test_cfgs.extend(TestStepBuilder._encode_tcid(tcid, TestStepBuilder.build_station_ping_step(**param_dict)))
        
    param_cfg = dict(target_ip_list = test_params['restricted_subnet_list'][:5],
                     sta_tag = 'sta_1') 
    test_cfgs.append((param_cfg, 
                      ...
                      '%sPing Targets and check if are not allowed' %tcid, 
                       EXEC_LEVEL+2, 
                       False))
    test_cfgs.extend(TestStepBuilder._encode_tcid(tcid, TestStepBuilder.build_station_post_steps(**param_dict)))
        
    
    
    #Test Maximum restricted subnet entries
    ...
    test_cfgs.extend(TestStepBuilder._encode_tcid(tcid, TestStepBuilder.build_station_check_steps(TestStepBuilder.HOTSPOT, **param_dict)))
    test_cfgs.extend(TestStepBuilder._encode_tcid(tcid, TestStepBuilder.build_station_ping_step(**param_dict)))
    param_cfg = dict(target_ip_list = test_params['restricted_subnet_list'],
                     sta_tag = 'sta_1') 
    test_cfgs.append((param_cfg, 
                      'CB_Station_Ping_Targets', 
                      '%sPing Targets and check if are not allowed' %tcid, 
...
def create_test_suite(**kwargs):
    ...
    attrs = dict(interactive_mode = True,
                 station = (0,"g"),
                 targetap = False,
                 testsuite_name = "Palo Alto WISPr Restricted Subnets - 11g",
                 )
    attrs.update(kwargs)
    ...
    if attrs["interactive_mode"]:
        ts_name = "Palo Alto WISPr Restricted Subnets - 11%s" %target_sta_radio    
        #ts_name = "Palo Alto WISPr Restricted Subnets - 11%s, %s" % (target_sta_radio, ap_model)    
    else:
        ts_name = attrs["testsuite_name"]

    ts = testsuite.get_testsuite(ts_name, 
                                 "Palo Alto WISPr Restricted Subnets - 11%s, %s" % (target_sta_radio, ap_model), 
                                 combotest=True)
