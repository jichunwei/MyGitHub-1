...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Operator level LDAP User privilege level test-Download File"
    )
    attrs.update(kwargs)
...
#   
    ...
    else: ts_name ="Operator level LDAP User privilege level test-Download Files"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Operator level LDAP User privilege level test-Download Files", interactive_mode = attrs["interactive_mode"], combotest=True)
