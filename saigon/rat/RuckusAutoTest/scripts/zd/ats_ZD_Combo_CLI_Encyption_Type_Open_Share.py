...
def define_wlan_cfg(tcfg):
    ...
    _wlan_cfgs['open-none'] = (dict(name = 'open-none',
                               description = utils.make_random_string(random.randint(1,64),type = 'alpha'),
                               type = 'standard-usage'
                               ))
    
    _wlan_cfgs['open-wep-64'] = (dict(name = 'open-wep-64',
                                      auth = "open", encryption = "wep-64",
                                      key_index = random.randint(1,4) , key_string = utils.make_random_string(10, "hex"),
                                  ))
    
    _wlan_cfgs['open-wep-128'] = (dict(name = 'open-wep-128',
                                       auth = "open", encryption = "wep-128",
                                       key_index = random.randint(1,4) , key_string = utils.make_random_string(26, "hex"),
                                    ))
    
    #_wlan_cfgs['share-wep-64'] = (dict(name= 'share-wep-64',
    #                       auth = "shared", encryption = "wep-64",
    #                       key_string = utils.make_random_string(10, "hex"),
    #                       key_index = random.randint(1,4),
    #                       ))

    #_wlan_cfgs['share-wep-128'] = (dict(name = 'share-wep-128',
    #                       auth = "shared", encryption = "wep-128",
    #                       key_string = utils.make_random_string(26, "hex"),
    #                       key_index = random.randint(1,4),
    #                       ))

    #_wlan_cfgs['open-wpa-tkip'] = (dict(name = 'open-wpa-tkip',
    #                       auth = "open", encryption = "wpa", algorithm =  'TKIP',
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       ))
    
    #_wlan_cfgs['open-wpa-aes'] = (dict(name = 'open-wpa-aes',
    #                       auth = "open", encryption = "wpa", algorithm =  'AES', 
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       ))
    
    #_wlan_cfgs['open-wpa-auto'] = (dict(name = 'open-wpa-auto',
    #                       auth = "open", encryption = "wpa", algorithm = 'auto' ,
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       ))
    
    _wlan_cfgs['open-wpa2-aes'] = (dict(name = 'open-wpa2-aes',
                           auth = "open", encryption = "wpa2",algorithm = 'AES' ,
                           passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                           priority = 'low'
                           ))
    
    #_wlan_cfgs['open-wpa2-tkip'] = (dict(name = 'open-wpa2-tkip',
    #                       auth = "open", encryption = "wpa2",algorithm = 'TKIP',
    #                       passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
    #                       priority = 'high'
    #                       ))
    
    _wlan_cfgs['open-wpa2-auto'] = (dict(name = 'open-wpa2-auto',
                           auth = "open", encryption = "wpa2",algorithm = 'auto',
                           passphrase = utils.make_random_string(random.randint(8, 63), "hex"),
                           ))
...
def _define_data_plane_test_cfg(cfg, test_case_name, target_ip_addr, wlan_cfg, sta_tag, browser_tag, ap_tag, is_wpa_auto=False):
    ...
    #Remove it for username/password not need
    ...
    test_cfgs.append(({'sta_tag': sta_tag,
                       ...
                       #'username': username #username not need
                       },
                       test_name, common_name, 2, False))
...
def createTestSuite(**kwargs):
    ...
    if active_ap:
        ...
        if attrs["testsuite_name"]:
            ...
            ts_name = "ZDCLI- Encryption Types-Open and Share - 11%s" %target_sta_radio
            #ts_name = "ZDCLI- Encryption Types-Open and Share - 11%s %s" % (target_sta_radio, ap_model)
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(open and share) properly - 11%s radio" % target_sta_radio,
