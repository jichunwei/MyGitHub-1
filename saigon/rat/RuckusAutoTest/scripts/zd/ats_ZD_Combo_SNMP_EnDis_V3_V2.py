...
Config SNMP V2 and V3 agent via ZD CLI
    ...
    Enable SNMP v3 and v2 agent, then disable v2 and v3.
    1. Enable v3 agent, verify snmpd is started, snmp v3 command works well.
    2. Enable v2 agent, verify snmp v2 and v3 command works well.
    3. Disable v3 agent, verify snmp v3 command can't work, v2 command works well.
    4. Disable v2 agent, verify snmp v3 and v3 commands can't work, snmpd is shutdown [not in ps list].    
    expect result: All steps should result properly.
    
    How to:
        1) Enable snmp v3 agent via ZD CLI.
        2) Verify snmp v3 agent information are same between get and set.
        3) Verify snmpd process is in ps list.        
        4) Verify snmp v3 command works well.
        5) Enable snmp v2 agent via ZD CLI.
        6) Verify snmp v2 agent information are same between get and set.
        7) Verify snmp v3 and v2 command works well.
        8) Disable snmp v3 agent.
        7) Verify v3 agent information between get and set. 
        8) Verify snmp v3 command can't work.        
        9) Verify snmp v2 command works well.
        10) Disable snmp v2 agent.
        11) Verify snmp v2 command can't work.
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[Enable SNMP Agent V3 Then V2]'
    test_name = 'CB_ZD_Set_SNMP_Agent_V3_Info'
    ...
    common_name = '%sVerify SNMP V3 Command after Enable V2' % (test_case_name,)
    param_cfg = {'snmp_agent_cfg':tcfg['enable_snmp_agent_v3_cfg'],
                 'snmp_cfg': tcfg['snmp_cfg']}
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
    
    test_case_name = '[Disable SNMP Agent V3 Then V2]'
    ...
    test_name = 'CB_ZD_SNMP_Verify_SNMP_Commands'
    common_name = '%sVerify SNMP V2 Command after Disable V3' % (test_case_name,)
    param_cfg = {'snmp_agent_cfg':tcfg['enable_snmp_agent_v2_cfg'],
                 'snmp_cfg': tcfg['snmp_cfg']}
    test_cfgs.append((param_cfg, test_name, common_name, 2, False))
...
def create_test_suite(**kwargs):    
    ...
    ts_name = 'ZD CLI - SNMP Enable and Disable V3 Then V2'
    ts = testsuite.get_testsuite(ts_name, 'Verify Enable and Disable SNMP V3 Then V2', combotest=True)
