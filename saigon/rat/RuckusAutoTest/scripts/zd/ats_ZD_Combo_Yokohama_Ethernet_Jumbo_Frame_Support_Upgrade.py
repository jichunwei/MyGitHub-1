'''
Title: AP MTU configuration on ZD upgrade

Test purpose: Verify if AP MTU configuration can be kept after ZD upgrade and downgrade
...
4) Downgrade to the original build;
5) Check the MTU configuration on AP again.
...
#---TestCase_1-----------------------------------------------------------------------------------
    test_case_name = 'Verify AP MTU configuration on ZD upgrade'
    ...
    test_name = 'CB_ZD_Upgrade_Without_Check_AP_Status'
    common_name = '%sUpgrade ZD to target build' % test_combo_case_name 
    test_cfgs.append(({'build': 'target'}, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Check_Ap_Num_From_Web'
    common_name = '%sWait for AP reconnect after ZD upgrade' % test_combo_case_name 
    test_cfgs.append(({}, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Check_Version'
    common_name = '%sCheck ZD version after upgrade' % test_combo_case_name 
    test_cfgs.append(({'build': 'target'}, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Verify_AP_Eth_Mtu_Setting'
    common_name = '%sCheck AP eth port MTU after upgrade' % test_combo_case_name 
    test_params = {'ap_tag': ap1_tag, 
                   'eth_interface': ['eth0', 'eth1'], 
                   'retrieve_mtu': True,
                   'expect_status': True}
    ...
    test_name = 'CB_ZD_Upgrade_Without_Check_AP_Status'
    common_name = '%sDowngrade ZD to original build' % test_combo_case_name 
    test_cfgs.append(({'build': 'base'}, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Check_Ap_Num_From_Web'
    common_name = '%sWait for AP reconnect after ZD downgrade' % test_combo_case_name 
    test_cfgs.append(({}, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Check_Version'
    common_name = '%sCheck ZD version after downgrade' % test_combo_case_name 
    test_cfgs.append(({'build': 'base'}, test_name, common_name, 2, False))
    
    test_name = 'CB_ZD_Verify_AP_Eth_Mtu_Setting'
    common_name = '%sCheck AP eth port MTU after downgrade' % test_combo_case_name 
    test_params = {'ap_tag': ap1_tag, 
                   'eth_interface': ['eth0', 'eth1'], 
                   'retrieve_mtu': True,
                   'expect_status': True}
...
def create_test_suite(**kwargs):
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = "AP MTU configuration on ZD upgrade"

    ts = testsuite.get_testsuite(
        ts_name, "Verify if AP MTU can be kept after ZD upgrade and downgrade",
