...
def build_hotspot_cfg_list():
    return [{'login_page': 'http://[2020:db8:1::249]/vlogin.html',
             'name': 'Wispr_Walled_Garden_DNS',             
             'walled_garden_list':['www.ipv6-example.net'],
             'target_ip_list':['www.ipv6-example.net'],
             },
             {'login_page': 'http://[2020:db8:1::249]/vlogin.html',
              'name': 'Wispr_Walled_Garden_IP',              
              'walled_garden_list':['2020:db8:1::251'],
              'target_ip_list':['2020:db8:1::251'],                          
             },
             {'login_page': 'http://[2020:db8:1::249]/vlogin.html',
              'name': 'Wispr_Walled_Garden_Prefix_IP',              
              'walled_garden_list':['2020:db8:1::/64'],
              'target_ip_list':['2020:db8:1::251',
                                '2020:db8:1::252',
                                #'2020:db8:1::2'
                                ],                          
...
def build_wlan_cfg(**kwargs):    
    ...
    wlan_cfg = dict(ssid=ssid, auth="open", name="",
...
#        target_ip_list.append(target_ipv6_addr)
    ...
    test_name = 'CB_Station_Ping_Targets_IPV6'
    common_name = '%sVerify client can not ping target IPs before auth' % (test_case_name,)
    test_cfgs.append(({'sta_tag': sta_tag,
                       'allow': False,
                       'target_ip_list': ['2020:db8:50::251'],
                       'ping_timeout_ms':30 * 1000
                       }, test_name, common_name, 2, False))
...
def create_test_suite(**kwargs):
    ts_cfg = dict(interactive_mode = True,
                 station = (0, "g"),
                 targetap = False,
                 testsuite_name = "WISPr Walled Garden Testing with IP Address",
                 )
    ...
    if ts_cfg['interactive_mode']:
        ts_name = "WISPr Walled Garden Testing with IP Address - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)                        
    else:
    ...
    if active_ap:
        ...
        ts = testsuite.get_testsuite(ts_name, "WISPr Walled Garden Testing in IPV6", combotest = True)
