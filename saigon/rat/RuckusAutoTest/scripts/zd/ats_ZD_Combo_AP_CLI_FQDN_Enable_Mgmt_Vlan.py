...
def define_test_cfg(cfg):
    ...
    test_cfgs.append(({'auto_approval': True,
                       'zd_tag': zd1_gui_tag,
                       'mac_addr_list': [active_ap_mac],
                       'verify_ap_component': False}, test_name, common_name, 0, False))
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "AP CLI - Enable FQDN with Mgmt Vlan 11%s" % (target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify AP join specified ZD when enable FQDN and AP mgmt vlan", combotest = True)
