...
def defineTestConfiguration():
    ...
    common_name = 'Remove all the Configurations'
    test_cfgs.append(({},test_name, common_name,1,False))
    
    
    test_name = 'CB_ZD_Create_Wlan'
    common_name = '[Backup in SM-Policy]Create "OPEN-NONE" WLANs'
    ...
    common_name = '[Backup in SM-Policy]Create a Single Role on ZD named rat-role'
    test_cfgs.append(({'role_cfg':role_cfg},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Create_Local_User'
    common_name = '[Backup in SM-Policy]Create a Local user rat.local.user'
    ...
    common_name = '[Backup in SM-Policy]Backup System'
    test_cfgs.append(({'save_to':save_to},test_name, common_name,2,False))  
    
    test_name = 'CB_Scaling_RemoveZDAllConfig'
    common_name = '[Restore in SM-Policy]Remove all the Configurations'
    test_cfgs.append(({},test_name, common_name,1,False)) 
    
    test_name = 'CB_ZD_Restore'
    common_name = '[Restore in SM-Policy]Restore the backup file'
    test_cfgs.append(({'restore_type':'restore_basic_config'},test_name, common_name,1,False)) 
    
    
    test_name = 'CB_ZD_Verify_L3_ACLs_Info'
    common_name = '[Restore in SM-Policy]Verify L3 ACL has restored correctly'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_Local_User'
    common_name = '[Restore in SM-Policy]Verify users which are restored to original configuration'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_Wlan_Groups_Info'
    common_name = '[Restore in SM-Policy]Verify WLAN Group rat-restore-wlangroup exist'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_Wlans_Info'
    common_name = '[Restore in SM-Policy]Verify WLANs which are restored to original configuration'
    test_cfgs.append(({},test_name, common_name,2,False)) 
    
    test_name = 'CB_ZD_Verify_System_Name'
    common_name = '[Restore in SM-Policy]Verify System Name can not restore'
    ...
    for profile_cfg in gp_profile_list:
        ...
        common_name = "[Restore in SM-Policy]Verify grace period in profile: %s" % profile_cfg['name']
    ...
    for wlan_cfg in gp_wlan_cfg_list:
        test_name = 'CB_ZD_Verify_Grace_Period_In_GUI'
        common_name = "[Restore in SM-Policy]Verify grace period in WLAN: %s" % wlan_cfg['ssid']
...
def createTestSuite(**kwargs):
    ...
    ts_name = 'Backup and Restore by Policy mode under ZD smart redundancy'
    ts = testsuite.get_testsuite(ts_name, 'Verify ZD can backup restore by Policy mode under ZD smart redundancy ENV', combotest=True)
