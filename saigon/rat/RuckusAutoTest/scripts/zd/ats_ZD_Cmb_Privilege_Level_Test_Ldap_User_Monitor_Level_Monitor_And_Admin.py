...
def define_test_configuration(tcfg):
    ...
    common_name = 'create LDAP server via WebUI'
    ...
    common_name = '%slogin zd web by LDAP monitor user' % test_case_name
    ...
    common_name = 'remove ldap server %s ' % (tcfg['server_cfg']['server_name'])
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Monitor level LDAP User privilege level test-Monitor and Admin"
    )
    attrs.update(kwargs)
...
#    
    ...
    else: ts_name ="Monitor level LDAP User privilege level test-Monitor and Admin"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Monitor level LDAP User privilege level test-Monitor and Admin", interactive_mode = attrs["interactive_mode"], combotest=True)
