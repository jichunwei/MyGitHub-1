...
def define_wlan_cfg(ssid, isolation_option):
    wlan_cfg = dict(ssid = 'rat-client-isolation-01', type = 'standard-usage', 
                    auth = "open", algorithm = "", encryption = "none",
                    key_index = "" , key_string = "",
                    username = "", password = "", use_radius = False,
                    client_isolation = "local")
    
    wlan_cfg['name'] = wlan_cfg['ssid']
    wlan_cfg['client_isolation'] = isolation_option
...
def define_test_cfg(cfg):
    ...
    common_name = "Create a open-none wlan with %s via CLI" % isolation_option
...
def createTestSuite(**kwargs):
    ...
    if active_ap:
        for isolation_option in ['none','local','full']:
            ...
            ts_name = 'ZD CLI - Client Isolation %s Sta01-11%s Sta02-11%s ZD %s AP %s' % (isolation_option, 
                                                                                          target_sta_01_radio,
                                                                                          target_sta_02_radio,
                                                                                          zd_ip_version,
                                                                                          ap_ip_version,)
