...
def define_test_cfg(cfg):
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        test_name = 'CB_AP_CLI_Get_Verify_Primary_Secondary_ZD'
        common_name = '[%s]Verify FQDN in AP CLI after reboot' % test_case_name
        test_params = {'ap_tag': ap_tag,
                       'is_verify': True,
                       'primary_zd_ip': primary_zd_ip,
                       'secondary_zd_ip': secondary_zd_ip
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Invalid IPV4 FQDN and IPV6',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - Enable FQDN Invalid Domain Name or IP with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Enable FQDN Invalid Domain Name or IP 11%s" % (target_sta_radio)
