...
Verify AP joins expected ZD when enable FQDN in ZD CLI.
    ...
    Test Data:
       Two different domain name point to same IP.
        Primary IPV4 FQDN Secondary IPV4 FQDN Max Length
        Primary IPV4 FQDN Max Length Secondary IPV6 FQDN
        Primary IPV6 FQDN Secondary IPV6 FQDN
        Primary IPV6 FQDN Max Length Secondary IPV4 FQDN
...
def _define_zd_discovery_keep_ap_setting(tbcfg):
    zd_keep_ap_setting = {'enabled': True,
                          'keep_ap_setting': True
                          }
    import re
    for key in tbcfg.keys():
        if re.search("\Azd1\Z", key, re.IGNORECASE):
            zd_keep_ap_setting['primary_zd_ip'] = tbcfg.get(key).get('ip_addr')
        
        if re.search("\Azd2\Z", key, re.IGNORECASE):
            zd_keep_ap_setting['secondary_zd_ip'] = tbcfg.get(key).get('ip_addr')
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'Primary IPV4 FQDN Secondary IPV4 FQDN Max Length',
                          'primary_zd_ip': zd2_ipv4_dn,
                          'secondary_zd_ip': zd2_ipv4_dn_max_len,
                          'expected_zd_tag': zd2_gui_tag})
    
    test_cfg_list.append({'case_name': 'Primary IPV4 FQDN Max Length Secondary IPV6 FQDN',
                          'primary_zd_ip': zd2_ipv4_dn_max_len,
                          'secondary_zd_ip': zd2_ipv6_dn,
                          'expected_zd_tag': zd2_gui_tag})
    
    test_cfg_list.append({'case_name': 'Primary IPV6 FQDN Secondary IPV6 FQDN',
                          'primary_zd_ip': zd2_ipv6_dn,
                          'secondary_zd_ip': zd2_ipv6_dn_max_len,
                          'expected_zd_tag': zd2_gui_ipv6_tag})
    
    test_cfg_list.append({'case_name': 'Primary IPV6 FQDN Max Length Secondary IPV4 FQDN',
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        zd_discovery_cfg_keep_ap_setting = _define_zd_discovery_keep_ap_setting(tbcfg)
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - Enable FQDN Diff Domain Name to Same IP with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Enable FQDN Diff Domain Name to Same IP 11%s" % (target_sta_radio)
