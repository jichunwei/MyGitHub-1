...
def define_wlan_cfg(tcfg):
    ...
    _wlan_cfgs['dot1x-wpa-mixed-auto-local'] = (dict(name = 'dot1x-wpa-mixed-auto-local',
                           auth = "dot1x-eap", encryption = "wpa-mixed", algorithm = 'auto',
                           auth_server = 'local'
                           ))  
    
    #_wlan_cfgs['dot1x-wpa-mixed-tkip-local'] = (dict(name = 'dot1x-wpa-mixed-tkip-local',
    #                       auth = "dot1x-eap", encryption = "wpa-mixed", algorithm = 'TKIP', 
    #                       auth_server = 'local'
    #                       ))
    
    _wlan_cfgs['dot1x-wpa-mixed-aes-local'] = (dict(name = 'dot1x-wpa-mixed-aes-local',
                           auth = "dot1x-eap", encryption = "wpa-mixed", algorithm = 'AES', 
                           auth_server = 'local'
                           ))
    return _wlan_cfgs
...
def _define_data_plane_test_cfg(cfg, test_case_name, target_ip_addr, wlan_cfg, sta_tag, browser_tag, ap_tag, is_wpa_auto=False):
    ...
    #Remove this step for tn72 bad test environment
...
def createTestSuite(**kwargs):
    ...
    if active_ap :
        ...
        if attrs["testsuite_name"]:
            ...
            ts_name = "ZDCLI- Encryption Types-802.11x-WPA-Mixed-Local - 11%s" %target_sta_radio
            #ts_name = "ZDCLI- Encryption Types-802.11x-WPA-Mixed-Local - 11%s %s" % (target_sta_radio, ap_model)
        
        ts = testsuite.get_testsuite(ts_name,
                                     "Verify the ability of ZD to deploy WLANs with different encryption types(802.11x-WPA-Mixed-Local) properly - 11%s radio" % target_sta_radio,
