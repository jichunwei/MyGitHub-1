...
def define_test_cfgs(tcfg):
    ...
    tcid = '[Client reconnect after deleted by ZD]'
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-zd-delete-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-zd-delete-%s' % time.strftime("%H%M%S")
    ...
    tcid = '[Client reconnect to another AP]'
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-diff-ap-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-diff-ap-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    grace_period = 3
    ...
    hotspot_cfg = deepcopy(tcfg['hotspot_cfg'])
    hotspot_cfg['name'] = 'gp-roam-ap-%s' % time.strftime("%H%M%S")
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'wispr-roam-ap-%s' % time.strftime("%H%M%S")
    wlan_cfg['hotspot_profile'] = hotspot_cfg['name']
    grace_period = 3
...
def create_test_suite(**kwargs):
    ...
    if attrs["interactive_mode"]:
        ...
        active_ap_2 = choose_active_ap(ap_sym_dict, "Choose another AP: ")
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Hotspot - Grace Period - Client Reconnect - %s - %s" \
