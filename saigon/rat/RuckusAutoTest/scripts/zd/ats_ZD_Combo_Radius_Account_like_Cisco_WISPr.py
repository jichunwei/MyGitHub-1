"""
Verify Radius accounting procedure like Cisco with WLAN type WISPr
    ...
    How to:
        ...
        5) Create Hospot profile, enable Radius Accounting, and select Radius server as authentication database
        ...
        15) Modify NAS-ID type of the WLAN by ZDCLI command
        16) Repeat step 6)-13)
...
wlan_interim_update_session = '2'       #minute: Radius accounting interim update interval for session timeout test case
hotspot_session_timeout     = '3'       #minute: Radius accounting session timeout value in Hotspot profile
...
def _define_wlan_cfg_hotspot(hs_name='', rad_user='', do_tunnel=False, vlan_id=''):
    wlan_cfg = dict(ssid='ras-acct-cisco_wispr', auth="open", encryption="none")
...
def define_test_cfg(cfg):
    ...
    test_case_name = '[Radius Acct Hotspot WLAN, NAS-ID type %s]' % nas_id_type
    ...
    test_case_name = '[Radius Acct Hotspot WLAN, Session timeout]'
    new_hs_cfg = deepcopy(hs_cfg)
    new_hs_cfg['interim_update_interval'] = wlan_interim_update_session
    new_hs_cfg['session_timeout'] = hotspot_session_timeout
    rad_user     = radius_user['2min']
    nas_id_type  = 'user-define'
    user_def_str = 'Cisco-Ruckus'

    test_name = 'CB_ZD_Edit_Hotspot_Profiles'
    common_name = '%sEdit Hotspot profile with session timeout option' % (test_case_name)
    test_cfgs.append(({'hotspot_profiles_list':[new_hs_cfg]}, test_name, common_name, 1, False))
    ...
    test_cfgs.append(({'sta_tag': sta_tag,
                       ...
                       'acct_nas_id_type': nas_id_type,
                       'user_define_string': user_def_str,
                       'acct_session_time': int(wlan_interim_update_session)*60,
                       },
                       test_name, common_name, 2, False))
    ...
    test_cfgs.append(({'sta_tag': sta_tag,
                       ...
                       'acct_nas_id_type': nas_id_type,
                       'user_define_string': user_def_str,
                       'acct_session_time': (int(hotspot_session_timeout)-int(wlan_interim_update_session))*60,
                       },
                       test_name, common_name, 2, False))
...
def createTestSuite(**kwargs):
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = "Radius Accounting like Cisco - Hotspot"

    ts = testsuite.get_testsuite(ts_name, "Radius Accounting like Cisco - Hotspot" , combotest=True)
