...
Verify AP joins expected ZD when enable FQDN in ZD CLI.
    ...
    Test Data:
        APCLI Primary Secondary ZD1 Primary
        APCLI Primary Secondary ZD1 Primary Secondary
        APCLI Primary ZD1 Primary Secondary
        APCLI Primary ZD1 Primary
    ...
    How to:
        ...
        8) Prepare for test:
              ...
              Verify AP joins ZD1.
        9) Configure FQDN in AP CLI, one is zd1.
        10) Configure FQDN in ZD1.              
        11) Verify FQDN in AP CLI.
        12) Verify FQDN settings after AP join ZD2.
        13) Verify AP joins ZD2.
        14) Associate the station to wlan and verify ping traffic to server.
...
def define_test_cfg(cfg):
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        zd_fqdn_cfg = zd_discovery_test_cfg['zd_cfg']
        apcli_fqdn_cfg = zd_discovery_test_cfg['apcli_cfg']
        
        zd_primary_zd_ip = zd_fqdn_cfg['primary_zd_ip']
        zd_secondary_zd_ip = zd_fqdn_cfg['secondary_zd_ip']        
        ap_primary_zd_ip = apcli_fqdn_cfg['primary_zd_ip']
        ap_secondary_zd_ip = apcli_fqdn_cfg['secondary_zd_ip']
        ...
        test_name = 'CB_ZD_CLI_Set_Primary_Secondary_ZD'
        common_name = '[%s]Configure FQDN as keep AP setting in ZD1 via CLI' % test_case_name
        ...
        test_params = {'ap_tag': ap_tag,
                       'primary_zd_ip': ap_primary_zd_ip,
                       'secondary_zd_ip': ap_secondary_zd_ip
                       }
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        
        test_name = 'CB_AP_CLI_Get_Verify_Primary_Secondary_ZD'
        common_name = '[%s]Verify FQDN in AP CLI When ZD is Keep AP Setting' % test_case_name
        test_params = {'ap_tag': ap_tag,
                       'is_verify': True,
                       'primary_zd_ip': ap_primary_zd_ip,
                       'secondary_zd_ip': ap_secondary_zd_ip,
        ...
        test_params = {'zd_tag': zd1_cli_tag}
        test_params.update(zd_fqdn_cfg)
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        ...
        test_params = {'ap_tag': ap_tag,
                       'is_verify': True,
                       'primary_zd_ip': zd_primary_zd_ip,
                       'secondary_zd_ip': zd_secondary_zd_ip
                       }
        test_cfgs.append((test_params, test_name, common_name, 2, False))
...
def _define_zd_discovery_test_cfg(zd1_gui_tag, zd2_gui_tag, zd1_cli_tag, zd2_cli_tag, zd1_gui_ipv6_tag, zd2_gui_ipv6_tag):
    ...
    test_cfg_list.append({'case_name': 'APCLI Primary Secondary ZD1 Primary',
                          'apcli_cfg': {'primary_zd_ip': zd1_ipv4_dn,
                                        'secondary_zd_ip': zd_ipv6_dn_unreachable,},
                          'zd_cfg': {'primary_zd_ip': zd2_ipv6_dn},
                          'expected_zd_tag': zd2_gui_ipv6_tag,
                          })
    
    test_cfg_list.append({'case_name': 'APCLI Primary Secondary ZD1 Primary Secondary',
                          'apcli_cfg': {'primary_zd_ip': zd1_ipv6_dn,
                                        'secondary_zd_ip': zd_ipv4_dn_unreachable,},
                          'zd_cfg': {'primary_zd_ip': zd2_ipv4_dn,
                                     'secondary_zd_ip': zd_ipv6_dn_unreachable},
                          'expected_zd_tag': zd2_gui_tag,
                          })
    
    test_cfg_list.append({'case_name': 'APCLI Primary ZD1 Primary Secondary',
                          'apcli_cfg': {'primary_zd_ip': zd1_ipv4_dn},
                          'zd_cfg': {'primary_zd_ip': zd2_ipv6_dn,
                                     'secondary_zd_ip': zd_ipv4_dn_unreachable},
                          'expected_zd_tag': zd2_gui_ipv6_tag,
    ...
    test_cfg_list.append({'case_name': 'APCLI Primary ZD1 Primary',
                          'apcli_cfg': {'primary_zd_ip': zd1_ipv6_dn},
                          'zd_cfg': {'primary_zd_ip': zd2_ipv4_dn},
                          'expected_zd_tag': zd2_gui_tag,
                          })
    ...
    for cfg in test_cfg_list:
        new_zd_cfg = copy.deepcopy(default_cfg)
        new_zd_cfg.update(cfg['zd_cfg'])
        
        new_apcli_cfg = copy.deepcopy(default_cfg)
        new_apcli_cfg.update(cfg['apcli_cfg'])
        
        zd_test_cfg = {}
        zd_test_cfg['case_name'] = cfg['case_name']
        zd_test_cfg['expected_zd_tag'] = cfg['expected_zd_tag']
        zd_test_cfg['zd_cfg'] = new_zd_cfg
        zd_test_cfg['apcli_cfg'] = new_apcli_cfg
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD CLI - Enable FQDN on ZD and APCLI with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Enable FQDN on ZD and APCLI 11%s" % (target_sta_radio)
    
        ts = testsuite.get_testsuite(ts_name, "Verify ZD FQDN will overwrite APCLI settings", combotest = True)
