...
def define_test_configuration(cfg):
    ...
    common_name = 'Initial test environment of test, call 2 ZD up at target build'
    ...
    common_name = 'Enable Smart Redundancy at target build'
    ...
    common_name = 'Remove Roles, except default at target build'
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))

    test_name = 'CB_ZD_Remove_All_Wlans'
    common_name = 'Remove WLANs from ZD WebUI at target build'
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))
    
    pre_name='[create Wlan at the begining in target build]'  
    test_name = 'CB_ZD_Create_Wlan'
    common_name = '%sCreate open-none WLAN at target build' % pre_name
    wlan_cfg_list = []    
    open_none_wlan = {
                 'ssid': 'rat-open-none',                
                 'auth': 'open',
    ...
    pre_name='[create role at the begining in target build]'  
    test_name = 'CB_ZD_Create_Single_Role'
    common_name = '%sCreate a role at target build' % pre_name
    ...
    pre_name='[create user at the begining in target build]'  
    test_name = 'CB_ZD_Create_Local_User'
    common_name = "%sCreate local username/password as \'rat_local_user/rat_local_user\' at target build" % pre_name
    param_cfg = dict(username = 'rat_local_user', password = 'rat_local_user')
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))   

    pre_name='[create guest pass wlan at the begining in target build]' 
    test_name = 'CB_ZD_Create_Wlan'
    common_name = '%sCreate rat-guest-access WLAN at target build' % pre_name
    wlan_cfg_list = []    
    guest_access_wlan = {
    ...
    pre_name='[associate station at the begining in target build]' 
    test_name = 'CB_ZD_Find_Station'
    common_name = '%sFind an active station at target build' % pre_name
    param_cfg = dict(target_station = cfg['target_sta'])
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))
    
    
    test_name = 'CB_ZD_Remove_Wlan_From_Station'
    common_name = '%sRemove WLANs from station for guest access testing at target build' % pre_name
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))
    
    
    test_name = 'CB_ZD_Associate_Station'
    common_name = '%sAssociate station to ssid rat-guest-access at target build' % pre_name
    param_cfg = dict(wlan_cfg = guest_access_wlan)
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))
    
    
    pre_name='[add aaa server at the begining in target build]' 
    test_name = 'CB_Scaling_Remove_AAA_Servers'
    common_name = '%sRemove all Authentication for creating new servers at target build' % pre_name
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))        
    
    
    test_name = 'CB_ZD_Create_Authentication_Server'
    common_name = '%sCreate a LDAP/Radius/Accounting server rat_ldap_test at target build' % pre_name
    param_cfg = dict(auth_ser_cfg_list = [cfg['ldap_ser_cfg'], cfg['radius_ser_cfg'], cfg['ad_ser_cfg']])
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))  
    
    #disable SR,to downgrade separatly,related to bug23677
    
    pre_name='[disable SR]'
    test_name = 'CB_ZD_SR_Disable'
    common_name = '%sDisable Smart Redundancy' % pre_name
    test_cfgs.append(({},test_name, common_name, 0, False))  
    
    pre_name='[downgrade zd separately to base build]'
    test_name = 'CB_Scaling_Upgrade_ZD'
    common_name ='%szd1 downgrade zd to base build' % pre_name
    test_cfgs.append(({'image_file_path': cfg['base_img_file_path'],'default':True,'zd':'zd1','zdcli':'zdcli1'},
                      test_name, common_name, 0, False))
    
    
    test_name = 'CB_Scaling_Upgrade_ZD'
    common_name ='%szd2 downgrade zd to base build' % pre_name
    test_cfgs.append(({'image_file_path': cfg['base_img_file_path'],'default':True,'zd':'zd2','zdcli':'zdcli2'},
                      test_name, common_name, 0, False))
    ...
    common_name = '%sCheck the active ZD version, make sure the active ZD was downgrade' % pre_name
    test_cfgs.append(({'expect_version':cfg['base_full_version'],'zd_type':'active'},test_name,common_name,1,False))
    
    test_name = 'CB_ZD_SR_Check_Version'
    common_name = '%sCheck the Standby ZD version, make sure the Standby ZD was downgrade' % pre_name
    test_cfgs.append(({'expect_version':cfg['base_full_version']},test_name,common_name,1,False))
    
    
    pre_name='[SR enable at base build]'
    test_name = 'CB_ZD_SR_Enable'
    common_name = '%sEnable Smart Redundancy at base build' % pre_name
    test_cfgs.append(({"timeout":1000},test_name,common_name,0,False))
    ...
    pre_name='[check aps are all connected at base build]'
    test_name = 'CB_Scaling_Waiting'
    common_name = '%s:Waiting %d minutes for ap downgrade' % (pre_name,cfg['wait_for_ap_upgrading'])
    test_cfgs.append(({'timeout':cfg['wait_for_ap_upgrading']*60}, test_name, common_name, 1, False))
    
    test_name = 'CB_ZD_SR_Get_Active_ZD'
    common_name = "%sGet the Active ZD at base build" % pre_name
    test_cfgs.append(({},test_name,common_name,0,False))
    ...
    pre_name='[verify empty cfg in active zd]'
    test_name = 'CB_Scaling_Verify_Empty_CFG'
    common_name = '%sCheck WLAN, User, Role, AAA are empty at base build' % pre_name
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 1, False))
    
    pre_name='[verify empty cfg in standby zd]'
    test_name = 'CB_Scaling_Verify_Empty_CFG'
    common_name = '%sCheck WLAN, User, Role, AAA are empty at base build' % pre_name
    param_cfg = dict(zd='standby_zd')
    test_cfgs.append((param_cfg, test_name, common_name, 1, False))
    
    pre_name='[create Wlan after downgrade in base build]'
    test_name = 'CB_ZD_Remove_All_Wlans'
    ...
    pre_name='[create role after downgrade in base build]'
    test_name = 'CB_ZD_Remove_All_Roles'
    ...
    common_name = '%sget role list at base build' % pre_name
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))    
        
    pre_name='[create user after downgrade in base build]'
    test_name = 'CB_ZD_Create_Local_User'
    common_name = "%sreate local username/password as \'rat_local_user/rat_local_user\' at base build" % pre_name
    param_cfg = dict(username = 'rat_local_user', password = 'rat_local_user')
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))   
    ...
    common_name = '%sget WLANs list at base build' % pre_name
    param_cfg = dict()
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))   
#    
#        
...
#    
#    
    pre_name='[create aaa server after downgrade in base build]'
    test_name = 'CB_Scaling_Remove_AAA_Servers'
    ...
    test_name = 'CB_ZD_Create_Authentication_Server'
    common_name = '%sCreate a LDAP/Radius/Accounting server rat_ldap_test at base build' % pre_name
    param_cfg = dict(auth_ser_cfg_list = [cfg['ldap_ser_cfg'], cfg['radius_ser_cfg'], cfg['ad_ser_cfg']])
    test_cfgs.append((param_cfg, test_name, common_name, 0, False))    
    ...
    pre_name = '[verify ap after upgrade]'
    test_name = 'CB_Scaling_Waiting'
    common_name = '%s:Waiting for %d minutes for aps upgrade' % (pre_name,cfg['wait_for_ap_upgrading'])
    test_cfgs.append(({'timeout':cfg['wait_for_ap_upgrading']*60}, test_name, common_name, 1, False))
   
    test_name = 'CB_ZD_SR_Get_Active_ZD'
    common_name = "%sGet the Active ZD at target build" % pre_name
    test_cfgs.append(({},test_name,common_name,0,False))
    ...
    pre_name = '[verify cfg in active zd]'
    test_name = 'CB_ZD_Verify_Roles'
    common_name = "%sVerify roles info at target build" % pre_name
    test_cfgs.append(({},test_name,common_name,1,False))     
    
    test_name = 'CB_ZD_Verify_Local_User'
    common_name = "%sVerify Local user info at target build" % pre_name
    test_cfgs.append(({},test_name,common_name,1,False))
        
    test_name = 'CB_ZD_Verify_Wlans_Info'
    common_name = "%sVerify Wlans info" % pre_name
    test_cfgs.append(({},test_name,common_name,1,False)) 
    
    test_name = 'CB_ZD_Verify_Authentication_Server_Info'
    common_name = "%sVerify Authentication server info at target build" % pre_name
    test_cfgs.append(({},test_name,common_name,1,False))     
    
    pre_name = '[verify cfg in standby zd]'
    test_name = 'CB_ZD_Verify_Roles'
    common_name = "%sVerify roles info " % pre_name
    test_cfgs.append(({'zd':'standby_zd'},test_name,common_name,1,False))     
    
    test_name = 'CB_ZD_Verify_Local_User'
    common_name = "%sVerify Local user info" % pre_name
    test_cfgs.append(({'zd':'standby_zd'},test_name,common_name,1,False))
...
#        active_ap_list = getActiveAp(ap_sym_dict)        
    ...
    cfg['wait_for_sync_after_instal_simap'] = 2 #minutes
...
def create_test_suite(**kwargs):
    ...
    #Update paramters by user
    #interactive_cfg(cfg)
    ts_name = 'ZD downgrade as default level and upgrade as previous configuration level when SR enabled'
    ts = testsuite.get_testsuite(ts_name, 
                                 'ZD downgrade as default level and upgrade as previous configuration level when SR enabled', 
                                 combotest=True)
