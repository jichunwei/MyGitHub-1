'''
Configure SNMP V3 agent via ZD GUI [RO Auth=SHA Priv=None RW Auth=MD5 Priv=DES]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=SHA Priv=None RW Auth=MD5 Priv=DES]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD GUI - SNMP V3 Validation - RO (SHA,None) RW (MD5,DES)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=SHA,priv=None) RW (auth=MD5,priv=DES)', combotest=True)
