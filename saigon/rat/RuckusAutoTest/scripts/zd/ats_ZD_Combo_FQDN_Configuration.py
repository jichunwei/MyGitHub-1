"""
Verify configuration limited ZD discovery settings in ZD GUI. 

    Verify configure limited ZD discovery settings via ZD GUI.
    ...
    How to:
        ...
        3) Set limited zd discovery in zd gui.
        4) Verify lmited zd discovery setting between set and get.
        5) Get limited zd discovery in zd cli.
...
def define_test_cfg(cfg):
    ...
    test_cfgs.append(( {'auto_approval': True,
                        'zd_tag': zd1_gui_tag,
                        'mac_addr_list': all_ap_mac_list}, test_name, common_name, 0, False))
    ...
    for zd_discovery_test_cfg in zd_discovery_cfg_list:
        ...
        common_name = '[%s]Configure FQDN via GUI' % test_case_name
        test_params = {'zd_tag': zd1_gui_tag}
        ...
        test_name = 'CB_ZD_Verify_Primary_Secondary_GUI_Set_Get'
        common_name = '[%s]Verify FQDN between GUI Set and Get' % test_case_name
    ...
    common_name = '[%s]Reboot ZD from GUI'% test_case_name
    test_cfgs.append(( {'zd_tag': zd1_gui_tag}, test_name, common_name, 2, False))
    ...
    test_case_name = "GUI - Outside Boundary for Primary and Secondary"
    
    test_name = 'CB_ZD_Set_Primary_Secondary_ZD_Invalid_Values'
...
def create_test_suite(**kwargs):
    ...
    all_ap_sym_list = ap_sym_dict.keys()
    ...
    zd1_cli_tag = 'ZDCLI1'
    zd2_cli_tag = 'ZDCLI2'
    default_engine_ip = '192.168.0.10'
    ...
    if enable_mgmt_vlan:
        ap_mgmt_vlan = raw_input("Please input management vlan for ap, enter to access default [10]:")
        if not ap_mgmt_vlan:
            ap_mgmt_vlan = '10'
    ...
    for active_ap in active_ap_list:
        ...
        tcfg = {'zd_discovery_cfg_list': zd_discovery_cfg_list,
                ...
                'all_ap_sym_list': all_ap_sym_list,
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if enable_mgmt_vlan:
                ts_name = "ZD GUI - Configuration FQDN with AP Mgmt Vlan"
            else:
                ts_name = "ZD GUI - Configuration FQDN"
