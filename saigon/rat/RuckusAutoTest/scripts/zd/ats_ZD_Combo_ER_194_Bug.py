'''
https://jira.ruckuswireless.com/browse/ER-194
How to Test:
    -Create an accounting.
    -Create an Open-None WLAN with accounting server
    -Let Station associate wlan.
    -Start Radius-accounting server traffic capture:
        tshark -i <interface> udp port 1813
    -Reboot ZD
    -Stop Radius-accounting server traffic capture
    -Check if RAS-Type-ID is existed whatever ZD reboot.
...
wlan_cfg = dict(ssid = "rat-wlan-%s" % (time.strftime("%H%M%S")),                
                auth = "open",                 
                encryption = "none",
                acct_svr = "accounting-svr",                
                )

server_cfg = {
    'server_name': "accounting-svr",
...
def _setup_tcs():
    ...
    tcs.append(({'auth_ser_cfg_list':[server_cfg]},
...
def build_tcs(target_station):
    ...
    tcs.append(({'wlan_cfg':wlan_cfg,
                 'acct_nas_id_type':'user-define', 
                 'user_define_string':'chris'
                 },
                'CB_ZD_Modify_WLAN_Nas_Id_Type',
                'Update WLAN NAS ID Type',
                0,
                False
                ))
    ...
    tcs.append(({'params':' -f "udp port 1813"'},
                'CB_Server_Start_Tshark',
                '%sStart Tshark capture' % tcid,
                2,
                False
                ))
    ...
    tcs.append(({},
                ...
                '%sStop Tshark Capture' % tcid,
                2,
                False
                ))
    
    tcs.append(({'nas_identifier':'chris'},
                'CB_Server_Verify_NAS',
                '%sVerify NAS attribute if lost' % tcid,
                2,
                False
                ))
...
def create_test_suite(**kwargs):    
    attrs = dict(interactive_mode = True,                                  
                 testsuite_name = "ER-194 Regression",
                 target_station = (0, "ng"),
                 )
    attrs.update(kwargs)
    ...
    ts_name_list = [('ER-194 Regression', build_tcs),                                                            
                    ]
