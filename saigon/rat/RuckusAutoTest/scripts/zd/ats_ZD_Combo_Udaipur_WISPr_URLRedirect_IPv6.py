'''
1) URL is IPV4 ==> http://172.16.10.252
2) URL is IPV6 ==> http://[2020:db8:50::251]
3) URL is FQDN of IPV4 ===> www.example.net
4) URL is FQDN of IPV6 ===> www.ipv6-example.net
...
def build_hotspot_cfg_list():
    return [{'login_page': 'http://192.168.0.250/login.html',
             'name': 'Wispr_IPV4_Redirect',             
             'start_page':"http://172.16.10.252",
             'target_ip':'172.16.10.252'          
             },
             {'login_page': 'http://[2020:db8:1::249]/login.html',
              'name': 'Wispr_IPV6_Redirect',              
              'start_page':"http://[2020:db8:50::251]",
              'target_ip':'2020:db8:50::251'             
             },
            ]

def build_hotspot_cfg_list_with_dns():
    return[ {'login_page': 'http://192.168.0.250/login.html',
              'name': 'Wispr_IPV4_FQDN_Redirect',
              'start_page':"www.example.net",
              'target_ip':"www.example.net",
             },
             {'login_page': 'http://[2020:db8:1::249]/login.html',
              'name': 'Wispr_IPV6_FQDN_Redirect',
              'start_page':"www.ipv6-example.net",
              'target_ip':"www.ipv6-example.net",
             }
           ]
    

def build_hotspot_cfg_list_v2():
    return [{'login_page': 'http://[2020:db8:1::249]/vlogin.html',
             'name': 'Wispr_IPV6_only_Redirect',             
             'start_page':"http://[2020:db8:50::251]",
             'target_ip':"2020:db8:50::251",
             },             
            ]

def build_hotspot_cfg_list_v2_with_dns():
    return [{'login_page': 'http://[2020:db8:1::249]/vlogin.html',
              'name': 'Wispr_IPV6_only_FQDN_Redirect',              
              'start_page':"www.ipv6-example.net",
              'target_ip':"www.ipv6-example.net",             
             }
            ]
...
def build_station_test_cfg(cfg, test_case_name, wlan_cfg, sta_tag, browser_tag, ap_tag, zd_ip_version, auth_info, hotspot_cfg):
    ...
    target_ip = hotspot_cfg.pop('target_ip')
...
#        target_ip_list.append(target_ipv6_addr)
    ...
    param_cfgs = {'sta_tag':sta_tag,
                  ...
                  'start_page':hotspot_cfg['start_page']
                  }
...
def build_tcs(cfg, hotspot_cfg_list):
    test_cfgs = []
    ...
    for name in hotspot_cfg_name_list:
        wlan_list.append(build_wlan_cfg(**{'ssid':'RAT_URL_%s' % name,
    ...
    for wlan in wlan_list:
        ...
        #Modified by Liang Aihua on 2014-8-12
        #test_name = 'CB_Station_Remove_All_Wlans'
        #common_name = 'Remove the wlan %s from station' % (wlan['ssid'],)
        #test_cfgs.append(({'sta_tag': sta_tag}, test_name, common_name, 0, True))
            
        
        #test_name = 'CB_ZD_Remove_All_Wlans'
        #common_name = 'Remove WLAN %s from ZD' % wlan['ssid']
...
def create_test_suite(**kwargs):
    ts_cfg = dict(interactive_mode = True,
                 station = (0, "g"),
                 targetap = False,
                 testsuite_name_list = ["WISPr URL Redirect Testing with IP Address", 
                                        "WISPr URL Redirect Testing With FQDN"],
                 )
    ...
    if zd_ip_version == const.DUAL_STACK:
        ...
        dns_hotspot_cfg_list=build_hotspot_cfg_list_with_dns()
        
    if zd_ip_version == const.IPV6:
        hotspot_cfg_list = build_hotspot_cfg_list_v2()
        dns_hotspot_cfg_list=build_hotspot_cfg_list_v2_with_dns()
        
        
    if ts_cfg['interactive_mode']:
        ts_name_list = ["WISPr URL Redirect Testing with IP Address - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio),                    
                        "WISPr URL Redirect Testing With FQDN - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)]
    else:
        ts_name_list = ts_cfg['testsuite_name_list']
    
    hlist = []
    hlist.append(hotspot_cfg_list)
    hlist.append(dns_hotspot_cfg_list)
     
    for id, ts_name in enumerate(ts_name_list):
        ...
        if active_ap:
            ...
            test_cfgs = build_tcs(tcfg, hlist[id])
                
                
            ts = testsuite.get_testsuite(ts_name, "WISPr URL Redirect Testing in IPv6", combotest = True)
