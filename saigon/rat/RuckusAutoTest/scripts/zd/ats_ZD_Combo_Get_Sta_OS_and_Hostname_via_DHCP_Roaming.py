'''
Title: Get_Sta_OS_and_Hostname_via_DHCP_Roaming

Test purpose: Verify if ZD can get station OS type and hostname information correctly after roaming.
...
3) Create a new wlan group
4) Configure a standard wlan
5) Assign the wlan to the new wlan group
6) Assign AP_01 to the new wlan group
...
12) Assign AP_02 to the new wlan group
13) Make the station roam from AP_01 to AP_02 and check if roaming happens
...
#---TestCase---------------------------------------------------------------------------------------
    ...
    for test_case_name in test_case_name_list:
        ...
        common_name = "%sCreate wlan group: %s" % (test_combo_case_name, wg_cfg['name']) 
        test_cfgs.append(({'wgs_cfg': wg_cfg}, test_name, common_name, 1, False))
        ...
        common_name = '%sAssign the wlan to wlan group %s' % (test_combo_case_name, wg_cfg['name'])
        test_params = {'wlangroup_name': wg_cfg['name'], 'wlan_name_list': [wlan_cfg['ssid']]}
        test_cfgs.append((test_params, test_name, common_name, 2, False))
        
        test_name = 'CB_ZD_Assign_AP_To_Wlan_Groups'
        common_name = '%sAssign AP_01 to wlan group %s' % (test_combo_case_name, wg_cfg['name'])
    ...
    ### new added tests script.
        ...
        test_name = 'CB_ZD_Assign_AP_To_Wlan_Groups'
        common_name = '%sAssign AP_02 to wlan group %s' % (test_combo_case_name, wg_cfg['name'])
        ...
        common_name = '%sMake station roam from current AP to another' % (test_combo_case_name,)
        test_params = {'current_ap_tag': 'AP_01',
...
def create_test_suite(**kwargs):
    ...
    wg_cfg = {'name': 'wg_roaming_%s' % time.strftime("%H%M%S"),
              'description': 'WLANs for Roaming test',
              'vlan_override': False,
              'wlan_member': {},
              }
    tcfg = {
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = 'Roaming - Get Sta OS and Hostname via DHCP - Basic'

    ts = testsuite.get_testsuite(
        ts_name, 'Verify ZD obtaining station OS type and hostname via DHCP after roaming.',
