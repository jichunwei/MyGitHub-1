...
def _define_wlan_cfg_list(ras_name, username, password):
    ...
    wlan_cfgs.append(_get_wlan_cfg(dict(auth="open", encryption="none", username = username, password = password)))
...
def create_test_suite(**kwargs):
    ...
    wlan_cfg_list = _define_wlan_cfg_list(aaa_server_cfg['server_name'], username, password)
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "WLAN Open Zero IT - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify open wlan with Zero-IT - 11%s" % (target_sta_radio), combotest = True)
