'''
Configure SNMP V3 agent via ZD GUI [RO Auth=MD5 Priv=AES RW Auth=SHA Priv=DES]
...
def define_test_cfg(tcfg):
    ...
    test_case_name = '[RO Auth=MD5 Priv=AES RW Auth=SHA Priv=DES]'
...
def create_test_suite(**kwargs):
    ...
    ts_name = 'ZD GUI - SNMP V3 Validation - RO (MD5,AES) RW (SHA,DES)'
    ts = testsuite.get_testsuite(ts_name, 'Verify SNMP V3 RO (auth=MD5,priv=AES) RW (auth=SHA,priv=DES)', combotest=True)
