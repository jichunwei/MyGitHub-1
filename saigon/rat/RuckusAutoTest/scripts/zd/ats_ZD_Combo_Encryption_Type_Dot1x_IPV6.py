"""
Verify wlan with dot1x encryption types works well for ipv6

    Verify wlan with dot1x works well when ZD is dual stack and ipv6 only.
    Encryption type coverage:
        EAP-WPA-TKIP with Local Database
        EAP-WPA2-AES with Radius
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "WLAN Encryption Types Dot1x - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify WLAN with EAP encryption type - 11%s" % (target_sta_radio), combotest = True)
