...
def define_encryption_types_webauth_test_cfgs(tcfg, wlan_cfg, ap_sym):
    ...
    common_name = '%s Assign the wlan to wlan group: %s' % (com_name, tcfg['wlan_group_cfg']['name'])
    test_params = {'wgs_cfg': tcfg['wlan_group_cfg'], 'wlan_list': [wlan_cfg['ssid']]}
    ...
    common_name = '%s Verify station info on ZD before web auth' % com_name
    ...
    test_name = 'CB_ZD_Verify_Station_Info'
    common_name = '%s Verify station info on ZD after web auth' % com_name
    ...
    test_cfgs.append(({'sta_tag': tcfg['sta_ip_addr']}, test_name, common_name, 2, True))
    
    test_name = 'CB_ZD_Remove_Wlan_On_Wlan_Group'
    common_name = '%s Remove wlan from wlan group: %s' % (com_name, tcfg['wlan_group_cfg']['name'])
    test_params = {'wgs_cfg': tcfg['wlan_group_cfg'], 'wlan_list': [wlan_cfg['ssid']]}
...
def define_test_cfgs(tcfg):
    ...
    common_name = 'Create AP: %s' % ap_sym
    ...
    common_name = 'Create an empty wlan group: %s' % tcfg['wlan_group_cfg']['name']
    test_cfgs.append(({'wgs_cfg': tcfg['wlan_group_cfg']}, test_name, common_name, 0, False))

    test_name = 'CB_ZD_Assign_AP_To_Wlan_Groups'
    common_name = "Assign wlan group '%s' to radio '%s' of AP: %s"
    common_name = common_name % (tcfg['wlan_group_cfg']['name'], tcfg['active_radio'], ap_sym)
    test_params = {'active_ap': ap_sym,
                   'wlan_group_name': tcfg['wlan_group_cfg']['name'], 
    ...
    common_name = 'Remove all wlans from the target station to clean up'
...
def create_test_suite(**kwargs):
    ...
    wlan_group_cfg = dict(name = 'rat-wg-webauth',
    ...
    if attrs['ts_name']:
        ...
        #Modified by Liang Aihua in order to remove ap info in test suite name.    
        ts_name = "Mesh - Encryption Types Web Auth - Wlan Groups%s - %s" 
        ts_name = ts_name % (active_ap_role,active_radio)
        #ts_name = "Mesh - Encryption Types Web Auth - Wlan Groups - %s%s - %s" 
