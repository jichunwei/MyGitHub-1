...
Verify AP can insert DHCP option82 subopt into DHCP packages.
    ...
    How to:
        1) Disable SR
        2) Remove all configuraton of ZD1
        3) Enable SR
        4) Disable all APs' WLAN service
        5) Disable SW DHCP relay
        6) Enable ZD DHCP relay
        7) Create AP instance and enable its WLAN service
        8) Create station.
        9) Create WLAN and enable subopt randomly
        10) Failover ZD
        11) Start Tshark on DHCP server and station
        12) Associate station to WLAN, and authenticate if need
        13) Verify station connection
        14) Stop Tshark on DHCP server and station
        15) Verify DHCP package on DHCP server and station
        16) Edit WLAN to disable subopt
        17) jump to step 9 to 15
        18) Enable DHCP relay on SW
        19) Disable DHCP relay on ZD
        20) Enable all APs' WLAN service
        21) Disable SR
...
def define_test_cfg(cfg):
    ...
    common_name = 'Initial SR Test Environment'
    ...
    test_cfgs.append(({'zd_tag':'zd1', 'zdcli_tag':'ZDCLI1'}, test_name, common_name, 0, False))
    ...
    test_cfgs.append(({'cfg_type': 'init',
                       'zd_tag': 'active_zd'}, test_name, common_name, 0, False))
    ...
    tc_common_name = 'Verify DHCP Option82 subopt under SR combine with'
    ...
    for tc_name in wlan_cfgs.keys():
        ...
        test_cfgs.append(({'ssid':wlan_cfg.get('ssid'),'ap_tag': ap_sym_name}, test_name, common_name, 2, False))

        test_name = 'CB_ZD_SR_Failover'
        step += 1
        common_name = '[%s %s]%s.%s Failover ZD' % (tc_common_name, tc_name, round, step)
        ...
        if tc_name in ['guestaccess-wlan']:
        ...
        if tc_name not in ['hotspot-wlan']:
    ...
    test_cfgs.append(({'cfg_type': 'teardown',
                       'zd_tag': 'active_zd'}, test_name, common_name, 0, True))

    test_name = 'CB_ZD_Remove_All_Config' 
    common_name = 'Remove all configurations of ZD after test'
    test_cfgs.append(({'zd_tag':'zd1', 'zdcli_tag':'ZDCLI1'}, test_name, common_name, 0, True)) 
...
def create_test_suite(**kwargs):
    ...
    if active_ap_list != []:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if access_vlan:
                ts_name = "ZD CLI - Verify DHCP Option82 subopt under SR with AP Mgmt Vlan 11%s" % (target_sta_radio)
            else:
                ts_name = "ZD CLI - Verify DHCP Option82 subopt under SR 11%s" % (target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify DHCP Option82 subopt combine with different wlan type under SR", combotest = True)
