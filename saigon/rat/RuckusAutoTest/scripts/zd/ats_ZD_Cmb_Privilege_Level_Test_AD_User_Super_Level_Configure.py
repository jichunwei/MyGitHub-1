...
#    
    ...
    common_name = '%slogin zd web by AD super user' % test_case_name
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Super level AD User privilege level test-Configure"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="Super level AD User privilege level test-Configure"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Super level AD User privilege level test-Configure", interactive_mode = attrs["interactive_mode"], combotest=True)
