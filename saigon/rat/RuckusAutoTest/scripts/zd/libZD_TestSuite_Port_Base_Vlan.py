...
def get_zonedirector_apsdict(zdcfg):
    cfg = dict(
        ip_addr = '192.168.0.2',
        username = 'admin',
        password = 'admin',
...
def get_test_bed(**kwargs):
    atrs = {'zd_ip_addr':'192.168.0.2', 'svr_ip_addr':'192.168.0.252', 'tbtype':'ZD_Port_Base_Vlan',
    ...
    try:
        ...
        linpc_ip_list = atrs['linpc_ip_list'] if atrs.has_key('linpc_ip_list') \
                      else raw_input("Linux PC IP address list (separated by spaces): ").split()
        if atrs.has_key('ApUseSym') and atrs['ApUseSym']:
            ap_sym_dict = get_zonedirector_apsdict({'ip_addr': atrs['zd_ip_addr'],
        ...
        linuxpc_conf = {}
        id = 1
        for linpc_ip in linpc_ip_list:
            linuxpc_key = 'linuxpc%s' % str(id)
            linuxpc = _getAtrVal('linuxpc_conf', atrs,
                   {linuxpc_key: {'ip_addr': linpc_ip
                   , 'user': 'lab'
                   , 'password': 'lab4man1'
                   , 'root_password': 'lab4man1'
                   }})
            linuxpc_conf.update(linuxpc)
            id += 1
