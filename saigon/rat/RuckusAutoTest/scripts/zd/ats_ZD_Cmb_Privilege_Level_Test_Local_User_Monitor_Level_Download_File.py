'''
verify the monitor user can do the following operation
Administer page - Diagnotics    Diagnotics - Save debug info
Administer page - Diagnotics    Diagnotics - Save System Log
Administer page - Diagnotics    Registration - generate and download registration request file (csv file)
Administer - Backup    Download current backup configuration
Monitor - Access Points    Download AP debug info
https://zd_ip/user/guestpass.jsp    Download guest list
'''
...
def define_test_configuration(tcfg):
    ...
    common_name = 'login zd web by local monitor user'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Monitor level Local User privilege level test-download file"
    )
    attrs.update(kwargs)
    ...
    user_cfg =dict(  username = 'monitor_user',
    ...
    else: ts_name ="Monitor level Local User privilege level test-download file"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Monitor level Local User privilege level test-download file", interactive_mode = attrs["interactive_mode"], combotest=True)
