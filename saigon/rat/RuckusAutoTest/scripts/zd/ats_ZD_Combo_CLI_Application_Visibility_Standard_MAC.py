...
def define_wlan_cfg(auth_svr,rad_user): 

    key_string_wpa2 = utils.make_random_string(random.randint(8, 63), "hex")
    wlan_cfg = {
                'ssid': 'Application_Visibility_MAC', 
                'type': 'standard', 
                'auth': 'mac', 
                'wpa_ver': 'WPA2',
                'encryption': 'AES', 
...
def define_test_cfg(cfg,enable_tunnel):
    ...
    wlan_cfg = define_wlan_cfg(ras_cfg['server_name'],rad_user)
    ...
    common_name = "Delete station mac info in Radius"
    test_cfgs.append(({'mac_format_type': 'all_formats',
                       'sta_tag':sta_tag}, test_name, common_name, 0, False))
    
    test_name = 'CB_Server_Add_STA_MAC_As_Radius_User'
    common_name = "Add station's MAC addr as radius user"
    test_cfgs.append(({'mac_format_type': 'aabbccddeeff',
                       'sta_tag':sta_tag}, test_name, common_name, 0, False))
...
#testcase 1
    ...
    test_case_name = '[user_app_open_mac%s]'%case_name_suffix 

    user_app_rule_cfg = {'rule_description':'user_app_open_mac',
    ...
    test_cfgs.append(({'server_addr':'',
                       ...
                       'tos':'',
                       'multicast_srv':False,
                       'port':34567 }, test_name, common_name, 2, False))
    ...
    test_cfgs.append(({'application_description':'user_app_open_mac'}, test_name, common_name, 2, False)) 
...
#testcase 2
    ...
    test_case_name = '[port_mapping_open_mac%s]'%case_name_suffix  

    port_mapping_rule_cfg = {'rule_description':'port_mapping_open_mac','protocol':'udp','port':'43210'}
    ...
    test_cfgs.append(({'server_addr':'',
                       ...
                       'tos':'',
                       'multicast_srv':False,
                       'port':43210}, test_name, common_name, 2, False))
    ...
    test_name = 'CB_ZD_Verify_Application_Visibility_Info'
    common_name = '%sVerify application info in Monitor Clients page'% (test_case_name)
    test_cfgs.append(({'application_description':'port_mapping_open_mac',}, test_name, common_name, 2, False)) 
...
#testcase 3
    ...
    test_case_name = '[denial_policy_open_mac%s]'%case_name_suffix 
...
#clean_up                       
    ...
    test_name = 'CB_Server_Del_STA_MAC_As_Radius_User'                
    common_name = "Delete station mac info in Radius after test"
    test_cfgs.append(({'mac_format_type': 'all_formats',
                       'sta_tag':sta_tag}, test_name, common_name, 0, False))
...
Default selection is 2.Input your choice:")
    ...
    if ts_cfg["testsuite_name"]:
        ...
        ts_name = "Application_Visibility - Standard - MAC%s"%ts_suffix

    ts = testsuite.get_testsuite(ts_name, "Application_Visibility - Standard - MAC%s"%ts_suffix , combotest=True)
