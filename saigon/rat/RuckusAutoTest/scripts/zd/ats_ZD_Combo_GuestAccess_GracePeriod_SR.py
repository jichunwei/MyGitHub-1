...
def define_test_cfgs(tcfg):
    ...
    test_cfgs.append((guest_pass_cfg, test_name, common_name, 2, False))
    ...
    test_params = {'sta_tag': sta_tag,
                   ...
                   'no_need_auth': True,
                   'wlan_cfg': wlan_cfg,
                   'guest_name': guest_pass_cfg['guest_fullname'],
                   'radio_mode': radio_mode,
                   'target_ip': target_ip,
                   }
    test_cfgs.append((test_params, test_name, common_name, 2, False))
    ...
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-sr-%s-2' % time.strftime("%H%M%S")
    guest_pass_cfg = deepcopy(tcfg['guest_pass_cfg'])
    guest_pass_cfg['wlan'] = wlan_cfg['ssid']
    guest_pass_cfg['guest_fullname'] = 'guest-sr-2'
    grace_period = 8
    ...
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-sr-%s-3' % time.strftime("%H%M%S")
    guest_pass_cfg = deepcopy(tcfg['guest_pass_cfg'])
    guest_pass_cfg['wlan'] = wlan_cfg['ssid']
    guest_pass_cfg['guest_fullname'] = 'guest-sr-3'
    grace_period = 10
    ...
    wlan_cfg = deepcopy(tcfg['wlan_cfg'])
    wlan_cfg['ssid'] = 'gp-sr-%s-4' % time.strftime("%H%M%S")
    guest_pass_cfg = deepcopy(tcfg['guest_pass_cfg'])
    guest_pass_cfg['wlan'] = wlan_cfg['ssid']
    guest_pass_cfg['guest_fullname'] = 'guest-sr-2'
    grace_period = 8
...
def create_test_suite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        ts_name = "Guest Access - Grace Period - SR" 
#        ts_name = "Guest Access - Grace Period - SR - %s - %s" 
