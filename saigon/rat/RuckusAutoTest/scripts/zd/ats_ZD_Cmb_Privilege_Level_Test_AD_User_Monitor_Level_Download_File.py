...
#    
    ...
    common_name = 'create AD server via WebUI'
    ...
    common_name = '%slogin zd web by AD monitor user' % test_case_name
...
#    
    ...
    common_name = 'remove AD server %s ' % (tcfg['server_cfg']['server_name'])
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Monitor level AD User privilege level test-Downlaod Files"
    )
    attrs.update(kwargs)
...
#    
    ...
    else: ts_name ="Monitor level AD User privilege level test-Downlaod Files1"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Monitor level AD User privilege level test-Downlaod Files2", interactive_mode = attrs["interactive_mode"], combotest=True)
