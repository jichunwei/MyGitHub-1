...
def define_guest_access_test_cfgs(tcfg, guest_access_cfg, ap_sym):
    ...
    test_params = {'sta_tag': tcfg['sta_ip_addr'], 'status': 'Unauthorized',
                   'radio_mode': tcfg['zd_sta_radio'], 'wlan_cfg': tcfg['wlan_cfg'], 'ap_tag': ap_sym}
    test_cfgs.append((test_params, test_name, common_name, 2, False))   
    ...
    test_params = {'sta_tag': tcfg['sta_ip_addr'], 'use_tou': guest_access_cfg['use_tou'], 
                   'redirect_url': guest_access_cfg['redirect_url'], 
...
def define_test_cfgs(tcfg):
    ...
    #@author: Jane.Guo @since: 2013-09 add a step to add acls of guest access.
    restricted_subnet_list_1 = ['192.168.0.0/16',]
    test_name = 'CB_ZD_Set_Guest_Restricted_Subnet_Access'
    common_name = 'Add zd restricted subnet access'
    test_params = {'restricted_subnet_list': restricted_subnet_list_1}
    test_cfgs.append((test_params,test_name, common_name, 0, False))
    ...
    common_name = 'Verify wlan on %s' % ap_sym
    ...
    test_cfgs.append(({'sta_tag': tcfg['sta_ip_addr']}, test_name, common_name, 0, True))
...
def create_test_suite(**kwargs):
    ...
    wlan_group_cfg = dict(name = 'rat-wg-guestaccess',
                          description = 'WLANs for Guest Access Test',
                          vlan_override = False, 
    ...
    if attrs['ts_name']:
        ...
        #Modfified by Liang Aihua on 2014-8-26 to remove ap_model from test suite name.    
        ts_name = "Mesh - Guest Access - Wlan Groups%s -%s" 
        ts_name = ts_name % (active_ap_role, active_radio)
        #ts_name = "Mesh - Guest Access - Wlan Groups - %s%s - %s" 
