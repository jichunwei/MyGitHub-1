...
#    save_to = os.path.join(os.path.expanduser('~'), r"My Documents\Downloads" )
    ...
    test_case_name = ''
    test_name = 'CB_ZD_Cmb_N_Plus_One_Init'
    common_name = '%sset configure file for test'%test_case_name
...
#    
    ...
    test_cfgs.append(({'restore_type':'restore_everything_except_ip','zd':'zd1','for_n_plus_one':True}, test_name, common_name, 0, False))    
#    
...
#    test_cfgs.append(({'ap_mac':mesh_ap_mac,'zd_tag':'zdcli1'},test_name, common_name, 0, False))
    ...
    test_cfgs.append(( {'zd':'zd1','filename':'Nplusone.bak'}, test_name, common_name, 0, False))    
    ...
    common_name = '%sCheck all of APs are connected to pri zd after config' %test_case_name
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="N plus One basic"
    )
    attrs.update(kwargs)
...
#                active_radio = active_radio,
    ...
    ts = testsuite.get_testsuite(ts_name, "N plus One basic", interactive_mode = attrs["interactive_mode"], combotest=True)
