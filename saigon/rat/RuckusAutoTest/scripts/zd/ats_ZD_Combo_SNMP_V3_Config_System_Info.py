'''
Config system information via SNMP V3.

    Can set system nodes via snmp v3 successfully and the values are same with cli get.
...
def define_test_cfg(tcfg):
    ...
    test_name = 'CB_ZD_CLI_Set_SNMP_Agent'
    common_name = 'Enable SNMP Agent Version 3'
    ...
    test_name = 'CB_ZD_CLI_Set_SNMP_Agent'
    common_name = '%sRestore SNMP Agent V3 Config' % (test_case_name,)
    ...
    common_name = '%sVerify Disable SNMP Agent V3 via SNMP' % (test_case_name,)
    test_cfgs.append(( {'snmp_agent_cfg': tcfg['set_snmp_agent_cfg'], 'snmp_cfg': tcfg['snmp_cfg']}, 
                       test_name, common_name, 2, False))
...
def create_test_suite(**kwargs):
    ...
    if str(tb.tbtype) == "ZD_Stations_IPV6":
        ...
        ts_name = 'ZD SNMP V3 ZD %s AP %s - System Info Configuration' % (zd_ip_version, ap_ip_version)
    else:
        ts_name = 'ZD SNMP V3 - System Info Configuration'
