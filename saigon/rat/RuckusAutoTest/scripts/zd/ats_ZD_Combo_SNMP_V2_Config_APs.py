'''
Configure ZD AP settings via SNMP V2.

    Update ZD AP setting via SNMP V2 successfully and the value same as CLI.    
...
def define_test_parameters(tbcfg):
    ...
    wg_cfg = dict(wg_name = 'wg-for-WGforUpdateAP-ap', description = 'Wlan groups for update AP config')
...
def create_test_suite(**kwargs):
    ...
    if str(tb.tbtype) == "ZD_Stations_IPV6":
        ...
        ts_name = 'ZD SNMP V2 ZD %s AP %s - AP Configuration' % (zd_ip_version, ap_ip_version)
    else:
        ts_name = 'ZD SNMP V2 - AP Configuration'
