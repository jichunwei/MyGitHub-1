...
def _define_wlan_cfg():
    ...
    '''
    ssid = "guestaccess-%04d" % random.randrange(1,9999)
    wlan_cfg = {'ssid': '',
                'auth': "open",
                'encryption': "none",
                'type': 'guest-access',
...
def _define_restricted_rule_cfg(dst_addr, action = 'Deny'):
    ...
    '''
    rule_cfg = {'order': '', 'description': '', 'action': 'Deny', 'dst_addr': 'Any',
                'protocol': 'Any', 'dst_port': 'Any', 'icmp_type': 'Any'}
    ...
    rule_cfg['description'] = "rule%04d" % random.randrange(1,9999) 
...
def define_test_cfg(cfg):
    ...
    test_params = {'wlan_conf': wlan_cfg}
    test_cfgs.append((test_params, test_name, common_name, 0, False))
...
def create_test_suite(**kwargs):
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            ts_name = "ZD CLI - Guest Access with Restricted IPV6 Access - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)
