...
1. zds on sync
2. disable SR on standby zd
3. del/mod/add configuration on active ZD
4. enable SR on standby zd
'''
...
####################################################
    ...
    test_name = 'CB_ZD_Set_Device_IP_Settings'
    common_name = 'Set higner mac ZD device IP settings via GUI'
...
##############################################################################################   
    ...
    test_name = 'CB_ZD_SR_Disable'
    common_name = '[%s]Disable Smart Redundancy on standby ZD' % test_case_name
    test_cfgs.append(({'single':'higher_mac_zd'},test_name, common_name, 2, False))
    ...
    test_name = 'CB_ZD_SR_Enable'
    common_name = '[%s]enable Smart Redundancy on standby ZD' % test_case_name
    test_cfgs.append(({},test_name,common_name,2,False)) 
...
####################################################   
    ...
    test_name = 'CB_ZD_SR_Disable'
    common_name = '[%s]Disable Smart Redundancy on standby ZD' % test_case_name
    test_cfgs.append(({'single':'higher_mac_zd'},test_name, common_name, 1, False))
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="compare after operation when standby zd disable-enable SR"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="compare after operation when standby zd disable-enable SR"
    test_cfgs = defineTestConfiguration(active_ap_list)
    ts = testsuite.get_testsuite(ts_name, 'compare after operation when standby zd disable-enable SR', interactive_mode = attrs["interactive_mode"], combotest=True)
