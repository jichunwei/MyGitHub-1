'''
author: elina.liang@odc-ruckuswireless.com

Description: Test suite used to check priority between wlan service and its schedule configuration.
...
def define_expect_wlan_info_in_ap(tcfg, wlan_service_cfg):
    ...
    for radio in tcfg['radio_mode']:
        ...
        if radio in ['bg', 'ng']:
            wlan_cfg_list = tcfg['wlan_cfgs']['priority_wlan_list_2g']
            ...
            wlan_cfg_list = tcfg['wlan_cfgs']['priority_wlan_list_5g']
            for i in range(len(wlan_cfg_list)):
...
def define_prim_test_cfg(tcfg, wlan_service_cfg, com_name, ap_type = ""):
    ...
    for radio in tcfg['radio_mode']:
        if wlan_service_cfg[radio]:
            allow_wlan_cfg_list.extend(tcfg['wlan_cfgs']['priority_wlan_list_%s' % (RADIO_MAP[radio])])
        
        else:
            deny_wlan_cfg_list.extend(tcfg['wlan_cfgs']['priority_wlan_list_%s' % (RADIO_MAP[radio])])
...
def define_priority_test_cfg(tcfg):
    _test_cfgs = []
    
    wlan_cfg_list = []
    for radio in tcfg['radio_mode']:
        wlan_cfg_list_name = 'priority_wlan_list_%s' %(RADIO_MAP[radio])
    ...
    for radio in tcfg['radio_mode']:
        ...
        common_name = "Set radio '%s' wlan schedule to always on" %radio
        _test_cfgs.append(({'on': True, 'wlan_cfg': wlan_cfg}, test_name, common_name, 0, False))
    ...
    for radio in tcfg['radio_mode']:
        group_key = "priority_group_cfg_%s" % (RADIO_MAP[radio])
...
def define_test_cfg(tcfg):
    ...
    test_cfgs.extend(define_priority_test_cfg(tcfg))
...
def createTestSuite(**kwargs):
    ...
    if attrs['ts_name']:
        ...
        if len(radio_mode)== 1:
            ts_name = "Mesh - Wlan Service Priority Test - Single Band"
        else:
            ts_name = "Mesh - Wlan Service Priority Test- Dual Band"
        #ts_name = "Mesh - Wlan Service Enable and Disable - %s" % ap_model

    else:
        if len(radio_mode)== 1:
            ts_name = "Wlan Service Priority Test - Single Band"
        else:
            ts_name = "Wlan Service Priority Test - Dual Band"
