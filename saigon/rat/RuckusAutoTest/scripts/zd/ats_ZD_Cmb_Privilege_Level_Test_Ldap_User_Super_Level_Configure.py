...
#    
    ...
    common_name = 'login zd web by ldap super user'
    ...
    #@author: yuyanan @since: 2014-8-28 bug9639 behavior change
    ...
    #@author:yuyanan @since:;2014-8-28 optimize ap tag replace ap mac
    ...
    test_cfgs.append(({'level':'super','ap_tag':tcfg['ap_tag']}, test_name, common_name, 1,False))
...
#    test_cfgs.append((param_cfg, test_name, common_name, 0,True))  
    test_name = 'CB_ZD_Remove_All_Users' 
    common_name = 'remove all local users'
...
def make_test_suite(**kwargs):
    attrs = dict (
        ...
        sta_id = 0,
        targetap = False,
        testsuite_name="Super level LDAP User privilege level test-Configure"
    )
    attrs.update(kwargs)
    ...
    else: ts_name ="Super level LDAP User privilege level test-Configure"
...
#    check_validation(test_cfgs)
    ts = testsuite.get_testsuite(ts_name, "Super level ldap User privilege level test-Configure", interactive_mode = attrs["interactive_mode"], combotest=True)
