"""
Wlan with vlan works well with dualstack and IPV6

    Verify wlan with vlan works well when ZD is dual stack and ipv6 only.
    ...
    How to:
        ...
        6) Get station wifi ipv4 and ivp6 address and verify they are in expected subnet [specified vlan id]
...
all_vlan_mapping = {'10': {const.IPV4: {'ip_addr': '192.168.10.252',
                    ...
                    '20': {const.IPV4: {'ip_addr': '192.168.20.252',
                           ...
                           const.IPV6: {'ip_addr': '2020:db8:20::251',
                                    'prefix_len': '64'},},
                    '301': {const.IPV4: {'ip_addr': '192.168.0.0',
                                     'mask': '255.255.255.0', },
...
def _define_station_test_cfg(cfg, test_case_name, wlan_cfg, sta_tag, browser_tag, ap_tag, ip_version):
    ...
    if all_vlan_mapping.has_key(vlan_id):
        vlan_ip_cfg = all_vlan_mapping[vlan_id]
        ...
        raise Exception("The key [%s] is not in all vlan mapping." % (vlan_id))
...
def _define_wlan_cfg_list(ras_name, vlan_id, tunnel_mode, username, password):
    '''
    ...
    wlan_cfg = _get_wlan_cfg(dict(auth="EAP", wpa_ver="WPA2", encryption="AES",
                                  ...
                                  auth_svr = ras_name, vlan_id = vlan_id, 
...
def create_test_suite(**kwargs):
    ...
    vlan_id = get_selected_input(vlan_options, 'Please select VLAN ID[Default is 2]')
    if not vlan_id:
        vlan_id = '2'
    ...
    wlan_cfg_list = _define_wlan_cfg_list(aaa_server_cfg['server_name'], vlan_id, tunnel_mode, username, password)
    ...
    if active_ap:
        ...
        if ts_cfg["testsuite_name"]:
            ...
            if tunnel_mode:
                ts_name = "WLAN VLAN TUNNEL - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)
            else:
                ts_name = "WLAN VLAN - ZD %s AP %s 11%s" % (zd_ip_version, ap_ip_version, target_sta_radio)

        ts = testsuite.get_testsuite(ts_name, "Verify WLAN with vlan - 11%s" % (target_sta_radio), combotest = True)
