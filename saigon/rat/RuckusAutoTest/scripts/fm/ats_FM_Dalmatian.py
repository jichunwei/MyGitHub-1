...
from libFM_DevCfg import ZF7962_Cfg


'''
3.6.1.1    The AP Country Code list via Template should be consist with wireless
           radio 1 2.4GHz by standalone AP
3.6.1.2    The AP Channel List via Template should be consist with standalone AP
           with wireless radio 1 2.4GHz.

3.6.2.1    The AP Country Code via Template list should be consist with wireless
           radio 2 for 5GHz  by standalone AP
3.6.2.2    The AP Channel List via Template should be consist with standalone AP
           with wireless radio 2 for 5GHz.
...
def define_wpa_cfg(wlan_rd_1=[1,2,3,4,5,6,7,8], wlan_rd_2=[9,10,11,12,11,12,13,14,15,16],
        encrypt_type=[
...
def define_broadcast_cfg(wlan_rd_1=[1,3,5,7], wlan_rd_2=[9,11,13,15]):
    ...
    for wlan in wlan_rd_1:
        broadcast_ssid_rd_1_cfg.update(
            # get the first element of returned list
            dal_cfg_obj.get_wlan_det_cfg_tmpl(wlan_list=[wlan], encrypt_type='open')[0]
        )
...
def define_ts_cfg(**kwa):
   ...
   - models: a list of model, something likes ['zf2925', 'zf7942']
    ...
    tc_id = [
            # 2.4G
            '03.06.01.01.01', '03.06.01.01.02',
            '03.06.01.03', '03.06.01.05', '03.06.01.07', '03.06.01.09',
            '03.06.01.11', '03.06.01.13', '03.06.01.15', '03.06.01.17',

            # 5G
            '03.06.02.01.01', '03.06.02.01.02',
            '03.06.02.03', '03.06.02.05', '03.06.02.07', '03.06.02.09',
            '03.06.02.11', '03.06.02.13', '03.06.02.15', '03.06.02.17',
            ]

    sample_channel_1 = ['0','1','2','3','6','7','8','10','11']
    sample_channel_2 = ['0','36','40','44','100','104','108','161','165']
    sample_country_code = ['AT', 'CA', 'FR', 'ID', 'JP', 'TW', 'CH', 'SE', 'GB', 'US',]

    dal_cfg = ZF7962_Cfg()
    channel_1 = sample_channel_1[random.randint(0, len(sample_channel_1)-1)]
    channel_2 = sample_channel_2[random.randint(0, len(sample_channel_2)-1)]
    country_code = sample_country_code[random.randint(0, len(sample_country_code)-1)]
    ...
    tc_templates = [
      #2.4G tcs
      [ 'TCID:%s.%s - Check consistency Country Code list between Template and '
        ' standalone AP for 2.4GHz - %s',
        'FM_Check_CountryCode_Channel_Match',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            'ap_ip': '',
            'verify_item': COUNTRY_CODE,
            'test_name': 'Check consistency Country Code list between Template '
                         'and standalone AP for 2.4GHz',#'Name of template'
        },
      ],
      [ 'TCID:%s.%s - Check consistency Channel list between Template and '
        ' standalone AP for 2.4GHz - %s',
        'FM_Check_CountryCode_Channel_Match',
        {
            'model': 'ZF7962',
            ...
            'ap_ip': '',
            'verify_item': CHANNEL,
            'test_name': 'Check consistency Channel list between Template '
                         'and standalone AP for 2.4GHz',#'Name of template'
        },
      ],
      [ 'TCID:%s.%s - Change the AP channel via Template for 2.4GHz mode - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            'ap_ip': '',
            'test_name': 'Change channel of 2.4G to "%s" via Template' % channel_1,
      ...
      [ 'TCID:%s.%s - Change the AP Country Code via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            'ap_ip': '',
            'test_name': 'Change country code of 2.4G to "%s" via Template' % country_code,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
      ...
      [ 'TCID:%s.%s - Change the AP channel band width to 20MHz via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            'ap_ip': '',
            'test_name': 'Change channel width of 2.4G to "%s" via Template' % channel_width_no_1,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
      ...
      [ 'TCID:%s.%s - Change the AP channel band width to 40MHz via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            'ap_ip': '',
            'test_name': 'Change channel width of 2.4G to "%s" via Template' % channel_width_no_2,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
      ...
      [ 'TCID:%s.%s - Enable the Wireless Interface availability via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            ...
            'test_name': 'Enable the Wireless Interface availability on wlans %s '
                         'via Template' % broadcast_wlan_rd_1,
      ...
      [ 'TCID:%s.%s - Enable the Wireless Broadcast SSID via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
      ...
      [ 'TCID:%s.%s - Change the Wireless encryption to WEP via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
            'ap_ip': '',
            'test_name': 'Enable encryption type %s on wlans %s via Template' %\
      ...
      [ 'TCID:%s.%s - Change the Wireless encryption to WPA via Template for 2.4GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '2.4G',
      ...
      [ 'TCID:%s.%s - Change the AP channel via Template for 5GHz mode - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
            'ap_ip': '',
            'test_name': 'Change to channel of 5G to "%s" via Template' % channel_2,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
      ...
      [ 'TCID:%s.%s - Change the AP Country Code via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
            'ap_ip': '',
            'test_name': 'Change country code of 5G to "%s" via Template' % country_code,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
                keys = dict(country_code=country_code)
            ),
            'test_type': 'create',
      ...
      [ 'TCID:%s.%s - Change the AP channel band width to 20MHz via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
            'ap_ip': '',
            'test_name': 'Change channel width of 5G to "%s" via Template' % channel_width_no_1,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
      ...
      [ 'TCID:%s.%s - Change the AP channel band width to 40MHz via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
            'ap_ip': '',
            'test_name': 'Change channel width of 5G to "%s" via Template' % channel_width_no_2,
            'input_cfg': dal_cfg.get_wlan_common_cfg_tmpl(
      ...
      [ 'TCID:%s.%s - Enable the Wireless Interface availability via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
      ...
      [ 'TCID:%s.%s - Enable the Wireless Broadcast SSID via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
      ...
      [ 'TCID:%s.%s - Change the Wireless encryption to WEP via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
      ...
      [ 'TCID:%s.%s - Change the Wireless encryption to WPA via Template for 5GHz - %s',
        'FM_Dalmatian_CfgUpgrade',
        {
            'model': 'ZF7962',
            'radio_mode': '5G',
    ...
    for model in kwa['models']:
        for i in range(len(tc_id)):
            if tc_id[i]: # not None
                ...
                if 'provision_to' in tc[2]:
                    tc[2]['provision_to']['device'] = aps[model]
                test_cfgs['%s.%s' % (tc_id[i], model_map[model])] = tc

    # sort the dict and return as a list
    keys = test_cfgs.keys()
    keys.sort()
    return 'Provisioning - Dalmatian Config', [test_cfgs[k] for k in keys]
