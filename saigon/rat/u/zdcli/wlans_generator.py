...
Examples: 
tea.py u.zdcli.wlans_generator num_of_wlans=30
'''
...
default_cfg = dict(ip_addr = '192.168.0.2', 
                   username = 'admin', 
                   password = 'admin', 
                   shell_key = '!v54! CKQpcGkLBgVbYHWieLU9Y24SPpAeirqV\n',
                   )
def _generate_wlan_cfg_list(num_of_wlans = 1):
    
    auth_server = 'test'
    _wlan_cfgs = []
    for i in range(num_of_wlans):
        _wlan_cfgs.append(dict(name = 'open-none-%d' % i,
                               ssid = 'open-none-%d'% i ,
...
def do_test(tcfg):
    zdcli = tcfg['zdcli']    
    wlan_conf_list = _generate_wlan_cfg_list(tcfg['num_of_wlans'])
    set_wlan.create_multi_wlans(zdcli, wlan_conf_list)
    logging.info("Wlan config list is %s" % wlan_conf_list)        
    return ("PASS", "Create wlans information from ZD CLI successfully!")
