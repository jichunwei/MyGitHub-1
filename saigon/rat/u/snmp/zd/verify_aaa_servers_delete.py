...
@summary:
    ...
    Steps:
       1. Create aaa servers with test config.
       2. Delete the wlans.       
       3. Verify the wlan does not exist.  
    
Commands samples:
tea.py u.snmp.zd.verify_aaa_servers_delete
tea.py u.snmp.zd.verify_aaa_servers_delete ip_addr='192.168.0.10' username='admin' password='admin' shell_key='!v54!'
tea.py u.snmp.zd.verify_aaa_servers_delete ip_addr='192.168.0.10'
tea.py u.snmp.zd.verify_aaa_servers_delete ip_addr='192.168.0.10' version=3

#Also can set related config for agent, user read write account when execute snmp get commands.
tea.py u.snmp.zd.verify_aaa_servers_delete ip_addr='192.168.0.10' version=3 timeout=30 retries=3
...
from RuckusAutoTest.components.lib.snmp.zd.aaa_servers import (create_servers,remove_all_servers, delete_servers_by_index,                                                              
                                                               gen_server_test_cfg)
...
def do_test(conf):   
    ...
    res, server_cfg_list = create_servers(conf['snmp'], server_cfg_list)
    
    logging.info('Step 2: Delete aaa servers by snmp, and verify.')
    error_index_list = delete_servers_by_index(conf['snmp'], server_cfg_list.keys())
    
    if error_index_list:
        conf['result'] = 'FAIL: Some servers were not deleted: %s' % error_index_list
    else:
        conf['result'] = 'PASS'
