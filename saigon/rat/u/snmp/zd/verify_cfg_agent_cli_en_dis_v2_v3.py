...
@summary:
     ...
     Steps:
        1. Enable snmp v2 agent.
        2. Check the process works well. [ps -aux|grep 'snmpd']
        3. Execute snmpv2 command to make sure it works.
        4. Enable snmp v3 agent.
        5. Execute snmpv3 command to make sure it works.
        6. Disbale snmp v2 agent.        
        7. Check snmp command: snmpv2 can't work. 
        8. Disbale snmp v3 agent.        
        9. Check snmp command: snmpv3 can't work.
        10. Check the process is disappear. [ps -aux|grep 'snmpd']

Commands samples:
tea.py u.snmp.zd.verify_cfg_agent_cli_en_dis_v2_v3
tea.py u.snmp.zd.verify_cfg_agent_cli_en_dis_v2_v3 ip_addr='192.168.0.10' username='admin' password='admin' shell_key='!v54!'

#set agent config for snmp version 2 and 3.
tea.py u.snmp.zd.verify_cfg_agent_cli_en_dis_v2_v3 ro_community='public' rw_community='private'
...
def do_test(conf):
    try:
        ...
        logging.info('Step 1. Enable snmp v2 agent.')
        ...
        logging.info("Step 3. Execute snmpv2 command to make sure it works.")
        snmp_agent_cfg['version'] = 2
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
            res_d['Step 3'] = result
        
        logging.info('Step 4. Enable snmp v3 agent.')
        snmp_agent_cfg['enabled'] = True
        ...
        logging.info("Step 5. Execute snmpv3 command to make sure it works.")
        snmp_agent_cfg['enabled'] = True
        snmp_agent_cfg['version'] = 3
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
        ...
        logging.info('Step 6. Disbale snmp v2 agent.')
        ...
        logging.info("Step 7. Check snmpv2 command can't work.")
        snmp_agent_cfg['version'] = 2
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
            res_d['Step 3'] = result
        
        logging.info('Step 8. Disable snmp v3 agent.')
        snmp_agent_cfg['enabled'] = False
        snmp_agent_cfg['version'] = 3
        config_snmp_agent(conf['zd_cli'], snmp_agent_cfg)
        ...
        logging.info("Step 9. Check snmpv3 command can't work.")
        snmp_agent_cfg['enabled'] = False
        snmp_agent_cfg['version'] = 3
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
