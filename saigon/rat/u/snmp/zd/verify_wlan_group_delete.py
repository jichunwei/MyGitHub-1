...
@summary:
    ...
    Steps:
       ...
       2. Delete the wlan groups one bye one.       
       3. Verify the wlan group does not exist.   
       
    Notes:
       Default wlan groups with index = 1, can't be deleted.    
    
Commands samples:
tea.py u.snmp.zd.verify_wlan_group_delete
tea.py u.snmp.zd.verify_wlan_group_delete ip_addr='192.168.0.10' username='admin' password='admin' shell_key='!v54!'
tea.py u.snmp.zd.verify_wlan_group_delete ip_addr='192.168.0.10'
tea.py u.snmp.zd.verify_wlan_group_delete ip_addr='192.168.0.10' version=3

#Also can set related config for agent, user read write account when execute snmp get commands.
tea.py u.snmp.zd.verify_wlan_group_delete ip_addr='192.168.0.10' version=3 timeout=30 retries=3
...
from RuckusAutoTest.components.lib.snmp.zd.wlan_group_list import (create_wlan_groups, delete_wlan_groups, 
                                                                   remove_all_wlan_groups, gen_wlan_group_test_cfg) 
...
def _cfg_test_params(**kwargs):
    ...
    logging.info('Preparation: Remove all wlan groups.')    
...
def do_test(conf):
    logging.info('Step 1: Create the wlan groups by snmp.')
    wlan_cfg_list = gen_wlan_group_test_cfg()    
    wlan_cfg_list = create_wlan_groups(conf['snmp'], wlan_cfg_list)
        
    logging.info('Step 2: Delete the wlan groups, and verify.')
    error_index_list = delete_wlan_groups(conf['snmp'], wlan_cfg_list.keys())
