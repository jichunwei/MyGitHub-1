...
@summary:
     ...
     Steps:
        1. Enable snmp v2 and v3 agent.
        2. Check the process works well. [ps -aux|grep 'snmpd']
        3. Execute snmp command to make sure it works. [v2 and v3]
        4. Disbale snmp v2 and v3 agent.        
        5. Check snmp command: snmpv2 and snmpv3 can't work. 

Commands samples:
tea.py u.snmp.zd.verify_cfg_agent_cli_en_dis_both
tea.py u.snmp.zd.verify_cfg_agent_cli_en_dis_both ip_addr='192.168.0.10' username='admin' password='admin' shell_key='!v54!'

#set agent config.
tea.py u.snmp.zd.verify_cfg_agent_cli_en_dis_both ro_community='public' rw_community='private'
...
def do_test(conf):
    try:
        ...
        logging.info("Step 4. Disable snmp v2 and v3 agent.")
        ...
        logging.info("Step 5. Check snmp command: snmpv2 and snmpv3 can't works.")
        snmp_agent_cfg['enabled'] = False
        snmp_agent_cfg['version'] = 2
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
