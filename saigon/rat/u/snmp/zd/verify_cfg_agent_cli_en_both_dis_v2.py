...
@summary:
     ...
     Steps:
        ...
        3. Execute snmp command to make sure it works. [v2]
        4. Disbale snmp v2 agent.        
        5. Check snmp command: snmpv2 can't works.

Commands samples:
tea.py u.snmp.zd.verify_cfg_agent_cli_en_both_dis_v2
tea.py u.snmp.zd.verify_cfg_agent_cli_en_both_dis_v2 ip_addr='192.168.0.10' username='admin' password='admin' shell_key='!v54!'

#set agent config for snmp version 2.
tea.py u.snmp.zd.verify_cfg_agent_cli_en_both_dis_v2 ro_community='public' rw_community='private'
'''
...
def do_test(conf):
    try:
        ...
        logging.info('Step 1. Enable snmp v2 and v3 agent.')
        ...
        logging.info("Step 3. Execute snmp command to make sure it works. [v2 and v3]")
        snmp_agent_cfg['enabled'] = True        
        snmp_agent_cfg['version'] = 2
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
            res_d['Step 3-V2'] = result
        
        snmp_agent_cfg['version'] = 3
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
            res_d['Step 3-V3'] = result
            
        logging.info("Step 4. Disable snmp v2 agent.")
        ...
        logging.info("Step 5. Check snmp command: snmpv2 can't works. snmpv3 works well.")
        snmp_agent_cfg['enabled'] = False
        snmp_agent_cfg['version'] = 2        
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
            res_d['Step 5-V2'] = result
        
        snmp_agent_cfg['enabled'] = True
        snmp_agent_cfg['version'] = 3
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
            res_d['Step 5-V3'] = result
