...
@summary:
     ...
     Steps:
        ...
        3. Execute snmp command to make sure it works. [v3]
        4. Disbale snmp v3 agent.        
        5. Check snmp command: snmpv3 can't works. 

Commands samples:
tea.py u.snmp.zd.verify_cfg_agent_cli_en_both_dis_v3
tea.py u.snmp.zd.verify_cfg_agent_cli_en_both_dis_v3 ip_addr='192.168.0.10' username='admin' password='admin' shell_key='!v54!'

#set agent config for snmp version 3.
tea.py u.snmp.zd.verify_cfg_agent_cli_en_both_dis_v3 ro_sec_name='ruckus-read' ro_auth_protocol='MD5'
...
def do_test(conf):
    try:
        ...
        logging.info("Step 4. Disable snmp v3 agent.")
        ...
        logging.info("Step 5. Check snmp command: snmpv3 can't works. snmpv2 works well.")
        snmp_agent_cfg['enabled'] = True
        snmp_agent_cfg['version'] = 2
        result = verify_snmp_agent(conf['snmp'], conf['zd_cli'], snmp_agent_cfg)
        if result:
