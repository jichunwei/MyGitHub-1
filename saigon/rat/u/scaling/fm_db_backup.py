...
from u.scaling.lib.common_remote_control import backupFMDB, checkFMIso,pingFM
...
def do_test(cfg):
    if not pingFM(cfg['linux_srv_ip']): #FM must be pingable, then do following tests
        ...
        res2 = backupFMDB(res['fm_obj'],cfg['iso'])
        res.update(res2)
    
    print "####################################################################"
    print "[info] Backup FM database successfully                              "
...
def main(**kwa):
    ...
    res = do_test(tcfg)
    do_clean_up(tcfg)
    send_mail("172.16.100.20", tcfg['email'], "RAT <webber.lin@ruckuswireless.com>", "FM database is backup successfully", "Result: %s" % res )
    return res
