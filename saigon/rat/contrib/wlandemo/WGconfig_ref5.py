...
import defaultWlanConfigParams_ref as DWCFG
...
def do_station_test(**kwargs):
    fcfg = dict(sta_ip_addr = '192.168.1.10', sleep = 3, repeat = 1, debug = False)
    fcfg.update(kwargs)
    halt(fcfg['debug'])
...
#
def main(**kwargs):
    fcfg = dict(debug = False, repeat = 1, sleep = 3, do_config = True, do_cleanup = False)
    fcfg.update(kwargs)
