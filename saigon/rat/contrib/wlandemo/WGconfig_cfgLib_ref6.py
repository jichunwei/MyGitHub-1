...
# create users on ZD
def create_users(zd, username = 'TestingisFunJob', password = 'TestingisFunJob', fullname = 'TestingisFunJob', number_of_users = 1):
    if number_of_users == 1:
        zd.create_user(username, password, fullname)
    if number_of_users > 1:
        for u in range(number_of_users):
            username = "%s%d" % (username, u + 1)
            password = "%s%d" % (password, u + 1)
            fullname = "%s%d" % (fullname, u + 1)
        zd.create_user(username, password, fullname)
        username = 'TestingisFunJob'
        password = 'TestingisFunJob'
        fullname = 'TestingisFunJob'
    else:
        pass
...
def create_multi_wlan_groups(zd, mytb):
    ...
    _create_multi_wlan_groups(zd, mytb['wgs_name'], ssid_list, num_of_wgs = 10, description = 'AutomationIsfuntoolforQATeam', pause = 1.0)
...
def create_mac_list(acl_conf, num_of_mac = 10):
    if num_of_mac > 1:
...
def create_l2_acl_policy(zd, acl_conf, num_of_acl = 1):
    ...
    if num_of_acl == 1:
        ...
        for c in range(num_of_acl):
            ...
            logging.info("Create [acl_name %s] with [mac_addr %s]" % (acl_conf['acl_name'], acl_conf['mac_list']))
            ACL.create_l2_acl_policy(zd, acl_conf, pause = 1)
            acl_conf['acl_name'] = 'TestForFunL2ACL'

# A method of creating L2ACL w/ a list of MAC addresses and clone it up to Max. L2 ACL
def create_l2_acl_clone(zd, acl_conf, num_of_acl = 1):
    new_acl_conf = {}
    ACL.delete_all_l2_acl_policies(zd, pause = 1)
    create_mac_list(acl_conf)
    if num_of_acl == 1:
        ...
        logging.info("Create [acl_name %s] with [mac_addr %s]" % (acl_conf['acl_name'], acl_conf['mac_list']))
        for c in range(num_of_acl):
            new_acl_conf['acl_name'] = "%s-%d" % (acl_conf['acl_name'], c + 1)
            ACL.clone_l2_acl_policy(zd, acl_conf['acl_name'], new_acl_conf, pause = 1)
            logging.info("Clone [new_acl_name %s] with [mac_addr %s]" % (new_acl_conf['acl_name'], acl_conf['mac_list']))
            new_acl_conf['acl_name'] = 'TestForFunL2ACL'
...
# A method of creating L3L4 policy and Clone it to Max. policies & rules
def create_l3_acl_clone(zd, acl_conf, num_of_acl = 1):
    new_acl_conf = {}
    ACL.delete_all_l3_acl_policies(zd, pause = 1)
    if num_of_acl == 1:
        ACL.create_l3_acl_policy(zd, acl_conf, pause = 1)
        ...
        logging.info("Create [acl_name %s] with [rules]" % (acl_conf['name']))
        for c in range(num_of_acl):
            new_acl_conf['name'] = "%s-%d" % (acl_conf['name'], c + 1)
            ACL.clone_l3_acl_policy(zd, acl_conf['name'], new_acl_conf, pause = 1)
            logging.info("Create [acl_name %s] with [rules]" % (acl_conf['name']))
            new_acl_conf['name'] = 'TestForFunL3ACL'
