...
import WGconfig_cfgLib_ref5 as BCLIB
...
params = {'username':'jchu', 'password':'happy', 'redirect_url':'http://192.168.0.202', 'original_url':'http://192.168.0.210'}
param = {'guest_pass':'', 'use_tou':True, 'redirect_url':'http://192.168.0.202'}
    ...
    for r in range(1, int(fcfg['repeat']) + 1):
        ...
        for i in range(10):
            ...
            if wlan_id in ['guest-1', 'guest-2', 'guest-3']:
                for c in range(1, 10):
                    try:
                        sta.do_cmd("perform_guest_auth", str(param))
                        time.sleep(5)
                    except:
                        pass
            ...
            if wlan_id in ['hotspot-1', 'hotspot-2', 'hotspot-3']:
                for c in range(1, 10):
                    try:
                        sta.do_cmd("perform_hotspot_auth", str(params))
                        time.sleep(5)
                    except:
                        pass
...
def do_station3_test(**kwargs):
    fcfg = dict(sta_ip_addr = '192.168.3.12', sleep = 10, repeat = 1, debug = False)
    fcfg.update(kwargs)
    halt(fcfg['debug'])
    sta = RPC(dict(sta_ip_addr = fcfg['sta_ip_addr']))
    sta.do_cmd('get_current_status')
